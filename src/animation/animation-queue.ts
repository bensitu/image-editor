...
export class AnimationQueue {
    ...
    clear(reason?: unknown): void {
        // Snapshot and detach the pending list before settling so a
        // resolve/reject handler that synchronously calls `add` again
        // observes a fresh queue rather than mutating the iteration.
    ...
    waitForIdle(): Promise<void> {
        ...
        return this.add(() => Promise.resolve()).then(
            () => undefined,
            () => undefined,
        );
    }
