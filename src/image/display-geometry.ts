...
export function measureLayoutViewport(
    ...
    return context.viewportCache.measure(
...
export function getScrollbarStableViewportCanvasSize(viewport: ViewportSize): ViewportSize {
    return {
...
export function shouldNormalizeCanvasSizeAfterStateRestore(
    context: DisplayGeometryContext,
): boolean {
    const originalImage = context.getOriginalImage();
    if (!context.canvas || !originalImage) return false;

    originalImage.setCoords();
    const boundingRect = originalImage.getBoundingRect();
    const viewport = measureLayoutViewport(
        context,
        measureScrollbarSize(context.containerElement?.ownerDocument ?? null),
    );
...
export function settleFitCoverScrollbarsAfterStateRestore(context: DisplayGeometryContext): void {
    if (
        !context.canvas ||
        !context.containerElement ||
...
export function captureImageDisplayGeometry(
    context: DisplayGeometryContext,
): ImageDisplayGeometry | null {
    const originalImage = context.getOriginalImage();
    if (!context.canvas || !originalImage) return null;
    originalImage.setCoords();
    const boundingRect = originalImage.getBoundingRect();
    return {
...
export function restoreMergedImageDisplayGeometry(
    context: DisplayGeometryContext,
    geometry: ImageDisplayGeometry | null,
): void {
    const originalImage = context.getOriginalImage();
    if (!geometry || !context.canvas || !originalImage) return;
