 ...
 * the user-supplied callback. The stored `element` is the exact DOM node that
 * received the listener so cleanup remains correct when frameworks replace
 * nodes during re-render.
 */
...
export class DomBindings {
    ...
    private readonly resolveElement: (key: ElementKey) => HTMLElement | null | undefined;
     ...
     */
    constructor(
        resolveElement: (key: ElementKey) => HTMLElement | null | undefined,
        isDisposed: () => boolean,
    ) {
