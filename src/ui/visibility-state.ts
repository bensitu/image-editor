...
export function setPlaceholderVisible(
    ...
    // Placeholder side. `hidden` is the inverse of `show` because
    // `show === true` means the placeholder is the visible thing.
    if (placeholderElement) {
        placeholderElement.hidden = !show;
        placeholderElement.setAttribute('aria-hidden', show ? 'false' : 'true');
    }

    // Canvas container side. `hidden` matches `show` because the container
    // is hidden exactly when the placeholder is visible. Skipped when the
    // container reference is missing — the placeholder
    // side has already been updated above.
    if (containerElement) {
