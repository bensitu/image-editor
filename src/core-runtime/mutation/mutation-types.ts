...
import type {
    ...
    OperationDefinition,
    OperationToken,
} from '../../plugin-kernel/operation-registry.js';
import type { CoreMemento } from '../state/state-types.js';

export type DocumentMutationKind =
    'geometry' | 'raster' | 'overlay' | 'plugin-state' | 'compound' | (string & {});
...
export interface DocumentMutationRollbackContext<
    TResult = unknown,
...
export interface DocumentMutationParticipant<TResult = unknown, TPrepared = unknown> {
    ...
    rollback?(
        prepared: TPrepared,
        context: DocumentMutationRollbackContext<TResult>,
    ): MaybePromise<void>;
}
...
export interface DocumentMutationMementoPort {
    ...
    restore(
        memento: CoreMemento,
        options?: Readonly<{ rollbackOnFailure?: boolean; signal?: AbortSignal }>,
    ): Promise<void>;
...
export interface DocumentMutationOperationPort {
    ...
    run<TResult>(
        operationId: string,
        task: (context: DocumentMutationOperationContext) => MaybePromise<TResult>,
        options?: Readonly<{ parent?: OperationToken; signal?: AbortSignal }>,
    ): Promise<TResult>;
}
