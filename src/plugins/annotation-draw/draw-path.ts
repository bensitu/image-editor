...
export const MAX_DRAW_COORDINATE = 10_000_000;

export function normalizeDrawPoint(value: unknown): AnnotationPoint {
...
export function buildCurvedDrawPath(points: readonly AnnotationPoint[]): string {
...
function transformPathPoint(
    object: FabricNS.FabricObject & { pathOffset?: Partial<AnnotationPoint> },
    point: AnnotationPoint,
): AnnotationPoint {
...
export function drawPathIntersects(
    object: FabricNS.FabricObject & {
        editorDrawPoints?: readonly AnnotationPoint[];
        pathOffset?: Partial<AnnotationPoint>;
    },
    eraserPoints: readonly AnnotationPoint[],
    eraserRadius: number,
): boolean {
    ...
    const strokeRadius =
        ((Number(object.strokeWidth) || 0) *
            Math.max(
                Math.abs(Number(scale?.x) || Number(object.scaleX) || 1),
                Math.abs(Number(scale?.y) || Number(object.scaleY) || 1),
            )) /
        2;
    ...
    for (const eraserPoint of eraserPoints) {
        for (let index = 1; index < transformed.length; index += 1) {
            if (
                distanceToSegment(eraserPoint, transformed[index - 1]!, transformed[index]!) <=
                hitRadius
            ) {
                return true;
            }
