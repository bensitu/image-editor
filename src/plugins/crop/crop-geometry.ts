...
export function normalizeCropRotation(value: unknown): number {
...
export function constrainCropRectToRotation(
    value: CropRect,
    rotationDegrees: number,
    limits: CropRectLimits,
): CropRect {
    ...
    const scale = Math.min(
        1,
        limits.widthPx / Math.max(rotatedWidth, 1),
        limits.heightPx / Math.max(rotatedHeight, 1),
    );
    if (scale < 1) {
        ...
        if (
            widthPx * cosine + heightPx * sine > limits.widthPx + 1e-6 ||
            widthPx * sine + heightPx * cosine > limits.heightPx + 1e-6
        ) {
            throw new TypeError(
                '[ImageEditor] Rotated Crop rectangle cannot satisfy the configured minimum.',
            );
        }
    ...
    const centerX = Math.max(
        extentX,
        Math.min(limits.widthPx - extentX, rect.leftPx + rect.widthPx / 2),
    );
    const centerY = Math.max(
        extentY,
        Math.min(limits.heightPx - extentY, rect.topPx + rect.heightPx / 2),
    );
    return normalizeCropRect(
        {
            leftPx: Math.max(
                0,
                Math.min(limits.widthPx - rect.widthPx, centerX - rect.widthPx / 2),
            ),
            topPx: Math.max(
                0,
                Math.min(limits.heightPx - rect.heightPx, centerY - rect.heightPx / 2),
            ),
