...
const SCHEMA = 'image-editor.overlay-state' as const;
const COORDINATE_SPACE = 'image-normalized' as const;
...
export class OverlayStateV1MigrationError extends TypeError {
    override readonly name = 'OverlayStateV1MigrationError';
...
function numberValue(value: unknown, path: string, fallback?: number): number {
    if (value === undefined && fallback !== undefined) return fallback;
...
function metadata(value: unknown, path: string): Readonly<Record<string, unknown>> {
    ...
    return cloneJson(record(value, path), path) as Readonly<Record<string, unknown>>;
}
...
function rotate(pointValue: Point, origin: Point, degrees: number): Point {
...
function boundsCorners(
    ...
    height: number,
    angle: number,
): Readonly<{ readonly type: 'bounds'; readonly corners: readonly [Point, Point, Point, Point] }> {
    ...
    return Object.freeze({
        ...
        ]) as readonly [Point, Point, Point, Point],
    });
}

function scalarFromPixels(value: number, image: OverlayStateImageReference): number {
    return value / Math.min(image.naturalWidth, image.naturalHeight);
}

function scalarFromXRatio(value: number, image: OverlayStateImageReference): number {
...
function dashArray(
    ...
    return Object.freeze(
        value.map((entry, index) =>
            scalarFromPixels(numberValue(entry, `${path}[${index}]`), image),
        ),
    );
}

function interaction(source: JsonRecord, editable?: boolean): Readonly<Record<string, boolean>> {
    return Object.freeze({
...
function item(
    ...
    geometry: unknown,
    data: unknown,
    itemMetadata?: Readonly<Record<string, unknown>>,
): OverlayStateItem {
    return Object.freeze({
        id,
        kind,
...
function uniqueId(id: string, reserved: Set<string>): string {
    if (!reserved.has(id)) {
        reserved.add(id);
        return id;
    }
...
function convertMask(
    ...
    if (shape === 'rect' && geometry.type === 'rect') {
        ...
        points = Object.freeze(
            absolute.map((entry) =>
                Object.freeze({ x: (entry.x - left) / width, y: (entry.y - top) / height }),
            ),
        );
    } else {
    ...
    return item(
        ...
        source.hidden === true,
        false,
        boundsCorners(left, top, width, height, angle),
        Object.freeze({
            version: 1,
            kind: shape,
...
function convertText(
    ...
    return item(
        ...
        source.hidden === true,
        source.locked === true,
        boundsCorners(x, y, width, height, angle),
        annotationEnvelope(
            id,
            source,
            Object.freeze({
...
function convertShape(
    ...
    const style = record(source.style, `${path}.style`);
    const angle = numberValue(geometry.angle, `${path}.geometry.angle`, 0);
    let stateGeometry: unknown;
    if (shape === 'rect' && geometry.type === 'rect') {
        stateGeometry = Object.freeze({
            kind: 'rect',
            bounds: boundsCorners(
                numberValue(geometry.x, `${path}.geometry.x`),
                numberValue(geometry.y, `${path}.geometry.y`),
                positive(geometry.width, `${path}.geometry.width`),
                positive(geometry.height, `${path}.geometry.height`),
                angle,
            ),
        });
    ...
    return Object.freeze(
        source.strokes.map((entry, strokeIndex) => {
            ...
            return item(
                partId,
                'annotation:draw',
                layerStart + strokeIndex,
                source.hidden === true,
                source.locked === true,
                Object.freeze({ type: 'path', points }),
                annotationEnvelope(
                    partId,
                    source,
                    Object.freeze({
                        ...
                        lineCap:
                            brush.lineCap === 'butt' || brush.lineCap === 'square'
                                ...
                                : 'round',
                        lineJoin:
                            brush.lineJoin === 'bevel' || brush.lineJoin === 'miter'
                                ? brush.lineJoin
                                : 'round',
                    }),
                ),
...
export function migrateV1OverlayState(
    ...
    if (source.baseImageTransform !== undefined) {
        ...
        const meaningful =
            numberValue(transform.rotation, '$.baseImageTransform.rotation', 0) !== 0 ||
    ...
    const overlays: OverlayStateItem[] = [];
