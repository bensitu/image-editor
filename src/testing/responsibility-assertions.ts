...
export interface FabricGlobalMutationLifecycle<
    TModule = unknown,
    TDefinition extends {
        readonly manifest?: { readonly permissions?: readonly PluginPermission[] };
    } = {
        readonly manifest?: { readonly permissions?: readonly PluginPermission[] };
    },
    TRuntime = unknown,
...
async function resolveProof<T>(source: ProofSource<T>): Promise<T> {
    return typeof source === 'function' ? (source as () => MaybePromise<T>)() : source;
}

async function evaluate<T>(
    id: string,
    contract: string,
    source: ProofSource<T> | undefined,
    inspect: (observation: T) => Readonly<{
        passed: boolean;
        message: string;
        details?: Readonly<Record<string, unknown>>;
    }>,
): Promise<ConformanceAssertionResult> {
    if (!source) return unavailable(id, contract);
    try {
        const inspected = inspect(await resolveProof(source));
        return result(
            id,
            contract,
            inspected.passed ? 'PASS' : 'FAIL',
            inspected.message,
            inspected.details,
        );
    } catch (error) {
...
function isBundledCoreModule(moduleId: string): boolean {
    const normalized = moduleId.replace(/\\/gu, '/');
    return /(?:^|\/)node_modules\/@bensitu\/image-editor(?:\/|$)/u.test(normalized);
}

function isBundledFabricModule(moduleId: string): boolean {
    const normalized = moduleId.replace(/\\/gu, '/');
    return /(?:^|\/)node_modules\/fabric(?:\/|$)/u.test(normalized);
}

export function assertBundleIsolation(
    source?: ProofSource<BundleIsolationObservation>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'bundle-isolation',
        'The package uses public imports and has no private or unknown bundled modules.',
        source,
        (observation) => {
            const bundledRuntimeModules = observation.moduleIds.filter(
                (moduleId) => isBundledCoreModule(moduleId) || isBundledFabricModule(moduleId),
            );
            const passed =
                observation.internalImports === 0 &&
                observation.privateAliases === 0 &&
                (observation.unknownModules ?? 0) === 0 &&
            ...
            return {
                ...
                details: Object.freeze({
                    ...
                    unknownModules: observation.unknownModules ?? 0,
...
function descriptorFingerprint(descriptor: PropertyDescriptor): DescriptorFingerprint {
...
function captureSurface(
    surfaces: Map<string, ReadonlyMap<PropertyKey, DescriptorFingerprint>>,
    name: string,
    value: object,
): void {
    const properties = new Map<PropertyKey, DescriptorFingerprint>();
...
export function captureFabricGlobalState(
    fabric: Readonly<Record<PropertyKey, unknown>>,
): GlobalSnapshot {
    const surfaces = new Map<string, ReadonlyMap<PropertyKey, DescriptorFingerprint>>();
...
function descriptorsEqual(left: DescriptorFingerprint, right: DescriptorFingerprint): boolean {
    return (
...
function globalDifferences(before: GlobalSnapshot, after: GlobalSnapshot): readonly string[] {
    const differences = new Set<string>();
    const surfaceNames = new Set([...before.keys(), ...after.keys()]);
    for (const surfaceName of surfaceNames) {
        const left = before.get(surfaceName) ?? new Map();
        const right = after.get(surfaceName) ?? new Map();
...
export async function assertNoUndeclaredFabricGlobalMutation(
    lifecycle?: FabricGlobalMutationLifecycle,
): Promise<ConformanceAssertionResult> {
    const id = 'fabric-global-mutation';
    const contract =
        'Known Fabric global surfaces remain unchanged unless global mutation permission is declared.';
    if (!lifecycle) return unavailable(id, contract);
    const changes: Array<Readonly<{ phase: string; surfaces: readonly string[] }>> = [];
    let previous = captureFabricGlobalState(lifecycle.fabric);
    let definition: Awaited<ReturnType<typeof lifecycle.createDefinition>> | undefined;
    let runtime: unknown;
    try {
        ...
        if (surfaces.length > 0) changes.push(Object.freeze({ phase: 'import', surfaces }));
        previous = current;
        ...
        if (surfaces.length > 0) changes.push(Object.freeze({ phase: 'definition', surfaces }));
        previous = current;

        runtime = await lifecycle.setup(definition);
        current = captureFabricGlobalState(lifecycle.fabric);
        surfaces = globalDifferences(previous, current);
        if (surfaces.length > 0) changes.push(Object.freeze({ phase: 'setup', surfaces }));
        previous = current;

        await lifecycle.dispose(runtime, definition);
        current = captureFabricGlobalState(lifecycle.fabric);
        surfaces = globalDifferences(previous, current);
        if (surfaces.length > 0) changes.push(Object.freeze({ phase: 'dispose', surfaces }));
    } catch (error) {
        return result(id, contract, 'FAIL', describeError(error));
    }

    const permissions =
        lifecycle.declaredPermissions ?? definition?.manifest?.permissions ?? Object.freeze([]);
    const declared = permissions.includes('fabric:global-mutation');
    const changedSurfaces = Object.freeze(
        [...new Set(changes.flatMap((entry) => entry.surfaces))].sort(),
    );
    ...
    if (declared) {
        return result(
            id,
            contract,
            'PASS_WITH_DOWNGRADED_ISOLATION',
            'Declared Fabric global mutation requires downgraded isolation.',
            details,
        );
    }
    return result(
        ...
        contract,
        'FAIL',
        'Fabric global mutation was detected without the required declaration.',
        details,
    );
}

export function assertStrongMultiInstanceIsolation(
    source?: ProofSource<MultiInstanceObservation>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'multi-instance-isolation',
        'Core, Plugin, operation, tool, Overlay, History, and Fabric state are isolated by instance.',
        source,
        (observation) => {
...
export function assertPeerDependencyContract(
    source?: ProofSource<PeerDependencyManifest>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'peer-dependency-contract',
        'Core and Fabric are peers and are never vendored by the Plugin package.',
        source,
        (manifest) => {
            ...
            const missingPeers = requiredPeers.filter(
                (dependency) => typeof peerDependencies[dependency] !== 'string',
            );
            const vendored = requiredPeers.filter(
                (dependency) =>
                    dependency in runtimeDependencies ||
...
export function assertPackageDoesNotBundleCoreOrFabric(
    source?: ProofSource<PackageModuleObservation>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'package-runtime-externalization',
        'The Plugin artifact contains no Core or Fabric runtime module.',
        source,
        (observation) => {
            const core =
                observation.bundledCoreModules ??
                observation.moduleIds.filter(isBundledCoreModule).length;
            const fabric =
                observation.bundledFabricModules ??
                observation.moduleIds.filter(isBundledFabricModule).length;
            return {
...
export function assertBaseImageInvariant(
    source?: ProofSource<BaseImageInvariantObservation>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'base-image-invariant',
        'Prohibited Base Image actions are rejected without observable document effects.',
        source,
        (observation) => {
            ...
            return {
                ...
                message:
                    observation.attempts.length > 0 && failed.length === 0
...
export function assertOverlayMutationHistory(
    source?: ProofSource<OverlayMutationHistoryObservation>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'overlay-mutation-history',
        'A committed Overlay mutation owns one transaction, History record, and event.',
        source,
        (observation) => {
            const passed =
                observation.topLevelTransactions === 1 &&
...
export function assertCompoundTransaction(
    source?: ProofSource<CompoundTransactionObservation>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'compound-transaction',
        'Compound work commits one Memento pair, History record, and event with atomic restore.',
        source,
        (observation) => {
            const passed =
                observation.topLevelTransactions === 1 &&
...
export function assertSliceMigration(
    source?: ProofSource<SliceMigrationObservation>,
): Promise<ConformanceAssertionResult> {
    return evaluate(
        'slice-migration',
        'Plugin-owned Slice migration is deterministic, validated, isolated, and failure-atomic.',
        source,
        (observation) => {
            const passed =
                Number.isSafeInteger(observation.sourceVersion) &&
