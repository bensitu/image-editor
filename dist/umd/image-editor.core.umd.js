	...
	var OperationRegistry = class {
		...
		attachExternalAbort(request) {
			...
			const abort = () => {
				...
				const reason = signal ? abortReason$1(signal, `Operation "${request.record.definition.id}" was aborted.`) : abortError(`Operation "${request.record.definition.id}" was aborted.`);
				if (request.state === "pending") {
					this.pendingRequests = this.pendingRequests.filter((entry) => entry !== request);
