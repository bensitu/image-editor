(function(global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ?  factory(exports, require('@bensitu/image-editor/sdk'), require('@bensitu/image-editor/plugins/overlay'), require('@bensitu/image-editor/core')) :
  typeof define === 'function' && define.amd ? define(['exports', '@bensitu/image-editor/sdk', '@bensitu/image-editor/plugins/overlay', '@bensitu/image-editor/core'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory((global.ImageEditorPlugins = global.ImageEditorPlugins || {},global.ImageEditorPlugins.Mask = global.ImageEditorPlugins.Mask || {}), global.ImageEditor,global.ImageEditorPlugins.Overlay,global.ImageEditor));
...
//#region dist/esm/plugins/mask/mask-controller.js
	...
	var MaskPluginController = class {
			...
			this.disposables.add(overlay.registerKind({
				...
				stateCodec: {
					...
					serialize: (object, context) => {
						if (!isMaskObject(object)) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Mask State Codec received a non-mask.");
						const kind = maskStateKind(object);
						const metadata = object.overlayMetadata;
						return Object.freeze({
							geometry: (0, _bensitu_image_editor_plugins_overlay.captureOverlayStateBounds)(object, context),
					...
					validate: (value) => (0, _bensitu_image_editor_plugins_overlay.isOverlayStateBoundsGeometry)(value.geometry) && isMaskStateData(value.data) && isPlainRecord(value.metadata),
					deserialize: (value, context) => {
						if (!(0, _bensitu_image_editor_plugins_overlay.isOverlayStateBoundsGeometry)(value.geometry) || !isMaskStateData(value.data) || !isPlainRecord(value.metadata)) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Serialized Mask State data is malformed.");
						...
						(0, _bensitu_image_editor_plugins_overlay.restoreOverlayStateBounds)(mask, value.geometry, context, this.host.fabric);
		...
		create(config = {}) {
			return this.overlay.mutate({
				...
				mutate: () => {
					...
					if (!mask) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Mask configuration is invalid.");
					return mask;
				},
		...
		remove(id) {
			...
			if (!object || !isMaskObject(object)) return Promise.reject(new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Mask "${id}" was not found.`));
			return this.overlay.mutate({
		...
		serializeMask(object) {
			if (!isMaskObject(object)) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Mask serializer received a non-mask.");
		...
		async deserializeMask(data, fabricModule) {
			if (!isSerializedMaskData(data)) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Serialized Mask data is malformed.");
			const object = (await fabricModule.util.enlivenObjects([data.object]))[0];
			if (!object) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Fabric did not restore a Mask object.");
			const mask = object;
			mask.editorObjectKind = "mask";
			mask.maskId = data.maskId;
...
//#region dist/esm/plugins/mask/index.js
	const maskPluginRef = (0, _bensitu_image_editor_sdk.definePluginRef)("plugin:mask", "1.0.0");
	function maskPlugin(options = {}) {
		...
		return (0, _bensitu_image_editor_sdk.definePlugin)({
			...
			manifest: {
				...
				engine: "^3.0.0",
				requiresPlugins: [_bensitu_image_editor_plugins_overlay.overlayFoundationRef],
				requires: [
					{
					...
					{
						token: _bensitu_image_editor_sdk.CANVAS_RESIZE_CAPABILITY,
						range: "^1.0.0"
					},
					{
					...
					{
						token: _bensitu_image_editor_plugins_overlay.OVERLAY_REGISTRATION_CAPABILITY,
						range: "^1.0.0"
					}
				],
				permissions: [
			...
			setup(context) {
				...
				const resize = context.capabilities.require(_bensitu_image_editor_sdk.CANVAS_RESIZE_CAPABILITY);
				...
				const overlayRegistration = context.capabilities.require(_bensitu_image_editor_plugins_overlay.OVERLAY_REGISTRATION_CAPABILITY);
...
//# sourceMappingURL=image-editor.plugin.mask.umd.js.map