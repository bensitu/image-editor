(function(global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory((global.ImageEditorPlugins = global.ImageEditorPlugins || {},global.ImageEditorPlugins.Overlay = global.ImageEditorPlugins.Overlay || {}), global.ImageEditor,global.ImageEditor));
...
//#region dist/esm/foundations/overlay/overlay-errors.js
	var OverlayRecoverableObjectError = class extends _bensitu_image_editor_core.CoreRuntimeError {
...
//#region dist/esm/foundations/overlay/overlay-foundation-controller.js
	...
	function freezePersistence(definition) {
		...
		const failure = (message) => {
			throw new _bensitu_image_editor_sdk.PluginManifestError(`Plugin "${definition.ownerPluginId}" Overlay Kind "${definition.id}" ${message}`, { pluginId: definition.ownerPluginId });
		};
		...
		if (!isRecord(codec) || typeof codec.type !== "string" || !(0, _bensitu_image_editor_sdk.isRuntimeIdentifier)(codec.type) || typeof codec.version !== "string" || !(0, _bensitu_image_editor_sdk.isValidSemVer)(codec.version) || typeof codec.serialize !== "function" || typeof codec.validate !== "function" || typeof codec.deserialize !== "function") return failure("requires a valid Codec with type, SemVer version, serialize, validate, and deserialize.");
	...
	function isSerializedRecord(value) {
		return isRecord(value) && typeof value.kind === "string" && value.kind.trim().length > 0 && typeof value.persistentId === "string" && OVERLAY_ID_PATTERN.test(value.persistentId) && typeof value.hidden === "boolean" && typeof value.locked === "boolean" && isRecord(value.codec) && typeof value.codec.type === "string" && (0, _bensitu_image_editor_sdk.isRuntimeIdentifier)(value.codec.type) && typeof value.codec.version === "string" && (0, _bensitu_image_editor_sdk.isValidSemVer)(value.codec.version) && Object.prototype.hasOwnProperty.call(value, "data");
	}
	...
	var OverlayFoundationController = class {
		constructor(host, state, mementos, geometry, mutations, exportPort) {
			...
			Object.defineProperty(this, "onObjectModified", {
				...
				value: (event) => {
					...
					if (eventIds.size > 0 && this.activeGesture.targets.some((entry) => !eventIds.has(entry.persistentId))) {
						this.failGesture(this.activeGesture, new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay gesture target changed before commit."));
						return;
					}
			...
			try {
				...
				(0, _bensitu_image_editor_sdk.disposeInReverseSync)(this.registrations, { pluginId: OVERLAY_STATE_ID });
		...
		registerKind(definition) {
			...
			if (!isRecord(definition) || typeof definition.classify !== "function" || typeof definition.getPersistentId !== "function" || definition.setPersistentId !== void 0 && typeof definition.setPersistentId !== "function") throw new _bensitu_image_editor_sdk.PluginManifestError("Overlay Kind registration requires callable classify and persistent identity members.", isRecord(definition) && typeof definition.ownerPluginId === "string" ? { pluginId: definition.ownerPluginId } : {});
			...
			if (existing) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay kind "${definition.id}" is already registered by "${existing.definition.ownerPluginId}".`);
			const record = {
		...
		registerGeometryPolicy(policy) {
			...
			if (this.policies.has(policy.kind)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay kind "${policy.kind}" already has a geometry policy.`);
		...
		registerInteractionPolicy(policy) {
			...
			this.requireKindOwner(policy.kind, policy.ownerPluginId);
			if (this.interactionPolicies.has(policy.kind)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay kind "${policy.kind}" already has an interaction policy.`);
			const frozen = Object.freeze({ ...policy });
		...
		registerExportRenderer(renderer) {
			...
			if (!Number.isFinite(renderer.order)) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay export renderer order must be finite.");
			if (this.renderers.has(renderer.kind)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay kind "${renderer.kind}" already has an export renderer.`);
		...
		async mutate(request) {
			...
			return this.mutations.run({
				...
				validate: async (result, transaction) => {
					...
					if (!currentDescriptor) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay mutation synchronization did not produce a descriptor.");
				...
				describeCommit: () => {
					if (!descriptor) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay mutation descriptor is unavailable at commit.");
					return descriptor;
				}
			});
		}
		add(objects) {
			...
			return this.mutate({
				...
				affectedObjects: () => {
					...
					if (indexed.length !== uniqueObjects.length || indexed.some((entry) => entry.kind.definition.persistence.mode !== "persistent")) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay insertion accepts only registered persistent kinds.");
		...
		addTransient(objects) {
			...
			return this.host.runOperation("overlay:transient", async () => {
				...
				try {
					...
					this.rebuildIndex();
					const indexed = this.resolveOverlayObjects(uniqueObjects);
					if (indexed.length !== uniqueObjects.length || indexed.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Transient overlay insertion accepts only registered transient kinds.");
					this.host.requestRender();
				} catch (error) {
		...
		replaceTransient(ids, objects) {
			...
			return this.host.runOperation("overlay:transient", async () => {
				...
				if (removed.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Transient overlay replacement accepts only transient kinds.");
				try {
					for (const entry of removed) canvas.remove(entry.object);
					for (const object of uniqueObjects) canvas.add(object);
					this.rebuildIndex();
					const inserted = this.resolveOverlayObjects(uniqueObjects);
					if (inserted.length !== uniqueObjects.length || inserted.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Transient overlay replacement produced an invalid kind.");
					this.host.requestRender();
				} catch (error) {
					for (const object of uniqueObjects) canvas.remove(object);
		...
		removeTransient(ids) {
			...
			return this.host.runOperation("overlay:transient", async () => {
				...
				if (entries.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Transient overlay removal accepts only transient kinds.");
		...
		async flatten(query = {}, options = {}) {
			...
			await this.geometry.run({
				...
				mutateBase: async ({ signal, transaction }) => {
					...
					if (!baseImage) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Cannot flatten without a base image.");
		...
		assertFlattenBudget(canvas = this.host.requireCanvas("validate flatten dimensions"), baseImage = this.host.getBaseImage()) {
			...
			if (!isRasterAllocationWithinBudget(canvas.getWidth(), canvas.getHeight(), budget) || !isRasterAllocationWithinBudget(region.width, region.height, budget)) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Flatten dimensions exceed the configured budget.");
		}
		dispose() {
			...
			const registrationErrors = (0, _bensitu_image_editor_sdk.disposeInReverseSync)(this.registrations, { pluginId: OVERLAY_STATE_ID });
			this.registrations.length = 0;
			...
			this.attached = false;
			this.disposed = true;
			if (registrationErrors.length > 0) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay Foundation disposal had ${registrationErrors.length} registration cleanup error(s).`);
		}
		captureState() {
			var _a;
			this.setPreviewObjectsHidden(false);
			try {
				...
				for (const object of (_a = canvas === null || canvas === void 0 ? void 0 : canvas.getObjects()) !== null && _a !== void 0 ? _a : []) {
					const marked = object;
					if (typeof marked.editorOverlayKind === "string" && !this.byObject.has(object)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Persistent overlay kind "${marked.editorOverlayKind}" is not registered.`);
				}
				const overlays = [];
				for (const object of this.indexedCanvasObjects()) {
					...
					if (!serializer) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay kind "${indexed.kind.definition.id}" has no serializer.`);
					const classification = this.classificationFor(indexed);
		...
		async restoreState(value) {
			var _a, _b;
			const canvas = this.host.getCanvas();
			if (!canvas) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay state restore requires Canvas.");
			const restored = [];
			const preserved = [];
			try {
				for (const record of value.overlays) {
					...
					if (!serializer.validate(record.data)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Serialized overlay "${record.persistentId}" is invalid.`);
					const object = await serializer.deserialize(record.data, { fabric: this.host.fabric });
					if (!object || typeof object !== "object" || object.canvas) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Serialized overlay "${record.persistentId}" restored an incompatible object.`);
			...
			try {
				...
				this.disposeDetachedObjects(restored.map(({ object }) => object), "Rejected Overlay restore object cleanup failed.");
				if (rollbackError) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay state restore and local rollback both failed.", {
		...
		async rebaseSupersedingGesture(gesture) {
			...
			for (const target of gesture.targets) {
				...
				if (!transform) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Superseding Overlay gesture lost target "${target.persistentId}".`);
		...
		async validateMutation(targets, descriptor, transaction) {
			...
			if (new Set(descriptor.objectIds).size !== descriptor.objectIds.length) throw new _bensitu_image_editor_core.CoreRuntimeError("[ImageEditor] Overlay mutation contains duplicate ids.");
			for (const target of targets) {
				if (target.kind.definition.getPersistentId(target.object) !== target.persistentId) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay "${target.persistentId}" changed persistent identity.`);
				if (descriptor.action !== "delete" && (!liveObjects.has(target.object) || ((_a = this.byId.get(target.persistentId)) === null || _a === void 0 ? void 0 : _a.object) !== target.object)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay "${target.persistentId}" is missing from the committed index.`);
			}
		...
		requireIndexed(id) {
			...
			if (!indexed) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay "${id}" was not found.`);
			return indexed;
		}
		requireKindOwner(kindId, ownerPluginId) {
			const kind = this.kinds.get(kindId);
			if (!kind) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay kind "${kindId}" is not registered.`);
			if (kind.definition.ownerPluginId !== ownerPluginId) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Overlay kind "${kindId}" belongs to "${kind.definition.ownerPluginId}", not "${ownerPluginId}".`);
		}
		emitSelection() {
			if (this.disposed) return;
		...
		assertRuntimeIdentifier(value, label) {
			if (!(0, _bensitu_image_editor_sdk.isRuntimeIdentifier)(value)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Invalid ${label} Runtime ID.`);
		}
		assertOpaqueIdentifier(value, label) {
			if (!OVERLAY_ID_PATTERN.test(value)) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] ${label} must be a safe identifier no longer than 128 characters.`);
		}
		assertActive(operation) {
			if (this.disposed) throw new _bensitu_image_editor_core.CoreRuntimeError(`[ImageEditor] Cannot ${operation} after disposal.`);
		}
	};
...
//#region dist/esm/foundations/overlay/index.js
	const OVERLAY_CAPABILITY = (0, _bensitu_image_editor_sdk.createCapabilityToken)("foundation:overlay", "1.0.0");
	const OVERLAY_REGISTRATION_CAPABILITY = (0, _bensitu_image_editor_sdk.createCapabilityToken)("foundation:overlay-registration", "1.0.0");
	const overlayFoundationRef = (0, _bensitu_image_editor_sdk.definePluginRef)("foundation:overlay", "1.0.0");
		...
		return (0, _bensitu_image_editor_sdk.definePlugin)({
			...
			manifest: {
				...
				requires: [
					...
					{
						token: _bensitu_image_editor_sdk.CORE_PRESENTATION_CAPABILITY,
						range: "^1.0.0"
					},
					{
					...
					{
						token: _bensitu_image_editor_sdk.EXPORT_CONTRIBUTION_CAPABILITY,
						range: "^1.0.0"
					},
					{
						token: _bensitu_image_editor_sdk.DOCUMENT_MUTATION_CAPABILITY,
						range: "^1.0.0"
					}
				],
				permissions: [
			...
			setup(context) {
				...
				const presentation = context.capabilities.require(_bensitu_image_editor_sdk.CORE_PRESENTATION_CAPABILITY);
				...
				const mutations = context.capabilities.require(_bensitu_image_editor_sdk.DOCUMENT_MUTATION_CAPABILITY);
...
//# sourceMappingURL=image-editor.plugin.overlay.umd.js.map