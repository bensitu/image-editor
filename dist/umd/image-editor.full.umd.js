...
//#region dist/esm/plugin-kernel/operation-registry.js
	...
	function abortReason$1(signal, fallback) {
		var _a;
	...
	var OperationRegistry = class {
		...
		run(operationId, ownerPluginId, args, task, options = {}) {
			...
			if ((_a = options.signal) === null || _a === void 0 ? void 0 : _a.aborted) return Promise.reject(abortReason$1(options.signal, `Operation "${operationId}" was aborted.`));
		...
		startRequest(request) {
			...
			const tracked = Promise.resolve(output).then((value) => ({
				...
				else if (active.controller.signal.aborted) this.rejectRequest(request, abortReason$1(active.controller.signal, `Operation "${active.token.id}" was aborted.`));
			...
			return new Promise((resolve, reject) => {
				...
				const abort = () => {
					...
					if (request.state === "active" && request.active && request.waiters.length === 1) {
						...
						this.abortActive(request.active, abortReason$1(signal, `Operation "${request.record.definition.id}" was aborted.`));
						return;
					}
					waiter.settled = true;
					...
					reject(abortReason$1(signal, `Operation "${request.record.definition.id}" was aborted.`));
		...
		abortRequestWithoutWaiters(request, signal) {
			var _a;
			const reason = abortReason$1(signal, `Operation "${request.record.definition.id}" was aborted.`);
		...
		attachExternalAbort(request) {
			...
			const abort = () => {
				...
				const reason = signal ? abortReason$1(signal, `Operation "${request.record.definition.id}" was aborted.`) : abortError$3(`Operation "${request.record.definition.id}" was aborted.`);
				if (request.state === "pending") {
					this.pendingRequests = this.pendingRequests.filter((entry) => entry !== request);
	...
	var ToolCoordinator = class {
		...
		invokeStatusListener(listener, status) {
			try {
				...
				if (isPromiseLike$1(result)) Promise.resolve(result).catch((error) => {
					reportErrorSafely(this.options.errorSink, error);
				});
			} catch (error) {
				reportErrorSafely(this.options.errorSink, error);
	...
	var PluginManager = class {
		...
		initializeSync() {
			...
			try {
				for (const pluginId of this.installationOrder) {
					...
					if (isPromiseLike$1(result)) throw new PluginLifecycleError(pluginId, "init", /* @__PURE__ */ new Error("Synchronous plugin onInit returned a Promise."));
				}
				this.hostState = "initialized";
			} catch (error) {
				this.hostState = "disposing";
		...
		performPendingInstallSync(plugin, visibleTransactions) {
			...
			try {
				...
				if (isPromiseLike$1(api)) throw new InvalidPluginDefinitionError(`Plugin "${plugin.ref.id}" returned a Promise from synchronous setup.`, plugin.ref.id);
		...
		performInstallSync(input) {
			...
			try {
				const contexts = this.createContexts(plugin.ref, scope, required, optional);
				const api = plugin.setup(contexts.setup);
				if (isPromiseLike$1(api)) throw new InvalidPluginDefinitionError(`Plugin "${pluginId}" returned a Promise from synchronous setup.`, pluginId);
...
//#endregion
//#region dist/esm/image/image-preprocessor.js
	...
	function uint32(bytes, offset, littleEndian) {
		...
		if (littleEndian) return bytes[offset] + bytes[offset + 1] * 256 + bytes[offset + 2] * 65536 + bytes[offset + 3] * 16777216;
	...
	function findJpegExifOrientationEntry(bytes) {
		...
		while (offset + 4 <= bytes.length) {
			...
			if (marker === 225 && segmentEnd - segmentStart >= 14 && matches(bytes, segmentStart, [
				69,
				120,
				105,
				102,
				...
				const littleEndian = matches(bytes, tiff, [73, 73]);
				const bigEndian = matches(bytes, tiff, [77, 77]);
				...
				for (let index = 0; index < entryCount; index += 1) {
					...
					if (uint16(bytes, entry, littleEndian) !== 274) continue;
					...
					return value !== null && value >= 1 && value <= 8 ? Object.freeze({
						offset: entry + 8,
						littleEndian,
						value
					}) : null;
	...
	function readJpegExifOrientation(input) {
		var _a, _b;
		return (_b = (_a = findJpegExifOrientationEntry(input instanceof Uint8Array ? input : new Uint8Array(input))) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : 1;
	}
	function dataUrlBytes(source) {
		...
		if (!header.endsWith(";base64")) {
			const bytes = [];
			for (let index = 0; index < encoded.length; index += 1) if (encoded[index] === "%" && /^[\da-f]{2}$/iu.test(encoded.slice(index + 1, index + 3))) {
				bytes.push(Number.parseInt(encoded.slice(index + 1, index + 3), 16));
				index += 2;
			} else {
				const value = encoded.charCodeAt(index);
				if (value > 255) throw new TypeError("[ImageEditor] Image Data URL contains invalid bytes.");
				bytes.push(value);
			}
		...
		if (typeof globalThis.atob !== "function") throw new TypeError("[ImageEditor] No base64 decoder is available.");
	...
	function bytesDataUrl(bytes, mimeType) {
		...
		if (typeof globalThis.btoa !== "function") throw new TypeError("[ImageEditor] No base64 encoder is available.");
		const chunks = [];
	...
	function orientedSize(width, height, orientation) {
		return orientation >= 5 ? Object.freeze({
			width: height,
			height: width
		}) : Object.freeze({
			width,
	...
	function targetSize(width, height, options) {
		if (!options.downsample) return Object.freeze({
			width,
			height,
			downsampled: false
		});
	...
	function requiresImagePreprocessing(mimeType, width, height, options) {
	...
	async function decodeWithImageBitmap(bytes, mimeType, signal) {
		...
		const bitmap = await globalThis.createImageBitmap(new Blob([copy], { type: mimeType }), { imageOrientation: "none" });
		if (signal.aborted) {
			bitmap.close();
	...
	function decodeWithImage(source, ownerDocument, signal) {
		return new Promise((resolve, reject) => {
			...
			image.onerror = () => finish(() => reject(/* @__PURE__ */ new TypeError("Image decode failed.")));
			image.onload = () => {
				finish(() => {
					const dimensions = imageDimensions(image);
					if (!(dimensions.width > 0) || !(dimensions.height > 0)) {
						reject(/* @__PURE__ */ new TypeError("Image decode produced invalid dimensions."));
						return;
					}
	...
	async function preprocessImageDataUrl(request) {
		var _a, _b;
		...
		if (!requiresImagePreprocessing(request.mimeType, request.width, request.height, request.options)) return Object.freeze({
		...
		if (!orientationNormalized && !target.downsampled && !needsEncoding) return Object.freeze({
			source: request.source,
			mimeType: request.mimeType,
			width: request.width,
			height: request.height,
		...
		try {
			...
			return Object.freeze({
				source,
				mimeType: emitted === "image/jpeg" || emitted === "image/png" || emitted === "image/webp" ? emitted : requestedMimeType,
				width: target.width,
...
//#region dist/esm/core-runtime/core-state-adapter.js
	...
	var CanvasCoreStateAdapter = class {
		...
		capture(context) {
			...
			if (!isRecord$10(serializedValue)) throw new SnapshotValidationError("Fabric canvas serialization must be an object.");
			...
			for (let index = 0; index < serializedObjects.length; index += 1) {
				...
				if (!isRecord$10(serializedObject) || !liveObject) continue;
		...
		validateState(value, publicInput) {
			...
			if (!isRecord$10(value.canvas)) return {
				valid: false,
			...
			if (value.imageMimeType !== null && value.imageMimeType !== void 0 && !isImageMimeType$1(value.imageMimeType)) return {
				valid: false,
			...
			return {
				...
				value: {
					...
					canvas: value.canvas,
					imageMimeType: isImageMimeType$1(value.imageMimeType) ? value.imageMimeType : null,
...
//#region dist/esm/core-runtime/state/clone-state-value.js
	...
	function assertSafeStateValue(value, seen = /* @__PURE__ */ new WeakSet(), path = "$") {
		var _a;
		if (!isObject$1(value) || seen.has(value)) return;
		seen.add(value);
...
//#region dist/esm/core-runtime/mutation/document-mutation-coordinator.js
	...
	const INTERACTIVE_MUTATION_BOUNDARY$1 = Symbol.for("@bensitu/image-editor/internal-interactive-mutation-boundary/v1");
	function getInteractiveMutationBoundary(request) {
		var _a;
		return (_a = Reflect.get(request, INTERACTIVE_MUTATION_BOUNDARY$1)) !== null && _a !== void 0 ? _a : null;
	}
...
//#region dist/esm/core-runtime/state/snapshot-service.js
	...
	function isUnsupportedCanvasEnvelope(value) {
		if ("schema" in value || !Array.isArray(value.objects) || !isRecord$9(value._editorState)) return false;
	...
	var SnapshotService = class {
		...
		validateEnvelope(value) {
			if (!isRecord$9(value)) throw new SnapshotValidationError("snapshot must be an object.");
			...
			if (!isRecord$9(value.core)) throw new SnapshotValidationError("core must be an object.", "$.core");
			if (!isRecord$9(value.plugins)) throw new SnapshotValidationError("plugins must be an object.", "$.plugins");
			...
			for (const [id, entry] of entries) {
				...
				if (!isRecord$9(entry) || !Number.isSafeInteger(entry.version) || Number(entry.version) <= 0) throw new SnapshotValidationError("plugin entry requires a positive integer version and data.", `$.plugins.${id}`);
...
//#region dist/esm/core-runtime/image-editor-core.js
	...
	function resolveImagePreprocessing(options, base = DEFAULT_CORE_OPTIONS.imagePreprocessing) {
		...
		return Object.freeze({
			...
			format: (options === null || options === void 0 ? void 0 : options.format) === null || isImageMimeType(options === null || options === void 0 ? void 0 : options.format) ? options.format : base.format,
	...
	var ImageEditorCore = class {
		...
		subscribeStatus(listener, options = {}) {
			...
			if (typeof listener !== "function") throw new TypeError("[ImageEditor] Status listener must be a function.");
			this.statusListeners.add(listener);
			if (options.emitCurrent !== false) this.invokeStatusListener(listener, this.getRuntimeStatus());
			let active = true;
		...
		on(eventName, listener) {
			...
			if (typeof listener !== "function") throw new TypeError("[ImageEditor] Core event listener must be a function.");
		...
		createCanvas(elements) {
			...
			this.containerElement = (_b = resolveElement$1(elements.canvasContainer, ownerDocument)) !== null && _b !== void 0 ? _b : canvasElement.parentElement;
		...
		async performImageLoad(source, options = {}) {
			...
			if (!sourceMimeType || !encodedImage) throw new CoreRuntimeError("[ImageEditor] Unsupported image Data URL.");
			...
			try {
				await this.plugins.runOperationForHost("core:load-image", source, async (loadSource, operationContext) => {
					...
					const processed = dimensions && requiresImagePreprocessing(sourceMimeType, dimensions.width, dimensions.height, preprocessing) ? await withCoreTimeout((signal) => {
						var _a, _b;
					...
					}, this.options.imageLoadTimeoutMs, "Image preprocessing", operationContext.signal) : Object.freeze({
						source: loadSource,
						mimeType: sourceMimeType,
						width: 0,
						height: 0,
		...
		resizeCanvas(width, height) {
			...
			if (!Number.isFinite(width) || width <= 0 || !Number.isFinite(height) || height <= 0) throw new TypeError("[ImageEditor] Canvas dimensions must be positive finite numbers.");
			this.setCanvasSize(width, height);
			(_a = this.canvas) === null || _a === void 0 || _a.renderAll();
		}
		resizeToContainer() {
			var _a, _b, _c, _d, _e, _f;
			...
			if (!image) {
				...
				(_e = this.canvas) === null || _e === void 0 || _e.renderAll();
				return;
			}
			image.setCoords();
			const bounds = image.getBoundingRect();
			...
			} else this.setCanvasSize(Math.max(viewport.width, Math.ceil(bounds.left + bounds.width)), Math.max(viewport.height, Math.ceil(bounds.top + bounds.height)));
			(_f = this.canvas) === null || _f === void 0 || _f.renderAll();
		}
		observeContainer(options = {}) {
			var _a;
			this.assertReady("observe the Canvas container");
			const container = this.containerElement;
			if (!container) throw new CoreRuntimeError("[ImageEditor] Canvas container is unavailable.");
		...
		async relayout(options = {}) {
			...
			const scroll = this.containerElement ? Object.freeze({
				left: this.containerElement.scrollLeft,
				top: this.containerElement.scrollTop
			}) : null;
		...
		emitRuntimeStatus() {
			...
			if (previous && previous.lifecycle === status.lifecycle && previous.initialized === status.initialized && previous.imageLoaded === status.imageLoaded && previous.busy === status.busy && previous.activeToolId === status.activeToolId && previous.layoutMode === status.layoutMode && previous.geometryRevision === status.geometryRevision) return;
...
//#region dist/esm/foundations/overlay/overlay-errors.js
	var OverlayRecoverableObjectError = class extends CoreRuntimeError {
		constructor(message, cause) {
...
//#region dist/esm/foundations/overlay/overlay-foundation-controller.js
	...
	function freezePersistence(definition) {
		...
		if (!isRecord$8(persistence)) return failure("must declare persistence.");
	...
	function isSerializedRecord(value) {
		return isRecord$8(value) && typeof value.kind === "string" && value.kind.trim().length > 0 && typeof value.persistentId === "string" && OVERLAY_ID_PATTERN.test(value.persistentId) && typeof value.hidden === "boolean" && typeof value.locked === "boolean" && isRecord$8(value.codec) && typeof value.codec.type === "string" && isRuntimeIdentifier(value.codec.type) && typeof value.codec.version === "string" && isValidSemVer(value.codec.version) && Object.prototype.hasOwnProperty.call(value, "data");
	}
	function validateStateShape(value) {
		return isRecord$8(value) && value.version === 1 && Array.isArray(value.overlays) && value.overlays.length <= 1e5 && value.overlays.every(isSerializedRecord) && Array.isArray(value.selectionIds) && value.selectionIds.every((persistentId) => typeof persistentId === "string" && OVERLAY_ID_PATTERN.test(persistentId)) && new Set(value.selectionIds).size === value.selectionIds.length;
	}
	...
	var OverlayFoundationController = class {
		...
		registerKind(definition) {
			...
			if (!isRecord$8(definition) || typeof definition.classify !== "function" || typeof definition.getPersistentId !== "function" || definition.setPersistentId !== void 0 && typeof definition.setPersistentId !== "function") throw new PluginManifestError("Overlay Kind registration requires callable classify and persistent identity members.", isRecord$8(definition) && typeof definition.ownerPluginId === "string" ? { pluginId: definition.ownerPluginId } : {});
			...
			if (existing) throw new CoreRuntimeError(`[ImageEditor] Overlay kind "${definition.id}" is already registered by "${existing.definition.ownerPluginId}".`);
			const record = {
				definition: Object.freeze({
					...definition,
					persistence
		...
		registerGeometryPolicy(policy) {
			...
			if (this.policies.has(policy.kind)) throw new CoreRuntimeError(`[ImageEditor] Overlay kind "${policy.kind}" already has a geometry policy.`);
		...
		registerInteractionPolicy(policy) {
			...
			this.requireKindOwner(policy.kind, policy.ownerPluginId);
			if (this.interactionPolicies.has(policy.kind)) throw new CoreRuntimeError(`[ImageEditor] Overlay kind "${policy.kind}" already has an interaction policy.`);
			const frozen = Object.freeze({ ...policy });
		...
		registerExportRenderer(renderer) {
			...
			if (!Number.isFinite(renderer.order)) throw new CoreRuntimeError("[ImageEditor] Overlay export renderer order must be finite.");
			if (this.renderers.has(renderer.kind)) throw new CoreRuntimeError(`[ImageEditor] Overlay kind "${renderer.kind}" already has an export renderer.`);
		...
		async mutate(request) {
			...
			return this.mutations.run({
				...
				validate: async (result, transaction) => {
					...
					if (!currentDescriptor) throw new CoreRuntimeError("[ImageEditor] Overlay mutation synchronization did not produce a descriptor.");
				...
				describeCommit: () => {
					if (!descriptor) throw new CoreRuntimeError("[ImageEditor] Overlay mutation descriptor is unavailable at commit.");
					return descriptor;
				}
			});
		}
		add(objects) {
			...
			return this.mutate({
				...
				affectedObjects: () => {
					...
					if (indexed.length !== uniqueObjects.length || indexed.some((entry) => entry.kind.definition.persistence.mode !== "persistent")) throw new CoreRuntimeError("[ImageEditor] Overlay insertion accepts only registered persistent kinds.");
		...
		addTransient(objects) {
			...
			return this.host.runOperation("overlay:transient", async () => {
				...
				try {
					...
					this.rebuildIndex();
					const indexed = this.resolveOverlayObjects(uniqueObjects);
					if (indexed.length !== uniqueObjects.length || indexed.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new CoreRuntimeError("[ImageEditor] Transient overlay insertion accepts only registered transient kinds.");
					this.host.requestRender();
				} catch (error) {
		...
		replaceTransient(ids, objects) {
			...
			return this.host.runOperation("overlay:transient", async () => {
				...
				if (removed.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new CoreRuntimeError("[ImageEditor] Transient overlay replacement accepts only transient kinds.");
				try {
					for (const entry of removed) canvas.remove(entry.object);
					for (const object of uniqueObjects) canvas.add(object);
					this.rebuildIndex();
					const inserted = this.resolveOverlayObjects(uniqueObjects);
					if (inserted.length !== uniqueObjects.length || inserted.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new CoreRuntimeError("[ImageEditor] Transient overlay replacement produced an invalid kind.");
					this.host.requestRender();
				} catch (error) {
					for (const object of uniqueObjects) canvas.remove(object);
		...
		removeTransient(ids) {
			...
			return this.host.runOperation("overlay:transient", async () => {
				...
				if (entries.some((entry) => entry.kind.definition.persistence.mode !== "transient")) throw new CoreRuntimeError("[ImageEditor] Transient overlay removal accepts only transient kinds.");
		...
		async flatten(query = {}, options = {}) {
			...
			await this.geometry.run({
				...
				mutateBase: async ({ signal, transaction }) => {
					...
					if (!baseImage) throw new CoreRuntimeError("[ImageEditor] Cannot flatten without a base image.");
		...
		assertFlattenBudget(canvas = this.host.requireCanvas("validate flatten dimensions"), baseImage = this.host.getBaseImage()) {
			...
			if (!isRasterAllocationWithinBudget(canvas.getWidth(), canvas.getHeight(), budget) || !isRasterAllocationWithinBudget(region.width, region.height, budget)) throw new CoreRuntimeError("[ImageEditor] Flatten dimensions exceed the configured budget.");
		}
		dispose() {
			if (this.disposed) return;
			if (this.activeGesture) this.failGesture(this.activeGesture, abortError$2("Overlay Foundation was disposed during an active gesture."));
			const canvas = this.host.getCanvas();
			if (canvas) {
			...
			const registrationErrors = disposeInReverseSync(this.registrations, { pluginId: OVERLAY_STATE_ID });
			this.registrations.length = 0;
			...
			this.attached = false;
			this.disposed = true;
			if (registrationErrors.length > 0) throw new CoreRuntimeError(`[ImageEditor] Overlay Foundation disposal had ${registrationErrors.length} registration cleanup error(s).`);
		}
		captureState() {
			var _a;
			this.setPreviewObjectsHidden(false);
			try {
				...
				for (const object of (_a = canvas === null || canvas === void 0 ? void 0 : canvas.getObjects()) !== null && _a !== void 0 ? _a : []) {
					const marked = object;
					if (typeof marked.editorOverlayKind === "string" && !this.byObject.has(object)) throw new CoreRuntimeError(`[ImageEditor] Persistent overlay kind "${marked.editorOverlayKind}" is not registered.`);
				}
				const overlays = [];
				for (const object of this.indexedCanvasObjects()) {
					...
					if (!serializer) throw new CoreRuntimeError(`[ImageEditor] Overlay kind "${indexed.kind.definition.id}" has no serializer.`);
					const classification = this.classificationFor(indexed);
		...
		beginGesture(target, action) {
			...
			if (this.activeGesture) {
				...
				supersessionReason = abortError$2("Overlay gesture was superseded by another target.");
		...
		async rebaseSupersedingGesture(gesture) {
			...
			for (const target of gesture.targets) {
				...
				if (!transform) throw new CoreRuntimeError(`[ImageEditor] Superseding Overlay gesture lost target "${target.persistentId}".`);
		...
		async waitForGestureCompletion(gesture, signal) {
			var _a;
			if (signal.aborted) throw (_a = signal.reason) !== null && _a !== void 0 ? _a : abortError$2("Overlay gesture was aborted.");
			let abort;
			const aborted = new Promise((resolve, reject) => {
				abort = () => {
					var _a;
					return reject((_a = signal.reason) !== null && _a !== void 0 ? _a : abortError$2("Overlay gesture was aborted."));
				};
				signal.addEventListener("abort", abort, { once: true });
			});
			try {
		...
		async validateMutation(targets, descriptor, transaction) {
			...
			if (new Set(descriptor.objectIds).size !== descriptor.objectIds.length) throw new CoreRuntimeError("[ImageEditor] Overlay mutation contains duplicate ids.");
			for (const target of targets) {
				if (target.kind.definition.getPersistentId(target.object) !== target.persistentId) throw new CoreRuntimeError(`[ImageEditor] Overlay "${target.persistentId}" changed persistent identity.`);
				if (descriptor.action !== "delete" && (!liveObjects.has(target.object) || ((_a = this.byId.get(target.persistentId)) === null || _a === void 0 ? void 0 : _a.object) !== target.object)) throw new CoreRuntimeError(`[ImageEditor] Overlay "${target.persistentId}" is missing from the committed index.`);
			}
		...
		requireKindOwner(kindId, ownerPluginId) {
			...
			if (!kind) throw new CoreRuntimeError(`[ImageEditor] Overlay kind "${kindId}" is not registered.`);
			if (kind.definition.ownerPluginId !== ownerPluginId) throw new CoreRuntimeError(`[ImageEditor] Overlay kind "${kindId}" belongs to "${kind.definition.ownerPluginId}", not "${ownerPluginId}".`);
		}
		emitSelection() {
			if (this.disposed) return;
		...
		assertRuntimeIdentifier(value, label) {
			if (!isRuntimeIdentifier(value)) throw new CoreRuntimeError(`[ImageEditor] Invalid ${label} Runtime ID.`);
		}
		assertOpaqueIdentifier(value, label) {
			if (!OVERLAY_ID_PATTERN.test(value)) throw new CoreRuntimeError(`[ImageEditor] ${label} must be a safe identifier no longer than 128 characters.`);
		}
		assertActive(operation) {
			if (this.disposed) throw new CoreRuntimeError(`[ImageEditor] Cannot ${operation} after disposal.`);
		}
...
//#region dist/esm/foundations/annotation/annotation-metadata.js
	...
	function isPlainRecord$8(value) {
	...
	function cloneMetadataValue(value, depth, budget) {
		...
		try {
			...
			if (!isPlainRecord$8(value)) throw new AnnotationValidationError("Annotation metadata objects must be plain.");
	...
	function normalizeAnnotationMetadata(value = {}) {
		if (!isPlainRecord$8(value)) throw new AnnotationValidationError("Annotation metadata must be a plain object.");
...
//#region dist/esm/foundations/annotation/annotation-presentation-manager.js
	...
	function finiteRange$2(value, label, minimum, maximum) {
		if (value === void 0) return void 0;
	...
	function resolveHoverStyle(value) {
		...
		if (!isPlainRecord$7(value)) throw new AnnotationValidationError("Annotation hoverStyle must be an object or false.");
		...
		return Object.freeze({
			...
			...value.opacity !== void 0 ? { opacity: finiteRange$2(value.opacity, "Annotation hover opacity", 0, 1) } : {},
			...value.stroke !== void 0 ? { stroke: nullableColor(value.stroke, "Annotation hover stroke") } : {},
			...value.strokeWidth !== void 0 ? { strokeWidth: finiteRange$2(value.strokeWidth, "Annotation hover strokeWidth", 0, 256) } : {}
		});
	}
	function resolveControlStyle(value) {
		if (value === void 0) return Object.freeze({});
		if (!isPlainRecord$7(value)) throw new AnnotationValidationError("Annotation controlStyle must be an object.");
		const allowed = /* @__PURE__ */ new Set([
			"borderColor",
			"cornerColor",
			"cornerStrokeColor",
		...
		return Object.freeze({
			...
			...value.cornerSize !== void 0 ? { cornerSize: finiteRange$2(value.cornerSize, "Annotation cornerSize", 1, 128) } : {},
			...value.touchCornerSize !== void 0 ? { touchCornerSize: finiteRange$2(value.touchCornerSize, "Annotation touchCornerSize", 1, 256) } : {},
			...value.transparentCorners === void 0 ? {} : { transparentCorners: value.transparentCorners },
			...value.padding !== void 0 ? { padding: finiteRange$2(value.padding, "Annotation control padding", 0, 128) } : {}
		});
	}
	function resolveLabel(value) {
		var _a, _b;
		if (value === void 0 || value === false) return false;
		if (!isPlainRecord$7(value)) throw new AnnotationValidationError("Annotation label must be an object or false.");
		const allowed = /* @__PURE__ */ new Set([
			"showOn",
			"offset",
			"getText",
		...
		if (value.textOptions !== void 0 && !isPlainRecord$7(value.textOptions)) throw new AnnotationValidationError("Annotation label.textOptions must be an object.");
		return Object.freeze({
			showOn: (_a = value.showOn) !== null && _a !== void 0 ? _a : "selected",
			offset: (_b = finiteRange$2(value.offset, "Annotation label offset", 0, 256)) !== null && _b !== void 0 ? _b : 3,
	...
	function resolveLockIndicator(value) {
		...
		if (value !== void 0 && !isPlainRecord$7(value)) throw new AnnotationValidationError("Annotation lockIndicator must be an object or false.");
		...
		return Object.freeze({
			size: (_a = finiteRange$2(config.size, "Annotation lock indicator size", 8, 64)) !== null && _a !== void 0 ? _a : DEFAULT_LOCK_INDICATOR.size,
			offset: (_b = finiteRange$2(config.offset, "Annotation lock indicator offset", 0, 64)) !== null && _b !== void 0 ? _b : DEFAULT_LOCK_INDICATOR.offset,
	...
	var AnnotationPresentationManager = class {
		...
		createLockIndicator(config, ownerId) {
			...
			const group = markSessionObject(new this.host.fabric.Group([
				shackle,
				body,
				keyhole
			], {
...
//#region dist/esm/foundations/annotation/annotation-controller.js
	...
	function isInteractionState(value) {
		if (!isPlainRecord$6(value)) return false;
	...
	function equalMetadata(left, right) {
		...
		if (isPlainRecord$6(left) && isPlainRecord$6(right)) {
	...
	function isStateData(value) {
		return isPlainRecord$6(value) && Object.keys(value).every((key) => [
			"version",
			"name",
			"interaction",
			"feature"
	...
	function normalizeSharedUpdate(value) {
		if (!isPlainRecord$6(value)) throw new AnnotationValidationError("Annotation update must be a plain object.");
		const allowed = /* @__PURE__ */ new Set([
			"name",
			"metadata",
			"hidden",
		...
		return Object.freeze({
			...
			...value.hidden !== void 0 ? { hidden: validateBoolean$1(value.hidden, "Annotation hidden state") } : {},
			...value.locked !== void 0 ? { locked: validateBoolean$1(value.locked, "Annotation locked state") } : {}
		});
	}
	...
	var AnnotationController = class {
		...
		normalizeQuery(query) {
			if (!isPlainRecord$6(query)) throw new AnnotationValidationError("Annotation query must be a plain object.");
			const allowed = /* @__PURE__ */ new Set([
				"kinds",
				"ids",
				"includeHidden",
			...
			return Object.freeze({
				...
				...query.includeHidden === void 0 ? {} : { includeHidden: validateBoolean$1(query.includeHidden, "Query includeHidden") },
				...query.includeLocked === void 0 ? {} : { includeLocked: validateBoolean$1(query.includeLocked, "Query includeLocked") }
			});
		}
		normalizeRemoveAllOptions(options) {
			var _a;
			if (!isPlainRecord$6(options)) throw new AnnotationValidationError("Annotation removeAll options must be a plain object.");
			const allowed = /* @__PURE__ */ new Set([
				"kinds",
				"ids",
				"includeHidden",
			...
			const force = (_a = validateBoolean$1(typedOptions.force, "Annotation removeAll force")) !== null && _a !== void 0 ? _a : false;
...
//#region dist/esm/fabric/fabric-animation.js
	...
	function animateProps(object, props, options, control) {
		return new Promise((resolve, reject) => {
			...
			const cleanup = () => {
				...
				unregisterAborter === null || unregisterAborter === void 0 || unregisterAborter();
	...
	function collectAnimationAborters(animationResult) {
		return (Array.isArray(animationResult) ? animationResult : animationResult && typeof animationResult === "object" ? Object.values(animationResult) : [animationResult]).flatMap((handle) => {
...
//#endregion
//#region dist/esm/plugins/transform/transform-controller.js
	...
	var MutationAnimationScope = class {
		...
		cancelAnimations() {
			if (this.cancelled) return this.waitForIdle();
			this.cancelled = true;
			for (const abort of [...this.aborters]) try {
				abort();
			} catch {
				this.aborters.delete(abort);
			}
		...
		waitForIdle() {
			if (this.aborters.size === 0) return Promise.resolve();
			return new Promise((resolve) => this.idleWaiters.add(resolve));
		}
		resolveIdleWaiters() {
	...
	var PluginAnimationControl = class {
		constructor() {
			Object.defineProperty(this, "disposed", {
				enumerable: true,
				configurable: true,
	...
	var TransformPluginController = class {
		...
		enqueue(operationId, mutate, options) {
			...
			return operation.then(() => void 0).finally(() => animations.dispose());
		}
		async applyScale(image, factor, signal, animations) {
			...
			await this.runAnimation(signal, () => animateProps(image, {
				scaleX: target,
				scaleY: target
			}, {
		...
		computeTopLeftPoint(image) {
			...
			if (typeof PointConstructor === "function") return new PointConstructor(bounds.left, bounds.top);
			return {
				x: bounds.left,
				y: bounds.top
			};
		...
		throwIfAborted(signal) {
			var _a;
			if (signal.aborted) throw (_a = signal.reason) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Error("Transform operation aborted.");
			if (this.animations.isDisposed()) throw new Error("Transform plugin is disposed.");
		}
		async runAnimation(signal, animation, animations) {
			const cancel = () => {
				animations.cancelAnimations().catch(() => void 0);
			};
...
//#endregion
//#region dist/esm/plugins/transform/index.js
	const transformPluginRef = definePluginRef("plugin:transform", "1.0.0");
	function isTransformState(value) {
		if (typeof value !== "object" || value === null) return false;
		const candidate = value;
		return typeof candidate.scale === "number" && Number.isFinite(candidate.scale) && candidate.scale > 0 && typeof candidate.rotationDegrees === "number" && Number.isFinite(candidate.rotationDegrees) && typeof candidate.flipX === "boolean" && typeof candidate.flipY === "boolean";
	}
	function transformPlugin(options = {}) {
		...
		return definePlugin({
			...
			setup(context) {
				...
				const state = context.capabilities.require(SNAPSHOT_REGISTRATION_CAPABILITY);
				const geometry = context.capabilities.require(GEOMETRY_MUTATION_CAPABILITY);
				controller = new TransformPluginController(Object.freeze({
					...status,
					...fabricRuntime
				}), baseImage, render, geometry, resolved);
				for (const id of [
					"transform:scale",
					"transform:zoom-in",
					"transform:zoom-out",
				...
				context.disposables.add(state.registerSlice({
					...
					validate: (value) => isTransformState(value) ? {
						...
						valid: false,
						message: "Transform state is malformed."
					},
			...
			onImageLoaded() {
				controller === null || controller === void 0 || controller.resetStateFromImage();
			},
			onImageCleared() {
				controller === null || controller === void 0 || controller.resetStateFromImage();
			},
...
//#region dist/esm/plugins/history/history-controller.js
	...
	function resolveMaxBytes(value) {
		...
		if (!Number.isSafeInteger(value) || value <= 0) throw new CoreRuntimeError("[ImageEditor] History maxBytes must be a positive safe integer.", { code: "HISTORY_MAX_BYTES_INVALID" });
		return value;
	}
	var HistoryPluginController = class {
		...
		push(record) {
			...
			if (!record || typeof record.operationId !== "string" || record.operationId.length === 0) throw new CoreRuntimeError("[ImageEditor] History record operationId is invalid.");
			...
			if (bytes > this.maxBytes) {
				const changed = this.resetTimeline();
				this.baseline = record.after;
				const warning = new CoreRuntimeError(`[ImageEditor] History record "${record.operationId}" exceeds maxBytes and was not retained.`, { code: "HISTORY_RECORD_BYTE_LIMIT_EXCEEDED" });
		...
		enable(options) {
			...
			if ((options === null || options === void 0 ? void 0 : options.baseline) !== "current") throw new CoreRuntimeError("[ImageEditor] History can enable only from the current baseline.", { code: "HISTORY_BASELINE_UNSUPPORTED" });
		...
		disable(options = {}) {
			...
			if (options.clear !== void 0 && typeof options.clear !== "boolean") throw new CoreRuntimeError("[ImageEditor] History disable clear must be a boolean.", { code: "HISTORY_DISABLE_OPTION_INVALID" });
		...
		assertActive(operation) {
			if (this.disposed) throw new CoreRuntimeError(`[ImageEditor] Cannot ${operation} after History disposal.`);
		}
	};
...
//#region dist/esm/fabric/safe-fabric-serialization.js
	...
	function isSafeSerializedFabricObject(value, options) {
		var _a, _b, _c;
		if (!isPlainRecord$5(value)) return false;
		...
		const inspect = (entry, depth, root, propertyName) => {
			...
			if (!isPlainRecord$5(entry)) return false;
...
//#region dist/esm/plugins/mask/mask-controller.js
	...
	function isSerializedMaskData(value) {
		...
		try {
			...
			return isSafeSerializedFabricObject(serializedObject, { rootTypes: [
				"rect",
				"circle",
				"ellipse",
				"polygon"
	...
	function isMaskStateData(value) {
		if (!isPlainRecord$4(value) || value.version !== 1) return false;
		const validKind = value.kind === "rect" || value.kind === "circle" || value.kind === "ellipse" || value.kind === "polygon";
		const validPoints = value.points === null || Array.isArray(value.points) && value.points.length >= 3 && value.points.length <= 4096 && value.points.every((point) => isPlainRecord$4(point) && typeof point.x === "number" && Number.isFinite(point.x) && typeof point.y === "number" && Number.isFinite(point.y));
	...
	var MaskPluginController = class {
		constructor(host, state, overlay, disposables, options) {
			...
			this.disposables.add(overlay.registerKind({
				...
				stateCodec: {
					...
					version: "1.0.0",
					serialize: (object, context) => {
						if (!isMaskObject(object)) throw new CoreRuntimeError("[ImageEditor] Mask State Codec received a non-mask.");
						const kind = maskStateKind(object);
						const metadata = object.overlayMetadata;
						return Object.freeze({
							geometry: captureOverlayStateBounds(object, context),
							metadata: isPlainRecord$4(metadata) ? Object.freeze({ ...metadata }) : Object.freeze({}),
							data: Object.freeze({
								version: 1,
								kind,
					...
					validate: (value) => isOverlayStateBoundsGeometry(value.geometry) && isMaskStateData(value.data) && isPlainRecord$4(value.metadata),
					deserialize: (value, context) => {
						if (!isOverlayStateBoundsGeometry(value.geometry) || !isMaskStateData(value.data) || !isPlainRecord$4(value.metadata)) throw new CoreRuntimeError("[ImageEditor] Serialized Mask State data is malformed.");
		...
		remove(id) {
			const object = this.overlay.getByPersistentId(id);
			if (!object || !isMaskObject(object)) return Promise.reject(new CoreRuntimeError(`[ImageEditor] Mask "${id}" was not found.`));
			return this.overlay.mutate({
		...
		serializeMask(object) {
			if (!isMaskObject(object)) throw new CoreRuntimeError("[ImageEditor] Mask serializer received a non-mask.");
		...
		async deserializeMask(data, fabricModule) {
			if (!isSerializedMaskData(data)) throw new CoreRuntimeError("[ImageEditor] Serialized Mask data is malformed.");
			const object = (await fabricModule.util.enlivenObjects([data.object]))[0];
			if (!object) throw new CoreRuntimeError("[ImageEditor] Fabric did not restore a Mask object.");
			const mask = object;
			mask.editorObjectKind = "mask";
			mask.maskId = data.maskId;
...
//#region dist/esm/plugins/filters/filters-errors.js
	...
	var FiltersPreviewMissingError = class extends Error {
		constructor() {
	...
	var FiltersPluginDisposedError = class extends Error {
		constructor(operation) {
	...
	var FilterImplementationError = class extends Error {
	...
	var FilterBakeValidationError = class extends Error {
		constructor(message, cause) {
			super(`[ImageEditor] ${message}`);
			Object.defineProperty(this, "code", {
				enumerable: true,
...
//#endregion
//#region dist/esm/plugins/filters/filter-definitions.js
	...
	function normalizeMaxFilterCount(value) {
		const maxFilterCount = value !== null && value !== void 0 ? value : 8;
		if (!Number.isSafeInteger(maxFilterCount) || maxFilterCount < 1 || maxFilterCount > 8) throw new FilterDefinitionError(`maxFilterCount must be an integer from 1 to ${8}.`, "$.maxFilterCount");
		return maxFilterCount;
	}
	function normalizeDefinition(value, index) {
		const path = `$[${index}]`;
		if (!isRecord$7(value)) throw new FilterDefinitionError("Each Filter definition must be a plain object.", path);
		validateKeys(value, ["type", "value"], path);
		const type = value.type;
		if (typeof type !== "string" || !SUPPORTED_FILTER_TYPES.includes(type)) throw new FilterDefinitionError(`Unknown Filter type "${String(type)}".`, `${path}.type`);
		if (type === "grayscale" || type === "sepia" || type === "vintage") {
			validateKeys(value, ["type"], path);
			return Object.freeze({ type });
		}
		if (typeof value.value !== "number" || !Number.isFinite(value.value)) throw new FilterDefinitionError("Filter value must be finite.", `${path}.value`);
		const numericType = type;
		const [minimum, maximum] = numericRanges[numericType];
		if (value.value < minimum || value.value > maximum) throw new FilterDefinitionError(`${type} value must be within [${minimum}, ${maximum}].`, `${path}.value`);
	...
	function normalizeFilterDefinitions(value, limits = {}) {
		if (!Array.isArray(value)) throw new FilterDefinitionError("Filter definitions must be an array.");
		const maxFilterCount = normalizeMaxFilterCount(limits.maxFilterCount);
		if (value.length > maxFilterCount) throw new FilterDefinitionError(`Filter count exceeds ${maxFilterCount}.`);
		const definitionByType = /* @__PURE__ */ new Map();
		const seenTypes = /* @__PURE__ */ new Set();
		for (let index = 0; index < value.length; index += 1) {
			const definition = normalizeDefinition(value[index], index);
			const type = value[index].type;
			if (seenTypes.has(type)) throw new FilterDefinitionError(`Duplicate Filter type "${type}" is not supported.`, `$[${index}].type`);
			seenTypes.add(type);
	...
	function areFilterDefinitionsEqual(left, right) {
		...
		return left.every((definition, index) => {
			...
			return !("value" in definition) || "value" in candidate && definition.value === candidate.value;
		});
	}

//#endregion
//#region dist/esm/plugins/filters/fabric-filter-factory.js
...
//#endregion
//#region dist/esm/plugins/filters/filtered-image-renderer.js
	function abortError$1(message) {
		return new DOMException(message, "AbortError");
	}
	function throwIfAborted(signal) {
		var _a;
		if (signal.aborted) throw (_a = signal.reason) !== null && _a !== void 0 ? _a : abortError$1("Filter rendering was aborted.");
	}
	...
	function normalizeFilterBakeOptions(options, sourceMimeType) {
		...
		if (format !== "png" && format !== "jpeg" && format !== "webp") throw new FilterBakeValidationError("Filter bake format must be png, jpeg, or webp.");
		const quality = record.quality;
		if (quality !== void 0 && (typeof quality !== "number" || !Number.isFinite(quality))) throw new FilterBakeValidationError("Filter bake quality must be finite.");
		if (typeof quality === "number" && (quality < 0 || quality > 1)) throw new FilterBakeValidationError("Filter bake quality must be within [0, 1].");
		return Object.freeze({
			format,
	...
	async function renderBakedImage(fabric, baseImage, definitions, options, imageInfo, policy, signal) {
		...
		if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width <= 0 || height <= 0) throw new FilterBakeValidationError("Base Image dimensions are invalid.");
		const pixelBudget = Math.min(policy.maxInputPixels, policy.maxExportPixels);
		if (width > policy.maxExportDimension || height > policy.maxExportDimension || !isPixelAreaWithinBudget(width, height, pixelBudget)) throw new FilterBakeValidationError("Filtered Raster dimensions exceed the Core policy.");
		...
		try {
			...
			try {
				dataUrl = clone.toDataURL({
					...
					...normalizedOptions.quality === void 0 ? {} : { quality: normalizedOptions.quality },
					multiplier: 1,
				...
				if (hasErrorName(error, "SecurityError")) throw new FilterBakeValidationError("Filtered Raster pixels cannot be exported because canvas access is blocked.", error);
				throw error;
			}
			if (encodedBytes$2(dataUrl) > policy.maxInputBytes) throw new FilterBakeValidationError("Filtered Raster exceeds the Core input budget.");
			replacement = await decodeBakedImage(fabric, dataUrl, policy.imageLoadTimeoutMs, signal);
			throwIfAborted(signal);
			if (replacement.width !== width || replacement.height !== height) throw new FilterBakeValidationError("Filtered Raster dimensions changed during decode.");
			copyBaseImagePresentation(baseImage, replacement);
			return Object.freeze({
				image: replacement,
				mimeType: normalizedOptions.mimeType
			});
		} catch (error) {
...
//#region dist/esm/plugins/filters/filters-controller.js
	...
	function normalizeConfiguration$1(options) {
		var _a, _b;
		normalizeFilterDefinitions([], { maxFilterCount: (_a = options.maxFilterCount) !== null && _a !== void 0 ? _a : 8 });
		return Object.freeze({ maxFilterCount: (_b = options.maxFilterCount) !== null && _b !== void 0 ? _b : 8 });
	}
	...
	var FiltersController = class {
		...
		subscribe(listener) {
			...
			if (typeof listener !== "function") throw new TypeError("[ImageEditor] Filters status listener must be a function.");
			this.listeners.add(listener);
			return createDisposable(() => {
				this.listeners.delete(listener);
			});
		...
		async preview(filters) {
			...
			await this.operations.run("filters:preview", async (operationSignal) => {
				...
				const baseImage = this.host.getBaseImage();
				if (!baseImage) throw new Error("[ImageEditor] Cannot preview Filters without a Base Image.");
				const candidate = definitions.length === 0 ? null : await createFilteredImageClone(this.host.fabric, baseImage, definitions, operationSignal, this.host.backgroundColor);
		...
		cancelPreview() {
			...
			return this.operations.run("filters:cancel-preview", async (signal) => {
				if (signal.aborted) throw operationAbortReason(signal, "Filter preview cancellation aborted.");
				this.cancelPreviewSession(true);
			});
		}
		async commit(filters) {
			this.assertActive("commit Filters");
			const usesPreview = filters === void 0;
			if (usesPreview && this.previewDefinitions === null) throw new FiltersPreviewMissingError();
			const definitions = usesPreview ? this.previewDefinitions : this.normalizeDefinitions(filters);
			...
			await this.mutations.run({
				...
				synchronize: async (_result, context) => {
					if (usesPreview && this.previewDefinitions !== null && areFilterDefinitionsEqual(this.previewDefinitions, definitions)) {
		...
		async bakeIntoBase(parent, options) {
			...
			try {
				await this.mutations.run({
					...
					synchronize: () => this.replaceCommittedVisual(emptyDefinitions),
					validate: (image) => {
						if (!parent && this.host.getBaseImage() !== image) throw new Error("Raster Commit did not retain the baked Base Image.");
						if (this.committedState.filters.length !== 0) throw new Error("Raster Commit did not clear the baked Filter state.");
				...
				if (!committed && replacement && this.host.getBaseImage() !== replacement) disposeFabricImage(replacement);
			}
		}
		...
		validateState(value) {
			if (!isRecord$6(value)) return {
				valid: false,
				message: "Filters state must be an object."
			};
			...
			if (value.schema !== FILTERS_STATE_SCHEMA) return {
				valid: false,
				message: `Filters state schema must be "${FILTERS_STATE_SCHEMA}".`
			};
			if (value.version !== FILTERS_STATE_VERSION) return {
				valid: false,
		...
		renderExport(canvas, options) {
			...
			const baseImage = canvas.getObjects().find((object) => object instanceof this.host.fabric.FabricImage);
		...
		async synchronizeAfterCommittedMutation() {
			...
			const committedVisualIsCurrent = this.committedState.filters.length === 0 ? this.committedVisual === null : this.committedVisual !== null && this.committedSource === baseImage;
			const previewVisualIsCurrent = this.previewDefinitions === null || this.previewDefinitions.length === 0 ? this.previewVisual === null : this.previewVisual !== null && this.previewSource === baseImage;
			if (committedVisualIsCurrent && previewVisualIsCurrent) {
				if (this.committedVisual) copyBaseImagePresentation(baseImage, this.committedVisual, {
					backgroundColor: this.host.backgroundColor,
					transient: true
				});
				if (this.previewVisual) copyBaseImagePresentation(baseImage, this.previewVisual, {
					backgroundColor: this.host.backgroundColor,
					transient: true
				});
				this.host.requestRender();
		...
		normalizeDefinitions(value) {
			return normalizeFilterDefinitions(value, { maxFilterCount: this.configuration.maxFilterCount });
		}
		normalizeConfigurationPatch(patch) {
			...
			if (maxFilterCount < requiredCount) throw new TypeError(`[ImageEditor] maxFilterCount cannot be lower than the active Filter count ${requiredCount}.`);
		...
		async replaceCommittedVisual(definitions, signal = new AbortController().signal) {
			const baseImage = this.host.getBaseImage();
			const candidate = definitions.length === 0 || !baseImage ? null : await createFilteredImageClone(this.host.fabric, baseImage, definitions, signal, this.host.backgroundColor);
			if (signal.aborted) {
				disposeFabricImage(candidate);
		...
		detachVisual(image) {
			const canvas = this.host.getCanvas();
			if (image && (canvas === null || canvas === void 0 ? void 0 : canvas.getObjects().includes(image))) canvas.remove(image);
		}
		...
		validateBaseImageInvariant(transactionId) {
			...
			if (!baseImage || baseImages.length !== 1 || baseImages[0] !== baseImage) throw new Error(`Filters transaction "${transactionId}" violated the Base Image invariant.`);
		}
		status() {
			var _a, _b;
			return Object.freeze({
		...
		assertImageLoaded(operation) {
			if (!this.host.isImageLoaded()) throw new Error(`[ImageEditor] Cannot ${operation} without a loaded image.`);
		}
		assertActive(operation) {
			if (this.disposed || this.host.isDisposed()) throw new FiltersPluginDisposedError(operation);
		}
	};

//#endregion
//#region dist/esm/plugins/filters/index.js
		...
		return definePlugin({
			...
			setup(context) {
				...
				controller = new FiltersController(Object.freeze({
					...
					...resourcePolicy,
					...render
				}), Object.freeze({ run: (operationId, task) => context.operations.run(operationId, void 0, (args, operationContext) => {
				...
				context.capabilities.provide(VISIBLE_RASTER_BAKE_CAPABILITY, visibleRasterBake, { version: VISIBLE_RASTER_BAKE_CAPABILITY.version });
				context.disposables.add(snapshots.registerTransientObject(filtersPluginRef.id, (object) => {
					var _a;
					return (_a = controller === null || controller === void 0 ? void 0 : controller.ownsTransient(object)) !== null && _a !== void 0 ? _a : false;
				}));
			...
			onImageCleared() {
				controller === null || controller === void 0 || controller.clearForImage();
			},
			onDispose() {
				controller === null || controller === void 0 || controller.dispose();
				controller = null;
...
//#region dist/esm/plugins/crop/crop-geometry.js
	...
	function normalizeCropRect(value, limits) {
		...
		if (!isRecord$5(value)) throw new TypeError("[ImageEditor] Crop rect must be an object.");
...
//#region dist/esm/plugins/crop/crop-renderer.js
	...
	function normalizeCropApplyOptions(value, sourceMimeType) {
		var _a;
		if (value !== void 0 && !isRecord$3(value)) throw new CropValidationError("Crop apply options must be an object.");
	...
	async function renderCropImage(host, source, rect, rotationDegrees, options, signal) {
		...
		const pixelBudget = Math.min(policy.maxInputPixels, policy.maxExportPixels);
		if (rect.widthPx > policy.maxExportDimension || rect.heightPx > policy.maxExportDimension || !isPixelAreaWithinBudget(rect.widthPx, rect.heightPx, pixelBudget)) throw new CropValidationError("Crop dimensions exceed the Core resource policy.");
		...
		try {
			...
			if (hasErrorName(error, "SecurityError")) throw new CropValidationError("Crop pixels cannot be exported because canvas access is blocked.");
			throw error;
		}
		if (encodedBytes$1(dataUrl, options.mimeType) > policy.maxInputBytes) throw new CropValidationError("Crop output exceeds the Core input budget.");
...
//#region dist/esm/plugins/crop/crop-controller.js
	...
	var CropController = class {
		...
		enter(options = {}) {
			...
			if (!isRecord$2(options)) throw new CropValidationError("Crop enter options must be an object.");
			const allowedKeys = /* @__PURE__ */ new Set([
				"rect",
				"aspectRatio",
				"rotationDegrees",
		...
		return definePlugin({
			...
			manifest: {
				...
				optional: [{
					...
					token: VISIBLE_RASTER_BAKE_CAPABILITY,
					range: "^1.0.0"
				}],
				permissions: [
					"fabric:objects",
...
//#region dist/esm/plugins/mosaic/index.js
	const MOSAIC_TOOL_ID$1 = "plugin:mosaic";
	...
	function mosaicPlugin(options = {}) {
		...
		return definePlugin({
			...
			setup(context) {
				...
				return Object.freeze({
					...
					enter: (enterOptions) => runPreviewOperation("mosaic:enter", enterOptions !== null && enterOptions !== void 0 ? enterOptions : {}, async (mosaic, value) => {
						...
						await context.tools.enter(MOSAIC_TOOL_ID$1);
						try {
							mosaic.enter(value);
						} catch (error) {
							await context.tools.exit("operation");
			...
			onImageCleared(context) {
				if (context.tools.getActiveToolId() === MOSAIC_TOOL_ID$1) return context.tools.exit("operation");
				controller === null || controller === void 0 || controller.closeForImage();
			},
			onDispose() {
				controller === null || controller === void 0 || controller.dispose();
...
//#region dist/esm/plugins/annotation-text/text-controller.js
	...
	function validateText(value, label = "Text") {
		if (typeof value !== "string" || value.length > MAX_TEXT_LENGTH) throw new AnnotationValidationError(`${label} must be a string of at most ${MAX_TEXT_LENGTH} characters.`);
		return value;
	}
	function validateFontField(value, label) {
		if (typeof value !== "string" || value.length === 0 || value.trim() !== value || value.length > MAX_FONT_FIELD_LENGTH || [...value].some((character) => character.charCodeAt(0) < 32)) throw new AnnotationValidationError(`${label} is invalid.`);
		return value;
	}
	...
	function validateFiniteRange(value, label, minimum, maximum) {
		if (typeof value !== "number" || !Number.isFinite(value) || value < minimum || value > maximum) throw new AnnotationValidationError(`${label} must be from ${minimum} to ${maximum}.`);
		return value;
	}
	...
	function validateFallbacks(value) {
		if (!Array.isArray(value) || value.length > 8) throw new AnnotationValidationError("Text font fallbacks are invalid.");
	...
	function resolveTextConfiguration(value = {}, base = defaultConfiguration$1) {
		if (!isPlainRecord$3(value)) throw new AnnotationValidationError("Text configuration must be a plain object.");
		const allowed = new Set(Object.keys(defaultConfiguration$1));
		if (Object.keys(value).some((key) => !allowed.has(key))) throw new AnnotationValidationError("Text configuration contains unknown keys.");
		const merged = {
			...base,
			...value
		};
		if (merged.reflectionBehavior !== "preserve-readable" && merged.reflectionBehavior !== "mirror") throw new AnnotationValidationError("Text reflection behavior is invalid.");
		return Object.freeze({
	...
	function normalizeFeatureUpdate$1(value) {
		if (!isPlainRecord$3(value)) throw new AnnotationValidationError("Text update must be a plain object.");
		const allowed = /* @__PURE__ */ new Set([
			"text",
			"fontSize",
			"fontFamily",
		...
		if (Object.keys(value).some((key) => !allowed.has(key))) throw new AnnotationValidationError("Text update contains unknown keys.");
		return Object.freeze({
	...
	function isSerializedText(value) {
		if (!isPlainRecord$3(value)) return false;
		try {
	...
	function isTextStateData(value) {
		if (!isPlainRecord$3(value) || value.version !== 1) return false;
		try {
	...
	var TextAnnotationController = class {
			...
			return Object.freeze({
				...
				codec: {
					...
					deserialize: async (value, context) => {
						...
						if (!(object instanceof context.fabric.Textbox)) throw new AnnotationValidationError("Serialized Text Annotation did not restore a Textbox.");
						return object;
					}
				},
				stateCodec: {
					...
					deserialize: (value, context) => {
						if (!isOverlayStateBoundsGeometry(value.geometry) || !isTextStateData(value.data)) throw new AnnotationValidationError("Serialized Text Annotation State data is malformed.");
						const data = value.data;
		...
		async create(options = {}) {
			...
			if (!isPlainRecord$3(options)) throw new AnnotationValidationError("Text creation options must be a plain object.");
			const creation = options;
			const text = validateText((_a = creation.text) !== null && _a !== void 0 ? _a : this.configuration.defaultText);
			const left = validateFiniteRange((_b = creation.left) !== null && _b !== void 0 ? _b : 10, "Text left", -1e7, MAX_COORDINATE$1);
			const top = validateFiniteRange((_c = creation.top) !== null && _c !== void 0 ? _c : 10, "Text top", -1e7, MAX_COORDINATE$1);
		...
		async beginEditing(id) {
			...
			this.assertImageLoaded();
			if (this.session) throw new AnnotationValidationError("A Text editing session is already active.");
			const descriptor = this.annotations.get(id);
			const source = this.authoring.getObject(id, TEXT_ANNOTATION_KIND);
			if (!descriptor || !source) throw new AnnotationValidationError(`Text Annotation "${id}" was not found.`);
			if (descriptor.locked) throw new AnnotationValidationError("Locked Text cannot enter editing.");
		...
		subscribe(listener) {
			...
			if (typeof listener !== "function") throw new AnnotationValidationError("Text status listener must be a function.");
			this.listeners.add(listener);
			return createDisposable(() => {
				this.listeners.delete(listener);
			});
		...
		assertImageLoaded() {
			if (!this.host.isImageLoaded()) throw new AnnotationValidationError("Text Annotation requires a loaded image.");
		}
		assertActive(operation) {
			if (this.disposed) throw new AnnotationValidationError(`Cannot ${operation} after disposal.`);
		}
	};

//#endregion
//#region dist/esm/plugins/annotation-text/index.js
	const TEXT_TOOL_ID$1 = "annotation:text";
	...
	function textAnnotationPlugin(options = {}) {
		...
		return definePlugin({
			...
			manifest: {
				...
				requires: [
					{
						token: ANNOTATION_CAPABILITY,
						range: "^1.0.0"
					},
					{
						token: ANNOTATION_AUTHORING_CAPABILITY,
						range: "^1.0.0"
					},
					{
						token: CORE_DIAGNOSTICS_CAPABILITY,
			...
			setup(context) {
				...
				context.disposables.add(annotations.subscribe((status) => {
					...
					if (context.tools.getActiveToolId() === TEXT_TOOL_ID$1) observePromise(context.tools.exit("operation"), (error) => {
				...
				return Object.freeze({
					enter: () => context.operations.run("annotation-text:enter", void 0, async () => {
						...
						await context.tools.enter(TEXT_TOOL_ID$1);
					}),
					exit: () => {
						if (context.tools.getActiveToolId() !== TEXT_TOOL_ID$1) return Promise.resolve();
					...
					commitEditing: async () => {
						try {
							...
							if (context.tools.getActiveToolId() === TEXT_TOOL_ID$1) await context.tools.exit("operation");
						}
					},
					cancelEditing: () => context.operations.run("annotation-text:cancel-edit", void 0, async () => {
						requireController().cancelEditing();
						if (context.tools.getActiveToolId() === TEXT_TOOL_ID$1) await context.tools.exit("requested");
					}),
			...
			onImageCleared(context) {
				if (context.tools.getActiveToolId() === TEXT_TOOL_ID$1) return context.tools.exit("operation");
				controller === null || controller === void 0 || controller.closeForImage();
			},
			onDispose() {
				controller === null || controller === void 0 || controller.dispose();
...
//#region dist/esm/plugins/annotation-shape/shape-controller.js
	...
	function styleString$1(value, label, allowEmpty = false) {
		if (typeof value !== "string" || !allowEmpty && value.length === 0 || value.length > 128 || [...value].some((character) => character.charCodeAt(0) < 32)) throw new AnnotationValidationError(`${label} is invalid.`);
		return value;
	}
	...
	function point(value, label) {
		if (!isPlainRecord$2(value)) throw new AnnotationValidationError(`${label} is invalid.`);
		return Object.freeze({
			x: finiteRange$1(value.x, `${label} x`, -1e7, MAX_COORDINATE),
			y: finiteRange$1(value.y, `${label} y`, -1e7, MAX_COORDINATE)
		});
	}
	function normalizeShapeGeometry(value) {
		if (!isPlainRecord$2(value)) throw new AnnotationValidationError("Shape geometry must be a plain object.");
		const kind = shapeKind(value.kind);
		if (kind === "rect") return Object.freeze({
			kind,
			left: finiteRange$1(value.left, "Shape left", -1e7, MAX_COORDINATE),
			top: finiteRange$1(value.top, "Shape top", -1e7, MAX_COORDINATE),
		...
		if (Math.hypot(end.x - start.x, end.y - start.y) < MIN_GEOMETRY_SIZE) throw new AnnotationValidationError("Shape line and arrow endpoints must be distinct.");
		return Object.freeze({
			kind,
			start,
			end
	...
	function resolveShapeConfiguration(value = {}, base = defaultConfiguration) {
		if (!isPlainRecord$2(value)) throw new AnnotationValidationError("Shape configuration must be a plain object.");
		const allowed = new Set(Object.keys(defaultConfiguration));
		if (Object.keys(value).some((key) => !allowed.has(key))) throw new AnnotationValidationError("Shape configuration contains unknown keys.");
		const merged = {
			...base,
			...value
		};
		return Object.freeze({
			...
			strokeWidth: finiteRange$1(merged.strokeWidth, "Shape stroke width", .1, 1e3),
			...
			arrowHeadLength: finiteRange$1(merged.arrowHeadLength, "Arrow head length", 1, 1e3),
	...
	function normalizeFeatureUpdate(value) {
		if (!isPlainRecord$2(value)) throw new AnnotationValidationError("Shape update must be a plain object.");
		const allowed = /* @__PURE__ */ new Set([
			"stroke",
			"strokeWidth",
			"fill",
		...
		if (Object.keys(value).some((key) => !allowed.has(key))) throw new AnnotationValidationError("Shape update contains unknown keys.");
		return Object.freeze({
			...value.stroke !== void 0 ? { stroke: styleString$1(value.stroke, "Shape stroke") } : {},
			...value.strokeWidth !== void 0 ? { strokeWidth: finiteRange$1(value.strokeWidth, "Shape stroke width", .1, 1e3) } : {},
			...value.fill !== void 0 ? { fill: styleString$1(value.fill, "Shape fill", true) } : {},
			...value.opacity !== void 0 ? { opacity: finiteRange$1(value.opacity, "Shape opacity", 0, 1) } : {},
	...
	function isStatePoint$1(value) {
		return isPlainRecord$2(value) && typeof value.x === "number" && Number.isFinite(value.x) && typeof value.y === "number" && Number.isFinite(value.y);
	}
	function isShapeStateGeometry(value) {
		...
		return (value.kind === "line" || value.kind === "arrow") && isStatePoint$1(value.start) && isStatePoint$1(value.end);
	}
	function isShapeStateData(value) {
		if (!isPlainRecord$2(value) || value.version !== 1) return false;
		try {
	...
	var ShapeAnnotationController = class {
		...
		featureDefinition() {
			return Object.freeze({
				...
				stateCodec: {
					...
					deserialize: (value, context) => {
						if (!isShapeStateGeometry(value.geometry) || !isShapeStateData(value.data)) throw new AnnotationValidationError("Serialized Shape Annotation State data is malformed.");
						const data = value.data;
						const common = {
							stroke: data.stroke,
							strokeWidth: context.toCanvasScalar(data.strokeWidth),
		...
		enter(options) {
			...
			if (!isPlainRecord$2(options)) throw new AnnotationValidationError("Shape session options must be a plain object.");
		...
		async commit() {
			...
			if (!session.geometry) throw new AnnotationValidationError("Shape commit requires preview geometry.");
			const definition = {
				...session.options,
		...
		createDefinition(definition, operationId) {
			...
			this.assertImageLoaded();
			if (!isPlainRecord$2(definition)) return Promise.reject(new AnnotationValidationError("Shape definition must be a plain object."));
		...
		update(id, patch) {
			...
			if (!isPlainRecord$2(patch)) return Promise.reject(new AnnotationValidationError("Shape update must be an object."));
		...
		requireSession(operation) {
			this.assertActive(operation);
			if (!this.session) throw new AnnotationValidationError(`Cannot ${operation} without a Shape session.`);
			return this.session;
		}
		assertImageLoaded() {
			if (!this.host.isImageLoaded()) throw new AnnotationValidationError("Shape Annotation requires a loaded image.");
		}
		assertActive(operation) {
			if (this.disposed) throw new AnnotationValidationError(`Cannot ${operation} after disposal.`);
		}
...
//#region dist/esm/plugins/annotation-shape/index.js
	const SHAPE_TOOL_ID$1 = "annotation:shape";
	...
	function shapeAnnotationPlugin(options = {}) {
		...
		return definePlugin({
			...
			setup(context) {
				...
				return Object.freeze({
					enter: (enterOptions) => context.operations.run("annotation-shape:enter", enterOptions, async (value) => {
						await context.tools.enter(SHAPE_TOOL_ID$1);
						try {
					...
					commit: async () => {
						try {
							...
							if (context.tools.getActiveToolId() === SHAPE_TOOL_ID$1) await context.tools.exit("operation");
						}
					},
					cancel: () => context.operations.run("annotation-shape:cancel", void 0, async () => {
						requireController().cancel();
						if (context.tools.getActiveToolId() === SHAPE_TOOL_ID$1) await context.tools.exit("requested");
					}),
			...
			onImageCleared(context) {
				if (context.tools.getActiveToolId() === SHAPE_TOOL_ID$1) return context.tools.exit("operation");
				controller === null || controller === void 0 || controller.closeForImage();
			},
			onDispose() {
				controller === null || controller === void 0 || controller.dispose();
...
//#region dist/esm/plugins/annotation-draw/draw-controller.js
	...
	function integerRange(value, label, minimum, maximum) {
		if (!Number.isSafeInteger(value) || Number(value) < minimum || Number(value) > maximum) throw new AnnotationValidationError(`${label} must be an integer from ${minimum} to ${maximum}.`);
		return Number(value);
	}
	...
	function resolveBrushConfiguration(value = {}, base = defaultBrush) {
		if (!isPlainRecord$1(value)) throw new AnnotationValidationError("Draw brush configuration must be a plain object.");
		const allowed = new Set(Object.keys(defaultBrush));
		if (Object.keys(value).some((key) => !allowed.has(key))) throw new AnnotationValidationError("Draw brush configuration contains unknown keys.");
		const merged = {
			...base,
			...value
		};
	...
	function resolveEraserConfiguration(value = {}, base = defaultEraser) {
		if (!isPlainRecord$1(value)) throw new AnnotationValidationError("Eraser configuration must be a plain object.");
		const allowed = new Set(Object.keys(defaultEraser));
		if (Object.keys(value).some((key) => !allowed.has(key))) throw new AnnotationValidationError("Eraser configuration contains unknown keys.");
		const merged = {
			...base,
			...value
		};
	...
	function normalizePoints(value, maximumCount) {
		if (!Array.isArray(value) || value.length < 2 || value.length > maximumCount) throw new AnnotationValidationError("Draw path point data is invalid.");
		try {
	...
	function isSerializedDraw(value) {
		...
		try {
			...
			if (!objectDescriptor || !("value" in objectDescriptor)) return false;
			const serializedObject = objectDescriptor.value;
			if (value.version !== 1 || !isPlainRecord$1(serializedObject) || !isSafeSerializedFabricObject(serializedObject, { rootTypes: ["path"] })) return false;
	...
	function isStatePoint(value) {
		return isPlainRecord$1(value) && typeof value.x === "number" && Number.isFinite(value.x) && typeof value.y === "number" && Number.isFinite(value.y);
	}
	function isDrawStateGeometry(value) {
		return isPlainRecord$1(value) && value.type === "path" && Array.isArray(value.points) && value.points.length >= 2 && value.points.length <= 65536 && value.points.every(isStatePoint);
	}
	function isDrawStateData(value) {
		if (!isPlainRecord$1(value) || value.version !== 1) return false;
		try {
	...
	var DrawAnnotationController = class {
		constructor(host, authoring, options) {
			...
			if (!isPlainRecord$1(options)) throw new AnnotationValidationError("Draw options must be a plain object.");
			if (Object.keys(options).some((key) => key !== "brush" && key !== "eraser")) throw new AnnotationValidationError("Draw options contain unknown keys.");
		...
		featureDefinition() {
			return Object.freeze({
				...
				codec: {
					...
					deserialize: async (value, context) => {
						if (!isSerializedDraw(value)) throw new AnnotationValidationError("Serialized Draw data is malformed.");
						const object = (await context.fabric.util.enlivenObjects([value.object]))[0];
						if (!(object instanceof context.fabric.Path)) throw new AnnotationValidationError("Fabric did not restore a Draw path.");
				...
				stateCodec: {
					...
					serialize: (object, context) => {
						var _a, _b;
						const points = normalizePoints(object.editorDrawPoints, 65536).map((point) => Object.freeze(context.toImageNormalized(objectPointToCanvas(object, point))));
						return Object.freeze({
					...
					deserialize: (value, context) => {
						if (!isDrawStateGeometry(value.geometry) || !isDrawStateData(value.data)) throw new AnnotationValidationError("Serialized Draw Annotation State data is malformed.");
		...
		enter(options = {}) {
			...
			if (!isPlainRecord$1(options)) throw new AnnotationValidationError("Draw enter options must be a plain object.");
			if (Object.keys(options).some((key) => key !== "subMode")) throw new AnnotationValidationError("Draw enter options contain unknown keys.");
			this.session = {
		...
		beginStroke(value) {
			...
			if (session.points.length > 0) throw new AnnotationValidationError("A Draw stroke is already active.");
			let point;
			try {
		...
		configureBrush(patch) {
			...
			if ((_a = this.session) === null || _a === void 0 ? void 0 : _a.points.length) throw new AnnotationValidationError("Cancel the active Draw stroke before configuring it.");
			this.brush = resolveBrushConfiguration(patch, this.brush);
		}
		configureEraser(patch) {
			var _a;
			this.assertActive("configure Eraser");
			if ((_a = this.session) === null || _a === void 0 ? void 0 : _a.points.length) throw new AnnotationValidationError("Cancel the active Eraser stroke before configuring it.");
		...
		requireSession(operation) {
			this.assertActive(operation);
			if (!this.session) throw new AnnotationValidationError(`Cannot ${operation} without a Draw session.`);
			return this.session;
		}
		requireActiveStroke(operation) {
			const session = this.requireSession(operation);
			if (session.points.length === 0) throw new AnnotationValidationError(`Cannot ${operation} without an active stroke.`);
			return session;
		}
		assertImageLoaded() {
			if (!this.host.isImageLoaded()) throw new AnnotationValidationError("Draw Annotation requires a loaded image.");
		}
		assertActive(operation) {
			if (this.disposed) throw new AnnotationValidationError(`Cannot ${operation} after disposal.`);
		}
...
//#region dist/esm/plugins/annotation-draw/index.js
	const DRAW_TOOL_ID$1 = "annotation:draw";
	...
	function drawAnnotationPlugin(options = {}) {
		...
		return definePlugin({
			...
			setup(context) {
				...
				return Object.freeze({
					enter: (enterOptions = {}) => context.operations.run("annotation-draw:enter", enterOptions, async (value) => {
						await context.tools.enter(DRAW_TOOL_ID$1);
						try {
							requireController().enter(value);
						} catch (error) {
							await context.tools.exit("operation");
					...
					exit: () => context.operations.run("annotation-draw:exit", void 0, async () => {
						...
						if (context.tools.getActiveToolId() === DRAW_TOOL_ID$1) await context.tools.exit("requested");
					}),
			...
			onImageCleared(context) {
				if (context.tools.getActiveToolId() === DRAW_TOOL_ID$1) return context.tools.exit("operation");
				controller === null || controller === void 0 || controller.closeForImage();
			},
			onDispose() {
				controller === null || controller === void 0 || controller.dispose();
...
//#region dist/esm/plugins/overlay-state/overlay-state-errors.js
	...
	var OverlayStateImageMissingError = class extends Error {
		constructor() {
	...
	var OverlayStateCodecError = class extends Error {
	...
	var OverlayStateIdConflictError = class extends Error {
		constructor(id) {
	...
	var OverlayStatePluginDisposedError = class extends Error {
		constructor(operation) {
...
//#endregion
//#region dist/esm/plugins/overlay-state/overlay-state-coordinate.js
	function createOverlayStateContext(baseImagePort) {
		...
		if (!baseImage || !imageInfo || !Number.isSafeInteger(imageInfo.naturalWidth) || imageInfo.naturalWidth <= 0 || !Number.isSafeInteger(imageInfo.naturalHeight) || imageInfo.naturalHeight <= 0) throw new OverlayStateImageMissingError();
		const matrixValue = baseImage.calcTransformMatrix();
		if (!isFiniteAffineMatrix(matrixValue)) throw new TypeError("[ImageEditor] Base Image transform is invalid.");
		...
		if (!Number.isFinite(scalarReference) || scalarReference <= 0) throw new TypeError("[ImageEditor] Base Image transform is singular.");
		const codec = Object.freeze({
			...
			toImageNormalizedScalar(value) {
				if (!Number.isFinite(value)) throw new TypeError("[ImageEditor] Overlay State scalar must be finite.");
...
//#endregion
//#region dist/esm/plugins/overlay-state/overlay-state-validation.js
	const DEFAULT_OVERLAY_STATE_LIMITS = Object.freeze({
		...
		maxCodecPayloadBytes: 1e6,
		maxCoordinates: 2e5,
		maxCoordinateMagnitude: 1e6,
		maxDrawPoints: 1e5,
		maxPathCommands: 1e5
	});
	...
	const IMAGE_KEYS = /* @__PURE__ */ new Set([
		"naturalWidth",
		"naturalHeight",
		"mimeType",
		"sourceId",
	...
	const CODEC_KEYS = /* @__PURE__ */ new Set(["type", "version"]);
	const MIME_TYPES = /* @__PURE__ */ new Set([
		"image/jpeg",
		"image/png",
		"image/webp"
	]);
	...
	function cloneJsonValue(value, path, depth, context) {
		...
		if (value === null) return {
			ok: accountBytes(context, 4, path),
			value: null
		};
		if (typeof value === "boolean") return {
			ok: accountBytes(context, value ? 4 : 5, path),
			value
		};
		if (typeof value === "number") {
			...
			return {
				ok: accountBytes(context, String(value).length, path),
				value
			};
		}
		if (typeof value === "string") {
			...
			return {
				ok: accountBytes(context, utf8Bytes(value) + 2, path),
				value
			};
		}
	...
	function resolvedPositiveInteger(value, fallback, key) {
		if (value === void 0) return fallback;
		if (!Number.isSafeInteger(value) || value <= 0) throw new TypeError(`[ImageEditor] Overlay State limit "${key}" must be a positive integer.`);
		return value;
	}
	function resolveOverlayStateLimits(base = {}, override = {}) {
		...
		const entries = Object.entries(DEFAULT_OVERLAY_STATE_LIMITS).map(([key, fallback]) => [key, resolvedPositiveInteger(merged[key], fallback, key)]);
		return Object.freeze(Object.fromEntries(entries));
	}
	function hasOnlyKeys(value, allowed, path, issues) {
		const unknown = Object.keys(value).filter((key) => !allowed.has(key));
		for (const key of unknown) addIssue(issues, "object.unknownKey", `${path}.${key}`, "Unknown field.");
		return unknown.length === 0;
	}
	function validIdentifier(value, path, limits, issues, persistent = false) {
		if (typeof value !== "string" || value.length === 0 || value.length > limits.maxIdentifierLength || !(persistent ? PERSISTENT_ID_PATTERN.test(value) : isRuntimeIdentifier(value))) {
	...
	function validateMetadata(value, path, limits, issues) {
		...
		const visit = (entry, entryPath, depth) => {
			...
			if (Array.isArray(entry)) {
				for (let index = 0; index < entry.length; index += 1) visit(entry[index], `${entryPath}[${index}]`, depth + 1);
				return;
			}
	...
	function inspectCodecValue(value, path, limits, issues) {
		...
		const visit = (entry, entryPath, key) => {
			if (typeof entry === "number") {
				coordinates += 1;
				if (Math.abs(entry) > limits.maxCoordinateMagnitude) addIssue(issues, "coordinate.outOfRange", entryPath, `Coordinate magnitude exceeds ${limits.maxCoordinateMagnitude}.`);
				return;
			}
			if (Array.isArray(entry)) {
				if (key === "points" && entry.length > limits.maxDrawPoints) addIssue(issues, "draw.tooManyPoints", entryPath, `Point count exceeds ${limits.maxDrawPoints}.`);
				if ((key === "commands" || key === "path") && entry.length > limits.maxPathCommands) addIssue(issues, "path.tooManyCommands", entryPath, `Path command count exceeds ${limits.maxPathCommands}.`);
				entry.forEach((child, index) => visit(child, `${entryPath}[${index}]`, null));
				return;
			}
			if (!isPlainRecord(entry)) return;
			for (const [childKey, child] of Object.entries(entry)) visit(child, `${entryPath}.${childKey}`, childKey);
		};
		visit(value, path, null);
		if (coordinates > limits.maxCoordinates) addIssue(issues, "coordinate.tooMany", path, `Coordinate count exceeds ${limits.maxCoordinates}.`);
	}
	function validateImage(value, limits, issues) {
		...
		for (const key of ["naturalWidth", "naturalHeight"]) {
			...
			if (!Number.isSafeInteger(dimension) || Number(dimension) <= 0) addIssue(issues, "image.dimensionInvalid", `$.image.${key}`, "Image dimensions must be positive safe integers.");
		}
		if (value.mimeType !== void 0 && (typeof value.mimeType !== "string" || !MIME_TYPES.has(value.mimeType))) addIssue(issues, "image.mimeTypeInvalid", "$.image.mimeType", "MIME type is invalid.");
		for (const key of ["sourceId", "checksum"]) {
			const entry = value[key];
			if (entry !== void 0 && (typeof entry !== "string" || entry.length === 0 || entry.length > limits.maxStringLength)) addIssue(issues, "image.referenceInvalid", `$.image.${key}`, "Image reference is invalid.");
		}
		return true;
	}
	function validateItem(value, index, limits, issues) {
		...
		if (!isPlainRecord(value.codec)) addIssue(issues, "codec.invalid", `${path}.codec`, "Codec reference must be an object.");
		else {
			hasOnlyKeys(value.codec, CODEC_KEYS, `${path}.codec`, issues);
			validIdentifier(value.codec.type, `${path}.codec.type`, limits, issues);
			if (typeof value.codec.version !== "string" || value.codec.version.length > limits.maxIdentifierLength || !SEMVER_PATTERN$1.test(value.codec.version)) addIssue(issues, "codec.versionInvalid", `${path}.codec.version`, "Codec version must be valid semantic versioning.");
		}
		if (!Object.prototype.hasOwnProperty.call(value, "geometry")) addIssue(issues, "overlay.geometryMissing", `${path}.geometry`, "Geometry is required.");
		if (!Object.prototype.hasOwnProperty.call(value, "data")) addIssue(issues, "overlay.dataMissing", `${path}.data`, "Codec data is required.");
		if (!Number.isSafeInteger(value.layer) || Number(value.layer) < 0) addIssue(issues, "overlay.layerInvalid", `${path}.layer`, "Layer must be a non-negative integer.");
		if (typeof value.hidden !== "boolean") addIssue(issues, "overlay.hiddenInvalid", `${path}.hidden`, "Hidden must be boolean.");
		if (typeof value.locked !== "boolean") addIssue(issues, "overlay.lockedInvalid", `${path}.locked`, "Locked must be boolean.");
		if (value.metadata !== void 0) validateMetadata(value.metadata, `${path}.metadata`, limits, issues);
		const codecPayload = Object.freeze({
			geometry: value.geometry,
			data: value.data,
			...value.metadata !== void 0 ? { metadata: value.metadata } : {}
		});
		if (jsonBytes(codecPayload) > limits.maxCodecPayloadBytes) addIssue(issues, "codec.payloadTooLarge", path, `Codec payload exceeds ${limits.maxCodecPayloadBytes} bytes.`);
		inspectCodecValue(codecPayload, path, limits, issues);
		return true;
	}
	function validateOverlayStateDocument(payload, limits) {
		const issues = [];
		const cloned = cloneJsonValue(payload, "$", 0, {
			limits,
			issues,
		...
		if (!Array.isArray(document.overlays)) addIssue(issues, "document.overlaysInvalid", "$.overlays", "Overlays must be an array.");
		else if (document.overlays.length > limits.maxOverlays) addIssue(issues, "document.tooManyOverlays", "$.overlays", `Overlay count exceeds ${limits.maxOverlays}.`);
		else {
			...
			document.overlays.forEach((item, index) => {
				...
				if (typeof item.id === "string") {
					if (ids.has(item.id)) addIssue(issues, "overlay.duplicateId", `$.overlays[${index}].id`, "Persistent IDs must be unique.");
					ids.add(item.id);
				}
				if (Number.isSafeInteger(item.layer)) {
					if (layers.has(Number(item.layer))) addIssue(issues, "overlay.duplicateLayer", `$.overlays[${index}].layer`, "Layer values must be unique.");
					layers.add(Number(item.layer));
				}
			});
		}
		if (document.metadata !== void 0) validateMetadata(document.metadata, "$.metadata", limits, issues);
...
//#endregion
//#region dist/esm/plugins/overlay-state/overlay-state-controller.js
	...
	function isCodecValue(value) {
		return typeof value === "object" && value !== null && Object.prototype.hasOwnProperty.call(value, "geometry") && Object.prototype.hasOwnProperty.call(value, "data");
	}
	function resolveStateKind(overlay, kind) {
		...
		if ((adapter === null || adapter === void 0 ? void 0 : adapter.persistence.mode) !== "persistent" || !codec || typeof codec.type !== "string" || !isRuntimeIdentifier(codec.type) || typeof codec.version !== "string" || !SEMVER_PATTERN.test(codec.version) || typeof codec.serialize !== "function" || typeof codec.validate !== "function" || typeof codec.deserialize !== "function") return null;
		return Object.freeze({
			adapter,
			codec
		});
	...
	var OverlayStateController = class {
		...
		exportState(options = {}) {
			...
			objects.forEach((object, layer) => {
				...
				if ((adapter === null || adapter === void 0 ? void 0 : adapter.persistence.mode) === "transient") return;
				...
				if (!codecAccepts(resolved.codec, value)) throw new OverlayStateCodecError(classification.kind, `produced invalid State Codec data for "${classification.persistentId}"`);
				overlays.push({
					id: classification.persistentId,
					kind: classification.kind,
					codec: Object.freeze({
		...
		async importState(payload, options = {}) {
			...
			if (!validated.valid || !validated.document) throw new OverlayStateValidationError(validated.errors);
			const document = validated.document;
			const context = createOverlayStateContext(this.baseImage);
			const existingObjects = this.overlay.list({
				includeHidden: true,
				includeLocked: true
			});
			const persistentIds = [];
		...
		validateCodecs(document, missingKindPolicy) {
			...
			document.overlays.forEach((item, index) => {
				const resolved = resolveStateKind(this.overlay, item.kind);
				if (!resolved || resolved.codec.type !== item.codec.type || resolved.codec.version !== item.codec.version) {
					if (missingKindPolicy === "error") issues.push(Object.freeze({
				...
				if (!codecAccepts(resolved.codec, stateValue(document, index))) issues.push(Object.freeze({
...
//#endregion
//#region dist/esm/plugins/overlay-state/index.js
...
//#region dist/esm/plugins/dom-controls/dom-controls-controller.js
	...
	function isEventTarget(value) {
		return isObject(value) && typeof value.addEventListener === "function" && typeof value.removeEventListener === "function";
	}
	function isElement(value) {
		return isObject(value) && value.nodeType === 1 && isObject(value.ownerDocument) && isEventTarget(value);
	}
	function resolveElement(ownerDocument, target, label) {
		...
		if (typeof target === "string") {
			...
			if (!element) throw new DomControlsConfigurationError(`${label} selector "${target}" did not match an element.`);
		}
		if (!isElement(element)) throw new DomControlsConfigurationError(`${label} must resolve to a DOM element.`);
		if (element.ownerDocument !== ownerDocument) throw new DomControlsConfigurationError(`${label} belongs to a different document than ownerDocument.`);
		return element;
	}
	function resolveButton(ownerDocument, target, label) {
		const element = resolveElement(ownerDocument, target, label);
		if (!("disabled" in element) || typeof element.disabled !== "boolean") throw new DomControlsConfigurationError(`${label} must resolve to a button control.`);
		return element;
	}
	function resolveInput(ownerDocument, target, label) {
		const element = resolveElement(ownerDocument, target, label);
		if (!("value" in element) || !("checked" in element)) throw new DomControlsConfigurationError(`${label} must resolve to an input control.`);
		return element;
	}
	function resolveApi(binding, label) {
		if (!binding) return null;
		if (typeof binding.resolve !== "function") throw new DomControlsConfigurationError(`${label}.plugin.resolve must be a function.`);
		const api = binding.resolve();
		if (!isObject(api)) throw new DomControlsConfigurationError(`${label}.plugin did not resolve a Plugin API.`);
		return api;
	}
	...
	function readNumericInput(input, label) {
		...
		if (!Number.isFinite(parsed)) throw new DomControlsConfigurationError(`${label} must contain a finite number.`);
		return parsed;
	}
	function isEditableNode(value, ownerDocument) {
		...
		while (isElement(current) && current.ownerDocument === ownerDocument) {
			...
			if (tagName === "input" || tagName === "textarea" || tagName === "select" || current.isContentEditable === true || current.getAttribute("contenteditable") === "true" || current.getAttribute("contenteditable") === "") return true;
			current = current.parentElement;
		}
		return false;
	}
	var DomControlsController = class {
		...
		bind() {
			...
			if (this.disposed) throw new DomControlsConfigurationError("DOM Controls cannot bind after disposal.");
			if (this.bound) throw new DomControlsConfigurationError("DOM Controls are already bound.");
			const options = this.requireOptions();
			if ((options.transform || options.history || options.masks || options.filters || options.crop || options.mosaic || options.annotations || options.text || options.shape || options.draw || options.keyboard) && !options.ownerDocument) throw new DomControlsConfigurationError("ownerDocument is required when DOM controls are configured.");
			try {
		...
		bindHistory(ownerDocument, options) {
			...
			if (controls.enabledInput) {
				...
				this.listen(input, "change", () => {
					this.runAction("history:toggle", () => input.checked ? api.enable({ baseline: "current" }) : api.disable({ clear: false }));
				});
				this.synchronizers.push(() => {
		...
		bindCrop(ownerDocument, options) {
			...
			this.render(ownerDocument, controls.status, "crop.status", () => Object.freeze({
		...
		bindMosaic(ownerDocument, options) {
			...
			this.render(ownerDocument, controls.status, "mosaic.status", () => Object.freeze({
				isActive: api.isActive,
				session: api.getSession()
			}));
			this.subscribe(api.subscribe(() => this.refreshFromRuntime()));
		...
		bindDraw(ownerDocument, options) {
			...
			this.button(ownerDocument, controls.cancelStrokeButton, "draw.cancelStrokeButton", () => api.cancelStroke(), () => {
				var _a;
				return ((_a = api.getSession()) === null || _a === void 0 ? void 0 : _a.isStrokeActive) === true;
			});
		...
		bindKeyboard(ownerDocument, options) {
			...
			this.listen(target, "keydown", (event) => {
				...
				if (keyboard.allowInEditable !== true && isEditableNode(keyboardEvent.target, ownerDocument)) return;
		...
		keyboardAction(event, options) {
			...
			if (keyboard.cancelActiveSession !== false && event.key === "Escape" && !modifier && !event.altKey) {
				const cancellers = [];
				if ((_c = apis.text) === null || _c === void 0 ? void 0 : _c.getEditingSession()) cancellers.push(() => apis.text.cancelEditing());
				if ((_d = apis.shape) === null || _d === void 0 ? void 0 : _d.getSession()) cancellers.push(() => apis.shape.cancel());
				if ((_e = apis.draw) === null || _e === void 0 ? void 0 : _e.getSession()) cancellers.push(() => apis.draw.exit());
				if ((_f = apis.crop) === null || _f === void 0 ? void 0 : _f.isActive) cancellers.push(() => apis.crop.cancel());
				if ((_g = apis.mosaic) === null || _g === void 0 ? void 0 : _g.isActive) cancellers.push(() => apis.mosaic.cancel());
			...
			if (keyboard.removeSelection !== false && (event.key === "Delete" || event.key === "Backspace") && !modifier && !event.altKey && !event.shiftKey) {
		...
		resolveKeyboardTarget(ownerDocument, target) {
			if (target === void 0 || target === ownerDocument) return ownerDocument;
			if (typeof target === "string" || isElement(target)) return resolveElement(ownerDocument, target, "keyboard.target");
			if (!isEventTarget(target) || isObject(target) && target.nodeType !== 9) throw new DomControlsConfigurationError("keyboard.target must be ownerDocument, an element, or a selector.");
		...
		render(ownerDocument, adapter, label, read) {
			if (!adapter) return;
			if (typeof adapter.render !== "function") throw new DomControlsConfigurationError(`${label}.render must be a function.`);
		...
		listen(target, eventName, listener) {
			var _a;
			const events = (_a = this.occupiedBindings.get(target)) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Set();
			if (events.has(eventName)) throw new DomControlsConfigurationError(`The same target cannot bind the "${eventName}" event more than once.`);
		...
		releaseBindings() {
			...
			for (let index = this.removers.length - 1; index >= 0; index -= 1) try {
				this.removers[index]();
			} catch (error) {
		...
		requireOptions() {
			if (!this.options) throw new DomControlsConfigurationError("DOM Controls options are unavailable.");
			return this.options;
		}
		requireApis() {
			if (!this.apis) throw new DomControlsConfigurationError("DOM Controls Plugin APIs are unavailable.");
			return this.apis;
		}
	};
...
//#region dist/esm/plugins/dom-controls/index.js
	...
	function domControlsPlugin(options = {}) {
		...
		return definePlugin({
			...
			setupMode: "sync",
			setup(context) {
				if (!configuredOptions) throw new DomControlsConfigurationError("DOM Controls options are unavailable.");
				...
				for (const operationId of ["dom-controls:bind", "dom-controls:refresh"]) context.disposables.add(context.operations.register({
					id: operationId,
					mode: "busy",
					conflictDomains: ["state"],
					reentrancy: "queue"
				...
				const requireController = () => {
					if (!controller) throw new DomControlsConfigurationError("DOM Controls are not installed.");
					return controller;
				};
				return Object.freeze({
			...
			onInit() {
				controller === null || controller === void 0 || controller.bind();
			},
			onDispose() {
				controller === null || controller === void 0 || controller.dispose();
				configuredOptions = null;
	...
	var InteractionRuntime = class {
		...
		down(sample) {
			...
			try {
				...
				this.onStatusChange();
				if (isPromiseLike(claim.started)) observePromise(claim.started, (error) => {
					var _a;
		...
		invoke(owner, operation, task, complete = false) {
			try {
				const result = task();
				if (isPromiseLike(result)) observePromise(Promise.resolve(result).then(() => {
		...
		handleError(error, bindingId, operation) {
			...
			if (owner) try {
				...
				if (isPromiseLike(cleanup)) observePromise(cleanup, (cleanupError) => {
