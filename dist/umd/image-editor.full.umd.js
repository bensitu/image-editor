	...
	var OperationRegistry = class {
		...
		attachExternalAbort(request) {
			...
			const abort = () => {
				...
				const reason = signal ? abortReason(signal, `Operation "${request.record.definition.id}" was aborted.`) : abortError$3(`Operation "${request.record.definition.id}" was aborted.`);
				if (request.state === "pending") {
					this.pendingRequests = this.pendingRequests.filter((entry) => entry !== request);
		...
		registerFeature(definition) {
			...
			try {
				registrations.push(this.overlay.registerKind({
					...
					classify: (object) => object.editorAnnotationKind === normalizedDefinition.kind && normalizedDefinition.classify(object),
					getPersistentId: (object) => {
						var _a;
					...
					persistence: {
						...
						codec: {
							...
							deserialize: async (value, context) => {
								var _a;
								if (!isEnvelopeShape(value) || !normalizedDefinition.codec.validate(value.feature)) throw new AnnotationValidationError(`Serialized ${normalizedDefinition.kind} data is malformed.`);
								...
								(_a = normalizedDefinition.synchronize) === null || _a === void 0 || _a.call(normalizedDefinition, object);
								return object;
							}
						}
					},
					...normalizedDefinition.stateCodec ? { stateCodec: {
						...
						deserialize: async (value, context) => {
							var _a;
							if (!isStateData(value.data) || !isValidAnnotationMetadata(value.metadata)) throw new AnnotationValidationError(`Serialized ${normalizedDefinition.kind} State data is malformed.`);
				...
				registrations.push(this.overlay.registerGeometryPolicy({
					...
					supports: (mutation) => {
						var _a;
						return mutation.kind === "crop" || mutation.kind === "transform" && ((_a = normalizedDefinition.bindToImageTransform) === null || _a === void 0 ? void 0 : _a.call(normalizedDefinition)) === true;
					},
...
//#region dist/esm/mask/mask-factory.js
	...
	function createMask(context, config = {}) {
		...
		if (mergedConfig.left === void 0 && previousMask) {
			const previousRight = ((_b = previousMask.left) !== null && _b !== void 0 ? _b : 0) + (typeof previousMask.getScaledWidth === "function" ? previousMask.getScaledWidth() : ((_c = previousMask.width) !== null && _c !== void 0 ? _c : 0) * ((_d = previousMask.scaleX) !== null && _d !== void 0 ? _d : 1));
		...
		if (!validateFiniteField(options, "left", left) || !validateFiniteField(options, "top", top) || !validatePositiveField(options, "width", resolvedConfig.width) || !validatePositiveField(options, "height", resolvedConfig.height) || !validateFiniteField(options, "gap", resolvedConfig.gap) || !validateFiniteField(options, "angle", resolvedConfig.angle) || !validateFiniteField(options, "alpha", resolvedConfig.alpha)) return null;
		if (rx !== void 0 && !validateNonNegativeField(options, "rx", rx) || ry !== void 0 && !validateNonNegativeField(options, "ry", ry) || radius !== void 0 && !validatePositiveField(options, "radius", radius) || shapeType === "polygon" && polygonPoints === null) return null;
		...
		if (typeof config.fabricGenerator === "function") {
			...
			switch (shapeType) {
				...
				default: mask = new fabricModule.Rect({
					left,
					top,
					...originProps,
