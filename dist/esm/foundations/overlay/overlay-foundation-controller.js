...
async function decodeFlattenImage(fabric, dataUrl, timeoutMs, parentSignal) {
    ...
    try {
        ...
        if (parentSignal.aborted)
            throw (_a = parentSignal.reason) !== null && _a !== void 0 ? _a : error;
        if (controller.signal.aborted)
            throw (_b = controller.signal.reason) !== null && _b !== void 0 ? _b : error;
...
export class OverlayFoundationController {
    ...
    registerKind(definition) {
        ...
        if (!isRecord(definition) ||
            ...
            throw new PluginManifestError('Overlay Kind registration requires callable classify and persistent identity members.', isRecord(definition) && typeof definition.ownerPluginId === 'string'
    ...
    async restoreState(value) {
        ...
        try {
            for (const record of value.overlays) {
                ...
                if (!object || typeof object !== 'object' || object.canvas) {
                    throw new CoreRuntimeError(`[ImageEditor] Serialized overlay "${record.persistentId}" restored an incompatible object.`);
                }
        ...
        try {
            ...
            this.disposeDetachedObjects(restored.map(({ object }) => object), 'Rejected Overlay restore object cleanup failed.');
            if (rollbackError) {
                throw new CoreRuntimeError('[ImageEditor] Overlay state restore and local rollback both failed.', {
    ...
    beginGesture(target, action) {
        ...
        const predecessor = previousCommit && previousCommit === supersededTransaction
            ? previousCommit.catch((error) => {
                if (error !== supersessionReason)
                    throw error;
            })
            : previousCommit;
        ...
        transaction.then(() => {
            ...
            if (this.gestureCommitTail === transaction)
                this.gestureCommitTail = null;
        }, () => {
            this.clearGesture(gesture);
            if (this.gestureCommitTail === transaction)
                this.gestureCommitTail = null;
    ...
    previewGesture(target, action) {
        const gesture = this.activeGesture;
        if (!target || !gesture || gesture.completionSettled)
            return;
    ...
    async rebaseSupersedingGesture(gesture) {
        ...
        if (!gesture.completionSettled || !gesture.sealedTargets)
            return;
        ...
        for (const target of gesture.targets) {
            ...
            if (!transform) {
                throw new CoreRuntimeError(`[ImageEditor] Superseding Overlay gesture lost target "${target.persistentId}".`);
            }
