...
const MAX_OBJECT_COUNT = 100000;
const MAX_DEPTH = 64;
const MAX_CANVAS_DIMENSION = 32768;
const MAX_CANVAS_PIXELS = 50000000;
...
function inspectJsonValue(value, limits, path = '$', depth = 0, ancestors = new WeakSet(), counter = { value: 0 }) {
    ...
    if (value === null || typeof value !== 'object') {
        if (value === undefined ||
            ...
            typeof value === 'symbol' ||
            typeof value === 'bigint') {
            throw new SnapshotMigrationError('input.value', `Snapshot contains unsupported ${typeof value} data.`, path);
        }
...
function cloneInput(input, options) {
    ...
    if (typeof input === 'string') {
        ...
        try {
            ...
            throw new SnapshotMigrationError('input.json', 'Snapshot input is not valid JSON.', '$', {
                cause: error,
            });
        }
    }
...
function hasSourceDiscriminator(value) {
    if (!isRecord(value) || 'schema' in value || !Array.isArray(value.objects))
        return false;
...
export function detectSnapshotVersion(input) {
...
function issue(context, code, path, message) {
    var _a;
    if (context.policy === 'error')
        throw new SnapshotMigrationError(code, message, path);
    (_a = context.onWarning) === null || _a === void 0 ? void 0 : _a.call(context, Object.freeze({ code, path, message }));
}
...
function reportUnknownKeys(value, allowed, path, context) {
    for (const key of Object.keys(value)) {
        if (!allowed.has(key)) {
            issue(context, 'field.unsupported', `${path}.${key}`, `Unsupported persisted field "${key}" cannot be converted.`);
        }
    }
}
...
function requireSource(input, options) {
    ...
    if (!hasSourceDiscriminator(value)) {
        ...
        throw new SnapshotMigrationError('schema.unsupported', detection.kind === 'current'
...
function sourceObjects(value) {
    return value.objects.map((object, index) => {
        if (!isRecord(object)) {
            throw new SnapshotMigrationError('object.invalid', 'Canvas objects must be plain records.', `$.objects[${index}]`);
        }
        return object;
    });
}
function findBaseImage(objects) {
    ...
    if (explicit.length > 1) {
        throw new SnapshotMigrationError('base.multiple', 'Snapshot contains more than one Base Image.', '$.objects');
    }
    if (explicit.length === 1) {
        const image = explicit[0];
        if (objectType(image) !== 'image') {
            throw new SnapshotMigrationError('base.type', 'The Base Image must be a Fabric Image.', '$.objects');
        }
        return image;
    }
    const images = objects.filter((object) => objectType(object) === 'image');
    if (images.length > 1) {
        throw new SnapshotMigrationError('base.ambiguous', 'Snapshot has multiple unmarked Images and no unambiguous Base Image.', '$.objects');
    }
...
function dimensions(source, options) {
    ...
    if (!isFiniteNumber(width) || width <= 0 || !isFiniteNumber(height) || height <= 0) {
        throw new SnapshotMigrationError('canvas.size', 'Snapshot Canvas dimensions are missing; provide canvasSize explicitly.', '$');
    }
    if (width > MAX_CANVAS_DIMENSION ||
        height > MAX_CANVAS_DIMENSION ||
        width * height > MAX_CANVAS_PIXELS) {
        throw new SnapshotMigrationError('canvas.size', 'Snapshot Canvas dimensions exceed the public Snapshot limits.', '$');
    }
    return Object.freeze({ width, height });
}
function transformState(state, base) {
    if (!isFiniteNumber(state.currentScale) || state.currentScale <= 0) {
        throw new SnapshotMigrationError('transform.scale', 'Snapshot scale must be positive and finite.', '$._editorState.currentScale');
    }
    if (!isFiniteNumber(state.currentRotation)) {
        throw new SnapshotMigrationError('transform.rotation', 'Snapshot rotation must be finite.', '$._editorState.currentRotation');
    }
...
function filterDefinitions(state, base, context) {
    ...
    if (raw === undefined) {
        if (Array.isArray(base === null || base === void 0 ? void 0 : base.filters) && base.filters.length > 0) {
            issue(context, 'filter.fabric', '$.objects.filters', 'Fabric filter payload has no authoritative editor filter configuration.');
        }
        return Object.freeze([]);
    }
    if (!isRecord(raw)) {
    ...
    for (const key of FILTER_KEYS) {
        ...
        if (key === 'grayscale' || key === 'sepia' || key === 'vintage') {
            if (value === undefined || value === false)
                continue;
            if (value === true)
                definitions.push(Object.freeze({ type: key }));
            else
                issue(context, 'filter.value', `$._editorState.imageFilterConfig.${key}`, `Filter "${key}" must be boolean.`);
            continue;
        }
...
function pathPoints(value) {
    ...
    for (const segment of value) {
        ...
        if (!['M', 'L', 'T', 'Q', 'S', 'C'].includes(command))
            continue;
...
function shapeGeometry(object, kind, path) {
    if (kind === 'rect') {
        if (!isFiniteNumber(object.left) ||
            !isFiniteNumber(object.top) ||
            !isFiniteNumber(object.width) ||
            object.width <= 0 ||
            !isFiniteNumber(object.height) ||
            object.height <= 0) {
            throw new SnapshotMigrationError('annotation.geometry', 'Rectangle geometry is invalid.', path);
        }
        return Object.freeze({
            kind,
            left: object.left,
    ...
    if (!start || !end || Math.hypot(end.x - start.x, end.y - start.y) < 0.5) {
        throw new SnapshotMigrationError('annotation.geometry', 'Linear geometry is invalid.', path);
    }
...
function annotationRecord(object, index, context) {
    ...
    if (!overlayId(persistentId)) {
        return rejectObject(context, 'annotation.persistentId', path, 'Annotation persistent identifier is invalid.');
    }
    ...
    if (object.overlayMetadata !== undefined) {
        if (isRecord(object.overlayMetadata))
            ...
            issue(context, 'annotation.metadata', `${path}.overlayMetadata`, 'Annotation metadata was skipped because it is not an object.');
    }
    const interaction = Object.freeze({
        ...
        ...(object.annotationType === 'text'
            ? {
                editable: interactionValue(object, 'annotationEditable', object.editable !== false, path, context),
            }
            : {}),
    });
    const serialized = sanitizedFabricObject(object);
    ...
    let feature;
    if (object.annotationType === 'text') {
        if (objectType(serialized) !== 'textbox' ||
            ...
            return rejectObject(context, 'annotation.text', path, 'Text Annotation payload is invalid.');
        }
        codecType = 'annotation:textbox';
        feature = serialized;
    }
    else if (object.annotationType === 'shape') {
        const shapeKind = object.shapeAnnotationKind === 'line' || object.shapeAnnotationKind === 'arrow'
            ? object.shapeAnnotationKind
            : 'rect';
        const expectedType = shapeKind === 'arrow' ? 'path' : shapeKind;
        if (objectType(serialized) !== expectedType) {
            return rejectObject(context, 'annotation.shape', path, 'Shape Annotation payload is invalid.');
        }
        ...
        try {
            ...
            if (error instanceof SnapshotMigrationError && context.policy === 'warn-and-skip') {
                (_a = context.onWarning) === null || _a === void 0 ? void 0 : _a.call(context, Object.freeze({ code: error.code, path: error.path, message: error.message }));
                return null;
            }
            throw error;
        }
        ...
        if (objectType(serialized) !== 'path' || points.length < 2) {
            return rejectObject(context, 'annotation.draw', path, 'Draw Annotation payload is invalid.');
        }
...
function selectionIds(state, maskIds, annotationIds, context) {
    ...
    if (state.activeObjectKind === 'mask' && Number.isSafeInteger(state.activeMaskId)) {
        ...
        if (id)
            ...
            issue(context, 'selection.missing', '$._editorState.activeMaskId', 'Selected Mask was not present in the converted overlays.');
    }
    else if (state.activeObjectKind === 'annotation' &&
        Number.isSafeInteger(state.activeAnnotationId)) {
        const id = annotationIds.get(Number(state.activeAnnotationId));
        if (id)
            ids.push(id);
        else
            issue(context, 'selection.missing', '$._editorState.activeAnnotationId', 'Selected Annotation was not present in the converted overlays.');
    }
    else if (state.activeObjectKind !== undefined &&
        state.activeObjectKind !== null &&
        state.activeObjectKind !== 'mask' &&
        state.activeObjectKind !== 'annotation') {
        issue(context, 'selection.kind', '$._editorState.activeObjectKind', 'Unsupported active object state was skipped.');
    }
    return Object.freeze(ids);
}
export function migrateV2Snapshot(input, options = {}) {
    var _a;
    ...
    if (!isFiniteNumber(state.baseImageScale) || state.baseImageScale <= 0) {
        throw new SnapshotMigrationError('base.scale', 'Base Image scale must be positive and finite.', '$._editorState.baseImageScale');
    }
    const overlays = [];
    ...
    for (let index = 0; index < objects.length; index += 1) {
        ...
        else if (object.editorObjectKind === 'annotation' ||
            Number.isSafeInteger(object.annotationId)) {
            record = annotationRecord(object, index, context);
            if (record)
                annotationIds.set(Number(object.annotationId), record.persistentId);
        }
        else {
            rejectObject(context, object.editorObjectKind === 'session' ||
                object.isCropRect === true ||
                object.maskLabel === true ||
                object.isMosaicPreview === true
                ? 'object.transient'
                : 'object.unsupported', `$.objects[${index}]`, 'Unsupported Canvas object was skipped.');
        }
        if (record)
    ...
    if (new Set(persistentIds).size !== persistentIds.length) {
        throw new SnapshotMigrationError('overlay.duplicate', 'Converted overlays contain duplicate persistent identifiers.', '$.objects');
    }
    if (!base && overlays.length > 0) {
        throw new SnapshotMigrationError('base.missing', 'Overlay state cannot be converted without a Base Image.', '$.objects');
    }
...
export function v2SnapshotMigration(options = {}) {
    ...
    return Object.freeze({
        ...
        migrate: (input, context) => {
            var _a, _b;
            (_a = context.signal) === null || _a === void 0 ? void 0 : _a.throwIfAborted();
            const snapshot = migrateV2Snapshot(input, conversionOptions);
            (_b = context.signal) === null || _b === void 0 ? void 0 : _b.throwIfAborted();
            return snapshot;
        },
    });
}
export async function loadV2Snapshot(editor, input, options = {}) {
