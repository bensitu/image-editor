...
import { BASE_IMAGE_INFO_CAPABILITY, CORE_DIAGNOSTICS_CAPABILITY, FABRIC_RUNTIME_CAPABILITY, PluginNotInstalledError, coreOperationIds, definePlugin, definePluginRef, observePromise, } from '../../sdk/index.js';
...
export function textAnnotationPlugin(options = {}) {
    ...
    return definePlugin({
        ...
        setup(context) {
            ...
            context.disposables.add(annotations.subscribe((status) => {
                ...
                if (!session ||
                    status.annotations.some((annotation) => annotation.id === session.annotationId)) {
                    return;
                }
                if (context.tools.getActiveToolId() === TEXT_TOOL_ID) {
                    ...
                    controller === null || controller === void 0 ? void 0 : controller.cancelEditing();
                }
            }));
            ...
            const api = {
                enter: () => context.operations.run('annotation-text:enter', undefined, async () => {
                ...
                exit: () => {
                    ...
                    return context.operations.run('annotation-text:exit', undefined, () => context.tools.exit('requested'));
                },
