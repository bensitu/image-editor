...
const require_plugin_manager = require('../chunks/plugin-manager-BdEFq773.cjs');
...
function createAbortError() {
	const error = /* @__PURE__ */ new Error("Image decoding was aborted.");
...
function createControlledImageDecoder() {
	...
	const remove = (entry) => {
		...
		(_a = entry.signal) === null || _a === void 0 || _a.removeEventListener("abort", entry.abortListener);
	};
	const takeNext = () => {
	...
	return Object.freeze({
		...
		decode(input, signal) {
			...
			if (signal === null || signal === void 0 ? void 0 : signal.aborted) deferred.reject(createAbortError());
			else {
				pending.push(entry);
				signal === null || signal === void 0 || signal.addEventListener("abort", abortListener, { once: true });
			}
...
//#endregion
//#region dist/esm/testing/fabric-test-harness.js
function descriptorMatches(left, right) {
	if (!left || !right) return left === right;
	return left.configurable === right.configurable && left.enumerable === right.enumerable && left.writable === right.writable && left.value === right.value && left.get === right.get && left.set === right.set;
}
function createPluginTestFabric(module) {
	...
	return Object.freeze({
		...
		assertUnchanged() {
			...
			const sameKeys = currentKeys.length === keys.length && currentKeys.every((key, index) => key === keys[index]);
			const sameDescriptors = keys.every((key) => descriptorMatches(descriptors.get(key), Object.getOwnPropertyDescriptor(module, key)));
			if (!sameKeys || !sameDescriptors) throw new Error("Fabric namespace changed during the Plugin test.");
		}
	});
}

//#endregion
//#region dist/esm/testing/plugin-test-host.js
function createPluginTestHost(options = {}) {
	...
	const manager = new require_plugin_manager.PluginManager({
		...
		hostCapabilities: ((_a = options.hostCapabilities) !== null && _a !== void 0 ? _a : []).map((provider) => ({
			...
			...provider.requiredPermission ? { requiredPermission: provider.requiredPermission } : {}
		}))
	});
	return Object.freeze({
		get state() {
...
//#endregion
//#region dist/esm/testing/responsibility-assertions.js
...
async function evaluate(id, contract, source, inspect) {
	if (!source) return unavailable$1(id, contract);
	try {
...
function assertBundleIsolation(source) {
	return evaluate("bundle-isolation", "The package uses public imports and has no private or unknown bundled modules.", source, (observation) => {
		...
		const passed = observation.internalImports === 0 && observation.privateAliases === 0 && ((_a = observation.unknownModules) !== null && _a !== void 0 ? _a : 0) === 0 && bundledRuntimeModules.length === 0;
		return {
			passed,
			message: passed ? "Package module isolation is intact." : "Package module isolation contains forbidden imports or modules.",
...
function descriptorFingerprint(descriptor) {
	return Object.freeze({
		...
		writable: "writable" in descriptor ? descriptor.writable === true : void 0,
		value: "value" in descriptor ? descriptor.value : void 0,
		get: descriptor.get,
		set: descriptor.set
	});
}
function captureSurface(surfaces, name, value) {
	...
	for (const key of Reflect.ownKeys(value)) {
		...
		if (descriptor) properties.set(key, descriptorFingerprint(descriptor));
	}
	if (value instanceof Map) for (const [key, entry] of value) properties.set(`[[Map:${String(key)}]]`, descriptorFingerprint({ value: entry }));
	if (value instanceof Set) for (const entry of value) properties.set(`[[Set:${String(entry)}]]`, descriptorFingerprint({ value: entry }));
...
function captureFabricGlobalState(fabric) {
	...
	for (const key of Reflect.ownKeys(fabric)) {
		...
		if (typeof value === "function" && value.prototype && typeof value.prototype === "object") captureSurface(surfaces, `${name}.prototype`, value.prototype);
		if (typeof value === "object" && value !== null && [
			...
			for (const childKey of Reflect.ownKeys(value)) {
				...
				if (childDescriptor && "value" in childDescriptor && typeof childDescriptor.value === "object" && childDescriptor.value !== null) captureSurface(surfaces, `${name}.${String(childKey)}`, childDescriptor.value);
			}
		}
	}
	return surfaces;
...
function descriptorsEqual(left, right) {
	return left.configurable === right.configurable && left.enumerable === right.enumerable && left.writable === right.writable && Object.is(left.value, right.value) && left.get === right.get && left.set === right.set;
}
function globalDifferences(before, after) {
	var _a, _b;
	const differences = /* @__PURE__ */ new Set();
	const surfaceNames = /* @__PURE__ */ new Set([...before.keys(), ...after.keys()]);
	for (const surfaceName of surfaceNames) {
		const left = (_a = before.get(surfaceName)) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Map();
		const right = (_b = after.get(surfaceName)) !== null && _b !== void 0 ? _b : /* @__PURE__ */ new Map();
		const keys = /* @__PURE__ */ new Set([...left.keys(), ...right.keys()]);
		for (const key of keys) {
			const leftValue = left.get(key);
			const rightValue = right.get(key);
			if (!leftValue || !rightValue || !descriptorsEqual(leftValue, rightValue)) differences.add(`${surfaceName}.${String(key)}`);
		}
	}
...
async function assertNoUndeclaredFabricGlobalMutation(lifecycle) {
	...
	if (!lifecycle) return unavailable$1(id, contract);
	const changes = [];
	...
	try {
		...
		if (surfaces.length > 0) changes.push(Object.freeze({
			phase: "import",
			surfaces
		}));
		previous = current;
	...
	const declared = ((_c = (_a = lifecycle.declaredPermissions) !== null && _a !== void 0 ? _a : (_b = definition === null || definition === void 0 ? void 0 : definition.manifest) === null || _b === void 0 ? void 0 : _b.permissions) !== null && _c !== void 0 ? _c : Object.freeze([])).includes("fabric:global-mutation");
	...
	if (changedSurfaces.length === 0) return result(id, contract, "PASS", "Known Fabric global surfaces are unchanged.", details);
	if (declared) return result(id, contract, "PASS_WITH_DOWNGRADED_ISOLATION", "Declared Fabric global mutation requires downgraded isolation.", details);
...
function assertStrongMultiInstanceIsolation(source) {
	return evaluate("multi-instance-isolation", "Core, Plugin, operation, tool, Overlay, History, and Fabric state are isolated by instance.", source, (observation) => {
		const failures = Object.entries(observation).filter(([, isolated]) => !isolated).map(([name]) => name).sort();
		return {
			passed: failures.length === 0,
			message: failures.length === 0 ? "Strong multi-instance isolation is intact." : `Isolation failed for ${failures.join(", ")}.`,
...
function assertPeerDependencyContract(source) {
	return evaluate("peer-dependency-contract", "Core and Fabric are peers and are never vendored by the Plugin package.", source, (manifest) => {
		...
		const bundled = Array.isArray(manifest.bundledDependencies) ? manifest.bundledDependencies : [];
		...
		const vendored = requiredPeers.filter((dependency) => dependency in runtimeDependencies || dependency in optionalDependencies || bundled.includes(dependency) || manifest.bundledDependencies === true);
		const passed = missingPeers.length === 0 && vendored.length === 0;
		return {
			passed,
			message: passed ? "Core and Fabric peer dependency declarations are valid." : "Core or Fabric peer dependency declarations are invalid.",
			details: Object.freeze({
...
function assertPackageDoesNotBundleCoreOrFabric(source) {
	return evaluate("package-runtime-externalization", "The Plugin artifact contains no Core or Fabric runtime module.", source, (observation) => {
		...
		return {
			...
			message: core === 0 && fabric === 0 ? "Core and Fabric remain external." : "The Plugin artifact bundles Core or Fabric.",
			details: Object.freeze({
				bundledCoreModules: core,
				bundledFabricModules: fabric
			})
		};
	});
}
function assertBaseImageInvariant(source) {
	return evaluate("base-image-invariant", "Prohibited Base Image actions are rejected without observable document effects.", source, (observation) => {
		const failed = observation.attempts.filter((attempt) => !attempt.rejected || !attempt.documentUnchanged || !attempt.historyUnchanged || !attempt.committedEventAbsent || !attempt.instanceUsable).map((attempt) => attempt.action).sort();
		return {
			passed: observation.attempts.length > 0 && failed.length === 0,
			message: observation.attempts.length > 0 && failed.length === 0 ? "Base Image invariants rejected every prohibited action." : "A prohibited Base Image action produced an observable effect.",
			details: Object.freeze({
...
function assertOverlayMutationHistory(source) {
	return evaluate("overlay-mutation-history", "A committed Overlay mutation owns one transaction, History record, and event.", source, (observation) => {
		const passed = observation.topLevelTransactions === 1 && observation.historyRecords === 1 && observation.committedEvents === 1 && observation.registrationLeaks === 0;
		return {
			passed,
			message: passed ? "Overlay mutation ownership and cleanup are exact." : "Overlay mutation ownership or cleanup is not exact.",
...
function assertCompoundTransaction(source) {
	return evaluate("compound-transaction", "Compound work commits one Memento pair, History record, and event with atomic restore.", source, (observation) => {
		const passed = observation.topLevelTransactions === 1 && observation.mementoPairs === 1 && observation.historyRecords === 1 && observation.committedEvents === 1 && observation.undoRestoredAll && observation.redoRestoredAll && observation.participantFailureRolledBackAll && observation.nestedWorkPublishedOnce && observation.activeSelectionAtomic;
		return {
			passed,
			message: passed ? "Compound transaction ownership and restoration are atomic." : "Compound transaction evidence is incomplete or non-atomic.",
			details: Object.freeze({ ...observation })
		};
	});
}
function assertSliceMigration(source) {
	return evaluate("slice-migration", "Plugin-owned Slice migration is deterministic, validated, isolated, and failure-atomic.", source, (observation) => {
		const passed = Number.isSafeInteger(observation.sourceVersion) && Number.isSafeInteger(observation.targetVersion) && observation.sourceVersion < observation.targetVersion && observation.migrated && observation.deterministic && observation.validatedBeforeCommit && observation.failedMigrationMutationCount === 0 && observation.futureVersionTypedFailure && observation.missingPluginPolicyPreserved && observation.privateAccesses === 0;
		return {
			passed,
			message: passed ? "Plugin-owned Slice migration satisfies the public contract." : "Plugin-owned Slice migration evidence is incomplete.",
			details: Object.freeze({ ...observation })
		};
	});
}
...
function createSourcePlugin(plugin, options) {
	...
	if (source.ref.id !== plugin.ref.id || source.ref.apiVersion !== plugin.ref.apiVersion || source.manifest.version !== plugin.manifest.version) throw new Error("Plugin factory returned a different Plugin identity.");
	return source;
}
function wrapPlugin(source, definition) {
	...
	return Object.freeze({
		...
		onInit: (_c = definition.onInit) !== null && _c !== void 0 ? _c : ((context) => {
			var _a;
			return Promise.resolve((_a = source.onInit) === null || _a === void 0 ? void 0 : _a.call(source, context)).then(() => void 0);
		}),
		onImageLoaded: (_d = definition.onImageLoaded) !== null && _d !== void 0 ? _d : ((image, context) => {
			var _a;
			return Promise.resolve((_a = source.onImageLoaded) === null || _a === void 0 ? void 0 : _a.call(source, image, context)).then(() => void 0);
		}),
		onImageCleared: (_e = definition.onImageCleared) !== null && _e !== void 0 ? _e : ((context) => {
			var _a;
			return Promise.resolve((_a = source.onImageCleared) === null || _a === void 0 ? void 0 : _a.call(source, context)).then(() => void 0);
		}),
		onDispose: (_f = definition.onDispose) !== null && _f !== void 0 ? _f : ((context) => {
			var _a;
			return Promise.resolve((_a = source.onDispose) === null || _a === void 0 ? void 0 : _a.call(source, context)).then(() => void 0);
		})
	});
}
async function createFixture(options) {
	...
	try {
		for (const dependency of (_d = (_c = options.createDependencies) === null || _c === void 0 ? void 0 : _c.call(options)) !== null && _d !== void 0 ? _d : []) await host.install(dependency);
		return fixture;
	} catch (error) {
		try {
			await disposeFixture(fixture);
		} catch (cleanupFailure) {
			if (cleanupFailure instanceof Error && !("cause" in cleanupFailure)) Object.defineProperty(cleanupFailure, "cause", { value: error });
...
async function disposeFixture(fixture) {
	...
	for (const provider of fixture.providers) try {
	...
	if (failures.length > 0) throw new Error(`Plugin test cleanup failed: ${failures.join(" | ")}`);
}
async function useFixture(options, operation) {
	...
	try {
		...
		if (operationFailure === void 0) throw cleanupFailure;
		if (cleanupFailure instanceof Error && !("cause" in cleanupFailure)) Object.defineProperty(cleanupFailure, "cause", { value: operationFailure });
		throw cleanupFailure;
	}
	if (operationFailure !== void 0) throw operationFailure;
	return result;
}
function cloneStateValue(value, seen = /* @__PURE__ */ new Set()) {
	...
	if (typeof value === "object") {
		...
		for (const key of Object.keys(value).sort()) {
			if (require_plugin_identifier.isDangerousStateKey(key)) throw new Error(`State fixtures must not contain dangerous key "${key}".`);
			const descriptor = Object.getOwnPropertyDescriptor(value, key);
			if (!descriptor || !("value" in descriptor)) throw new Error("State fixtures must contain only data properties.");
...
async function assertInstallRollback(plugin, options = {}) {
	return executeAssertion("install-rollback", "Failed setup removes all scoped registrations and permits a clean installation.", async () => {
		...
		const sentinel = /* @__PURE__ */ new Error("Forced setup failure.");
		await useFixture(options, async ({ host }) => {
			const source = createSourcePlugin(plugin, options);
			const failing = wrapPlugin(source, { setup: async (context) => {
			...
			if (!(failure instanceof require_plugin_identifier.PluginSetupError) || failure.cause !== sentinel) throw new Error("Forced setup failure did not surface as PluginSetupError.");
			if (host.has(source.ref)) throw new Error("Failed setup left the Plugin API installed.");
		});
...
async function assertLifecycleOrder(plugin, options = {}) {
	return executeAssertion("lifecycle-order", "Lifecycle callbacks run in setup, initialization, image, clear, and disposal order.", async () => {
		...
		if (observed.join("|") !== [
			...
			"image-cleared",
			"dispose"
		].join("|")) throw new Error(`Unexpected lifecycle order: ${observed.join(" -> ")}.`);
	});
}
async function assertNoLeakedRegistrations(plugin, options = {}) {
	return executeAssertion("registration-cleanup", "Host disposal releases Plugin-owned registrations and Disposables.", async () => {
		...
		await useFixture(options, async ({ host }) => {
			const source = createSourcePlugin(plugin, options);
			await host.install(wrapPlugin(source, { setup: async (context) => {
				context.disposables.add({ dispose: () => {
					probeDisposed = true;
				} });
				return source.setup(context);
...
async function assertStateRoundTrip(plugin, options = {}) {
	return executeAssertion("state-round-trip", "Captured Plugin state survives mutation and restoration without semantic drift.", async () => {
		...
		if (adapter === void 0) return unavailable("No state round-trip adapter was supplied.");
		if (adapter === "not-applicable") return notApplicable("The Plugin declares no persistent state.");
		await useFixture(options, async ({ host }) => {
			const api = await host.install(createSourcePlugin(plugin, options));
			const protectedState = cloneStateValue(await adapter.capture(api));
			const before = stateFingerprint(protectedState);
			await adapter.mutate(api);
			if (stateFingerprint(await adapter.capture(api)) === before) throw new Error("State mutation did not change the captured fixture.");
			await adapter.restore(api, protectedState);
			if (stateFingerprint(await adapter.capture(api)) !== before) throw new Error("Restored Plugin state differs from the captured fixture.");
		});
	});
}
async function assertMissingCapabilityFailure(plugin, options = {}) {
	return executeAssertion("missing-capability", "A missing required Capability fails before Plugin setup starts.", async () => {
		const missingToken = require_core_capabilities.createCapabilityToken("testing:missing-capability", "1.0.0");
		await useFixture(options, async ({ host }) => {
			var _a;
			const source = createSourcePlugin(plugin, options);
			let setupCalls = 0;
			const wrapped = wrapPlugin(source, {
				manifest: {
					...source.manifest,
					requires: [...(_a = source.manifest.requires) !== null && _a !== void 0 ? _a : [], {
						token: missingToken,
						range: "^1.0.0"
					}]
				},
			...
			if (!(failure instanceof require_plugin_identifier.CapabilityMissingError) || failure.capabilityId !== missingToken.id) throw new Error("Missing Capability did not produce the typed failure.");
			if (setupCalls !== 0) throw new Error("Plugin setup ran before required Capability validation.");
		});
	});
}
async function assertOptionalCapabilityFallback(plugin, options = {}) {
	return executeAssertion("optional-capability-fallback", "An unavailable optional Capability resolves to null without blocking setup.", async () => {
		const optionalToken = require_core_capabilities.createCapabilityToken("testing:optional-capability", "1.0.0");
		await useFixture(options, async ({ host }) => {
			var _a;
			const source = createSourcePlugin(plugin, options);
			...
			const manifest = {
				...source.manifest,
				optional: [...(_a = source.manifest.optional) !== null && _a !== void 0 ? _a : [], {
					token: optionalToken,
					range: "^1.0.0"
				}]
			};
			await host.install(wrapPlugin(source, {
				manifest,
				setup: async (context) => {
					observedFallback = context.capabilities.optional(optionalToken) === null;
					return source.setup(context);
				}
			}));
			if (!observedFallback) throw new Error("Optional Capability did not resolve to null.");
		});
	});
}
...
async function assertPersistentKindCodecCoverage(plugin, options = {}) {
	return executeAssertion("persistent-codec-coverage", "Every persistent object Kind provides a complete, versioned Codec.", async () => {
		...
		if (inspection === void 0) return unavailable("No persistent Kind inspection was supplied.");
		if (inspection === "not-applicable") return notApplicable("The Plugin registers no persistent object Kinds.");
		return useFixture(options, async ({ host, providers }) => {
			const api = await host.install(createSourcePlugin(plugin, options));
			const persistent = (await inspection.inspect(api, providers)).filter((definition) => definition.persistence.mode === "persistent");
			if (persistent.length === 0) return notApplicable("The Plugin registers no persistent object Kinds.");
			for (const definition of persistent) {
				...
				if (codec.type.trim().length === 0 || !require_core_capabilities.isValidSemVer(codec.version) || typeof codec.serialize !== "function" || typeof codec.validate !== "function" || typeof codec.deserialize !== "function") throw new Error(`Persistent Kind "${definition.id}" has an incomplete Codec.`);
			}
		});
	});
}
...
async function runPluginConformance(plugin, options) {
	...
	if (options.profile !== "3.0") throw new RangeError("Unsupported Plugin conformance profile.");
	const assertions = [];
	...
	const failed = assertions.some((assertion) => assertion.status === "FAIL" || assertion.required && assertion.status === "NOT_AVAILABLE");
	return Object.freeze({
		schemaVersion: 1,
		profile: "3.0",
		packageVersion: PACKAGE_VERSION,
		coreApiVersion: require_core_capabilities.CORE_API_VERSION,
