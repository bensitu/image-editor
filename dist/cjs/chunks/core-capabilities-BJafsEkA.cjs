...
function createCapabilityToken(id, version) {
	if (!require_plugin_identifier.isRuntimeIdentifier(id)) throw new require_plugin_identifier.InvalidPluginDefinitionError("CapabilityToken id must use namespace:kebab-case and be at most 128 characters.");
	if (!isValidSemVer(version)) throw new require_plugin_identifier.InvalidCapabilityVersionError(id, version, "version");
...
function assertCapabilityRequirement(requirement) {
	...
	if (!isCapabilityToken(token)) throw new require_plugin_identifier.InvalidCapabilityVersionError("unknown", (_a = requirement === null || requirement === void 0 ? void 0 : requirement.range) !== null && _a !== void 0 ? _a : "", "range");
	if (!isValidSemVerRange(requirement.range)) throw new require_plugin_identifier.InvalidCapabilityVersionError(token.id, requirement.range, "range");
}
...
function definePluginRef(id, apiVersion) {
	require_plugin_identifier.assertPluginIdentifier(id, "PluginRef id");
	if (apiVersion.length > 64 || !isValidSemVer(apiVersion)) throw new require_plugin_identifier.InvalidPluginDefinitionError(`PluginRef "${id}" has invalid API SemVer "${apiVersion}".`, id);
	return Object.freeze({
		id,
		apiVersion,
		[pluginRefBrand]: true
...
function assertArrayLimit(value, fieldName, maximum) {
	...
	if (!Array.isArray(value) || value.length > maximum) throw new require_plugin_identifier.PluginManifestError(`${fieldName} must be an array containing at most ${maximum} entries.`);
	return value;
}
function freezeRequirements(pluginId, value, fieldName) {
	...
	return Object.freeze(requirements.map((requirement) => {
		try {
			...
			throw new require_plugin_identifier.PluginManifestError(`Plugin "${pluginId}" has an invalid capability requirement in ${fieldName}.`, {
				pluginId,
				cause
			});
		}
...
function freezePluginDependencies(pluginId, value) {
	...
	const validated = dependencies.map((dependency) => {
		if (!isPluginRef(dependency)) throw new require_plugin_identifier.PluginManifestError(`Plugin "${pluginId}" requiresPlugins entries must use definePluginRef().`, { pluginId });
		if (dependency.id === pluginId) throw new require_plugin_identifier.PluginManifestError(`Plugin "${pluginId}" cannot depend on itself.`, { pluginId });
		if (dependencyIds.has(dependency.id)) throw new require_plugin_identifier.PluginManifestError(`Plugin "${pluginId}" declares dependency "${dependency.id}" more than once.`, { pluginId });
...
function freezePermissions(pluginId, value) {
	...
	const validated = permissions.map((permission) => {
		if (typeof permission !== "string" || !isPluginPermission(permission)) throw new require_plugin_identifier.PluginManifestError(`Plugin "${pluginId}" declares unsupported permission "${String(permission)}".`, { pluginId });
		const typedPermission = permission;
		if (permissionSet.has(typedPermission)) throw new require_plugin_identifier.PluginManifestError(`Plugin "${pluginId}" declares permission "${typedPermission}" more than once.`, { pluginId });
...
function validatePluginManifest(ref, manifest) {
	if (typeof manifest !== "object" || manifest === null) throw new require_plugin_identifier.PluginManifestError(`Plugin "${ref.id}" must define a manifest.`, { pluginId: ref.id });
	const manifestId = require_plugin_identifier.assertPluginIdentifier(manifest.id, "Plugin manifest id");
	if (manifestId !== ref.id) throw new require_plugin_identifier.PluginIdentityConflictError(ref.id, manifestId);
	if (typeof manifest.version !== "string" || manifest.version.length > MAX_VERSION_LENGTH || !isValidSemVer(manifest.version)) throw new require_plugin_identifier.PluginManifestError(`Plugin "${ref.id}" has invalid implementation SemVer "${String(manifest.version)}".`, { pluginId: ref.id });
	if (typeof manifest.apiVersion !== "string" || manifest.apiVersion.length > MAX_VERSION_LENGTH || !isValidSemVer(manifest.apiVersion)) throw new require_plugin_identifier.PluginManifestError(`Plugin "${ref.id}" has invalid API SemVer "${String(manifest.apiVersion)}".`, { pluginId: ref.id });
	if (manifest.apiVersion !== ref.apiVersion) throw new require_plugin_identifier.PluginApiVersionError(ref.id, ref.apiVersion, manifest.apiVersion);
	if (typeof manifest.engine !== "string" || manifest.engine.length > MAX_VERSION_LENGTH || !isValidSemVerRange(manifest.engine)) throw new require_plugin_identifier.InvalidPluginDefinitionError(`Plugin "${ref.id}" has invalid engine SemVer range "${String(manifest.engine)}".`, ref.id);
	if (!satisfiesSemVer("3.0.0", manifest.engine)) throw new require_plugin_identifier.PluginEngineVersionError(ref.id, manifest.engine, CORE_API_VERSION);
	...
	for (const requirement of [...requires !== null && requires !== void 0 ? requires : [], ...optional !== null && optional !== void 0 ? optional : []]) {
		if (capabilityIds.has(requirement.token.id)) throw new require_plugin_identifier.PluginManifestError(`Plugin "${ref.id}" declares capability "${requirement.token.id}" more than once.`, { pluginId: ref.id });
...
function acquirePluginDefinitionLease(definition, host, pluginId) {
	...
	if (boundHost && boundHost !== host) throw new require_plugin_identifier.PluginDefinitionAlreadyBoundError(pluginId, boundHost.state);
...
function assertPluginPlanItem(value, key) {
	...
	if (typeof value !== "object" || value === null || !("ref" in value) || !("setup" in value)) throw new require_plugin_identifier.InvalidPluginDefinitionError(`Plugin Plan entry "${key}" must be a Plugin or nested Plugin Plan.`);
}
function composePlugins(definitions) {
	if (typeof definitions !== "object" || definitions === null) throw new require_plugin_identifier.InvalidPluginDefinitionError("Plugin Plan definitions must be an object.");
	const entries = Object.entries(definitions);
	if (entries.length === 0) throw new require_plugin_identifier.InvalidPluginDefinitionError("Plugin Plan must contain at least one Plugin.");
	const plugins = [];
...
//#endregion
Object.defineProperty(exports, 'BASE_IMAGE_INFO_CAPABILITY', {
  enumerable: true,
  get: function () {
    return BASE_IMAGE_INFO_CAPABILITY;
  }
});
Object.defineProperty(exports, 'BASE_IMAGE_READ_CAPABILITY', {
  enumerable: true,
  get: function () {
    return BASE_IMAGE_READ_CAPABILITY;
  }
});
Object.defineProperty(exports, 'CANVAS_READ_CAPABILITY', {
  enumerable: true,
  get: function () {
    return CANVAS_READ_CAPABILITY;
  }
});
Object.defineProperty(exports, 'CANVAS_RESIZE_CAPABILITY', {
  enumerable: true,
  get: function () {
    return CANVAS_RESIZE_CAPABILITY;
  }
});
Object.defineProperty(exports, 'CORE_API_VERSION', {
  enumerable: true,
  get: function () {
    return CORE_API_VERSION;
  }
});
Object.defineProperty(exports, 'CORE_DIAGNOSTICS_CAPABILITY', {
  enumerable: true,
  get: function () {
    return CORE_DIAGNOSTICS_CAPABILITY;
  }
});
Object.defineProperty(exports, 'CORE_PRESENTATION_CAPABILITY', {
  enumerable: true,
  get: function () {
    return CORE_PRESENTATION_CAPABILITY;
  }
});
Object.defineProperty(exports, 'CORE_STATUS_CAPABILITY', {
  enumerable: true,
  get: function () {
    return CORE_STATUS_CAPABILITY;
  }
});
Object.defineProperty(exports, 'DOCUMENT_MUTATION_CAPABILITY', {
  enumerable: true,
  get: function () {
    return DOCUMENT_MUTATION_CAPABILITY;
  }
});
Object.defineProperty(exports, 'EXPORT_CONTRIBUTION_CAPABILITY', {
  enumerable: true,
  get: function () {
    return EXPORT_CONTRIBUTION_CAPABILITY;
  }
});
Object.defineProperty(exports, 'FABRIC_RUNTIME_CAPABILITY', {
  enumerable: true,
  get: function () {
    return FABRIC_RUNTIME_CAPABILITY;
  }
});
Object.defineProperty(exports, 'GEOMETRY_MUTATION_CAPABILITY', {
  enumerable: true,
  get: function () {
    return GEOMETRY_MUTATION_CAPABILITY;
  }
});
Object.defineProperty(exports, 'IMAGE_RESOURCE_POLICY_CAPABILITY', {
  enumerable: true,
  get: function () {
    return IMAGE_RESOURCE_POLICY_CAPABILITY;
  }
});
Object.defineProperty(exports, 'MEMENTO_HISTORY_CAPABILITY', {
  enumerable: true,
  get: function () {
    return MEMENTO_HISTORY_CAPABILITY;
  }
});
Object.defineProperty(exports, 'RASTER_MUTATION_CAPABILITY', {
  enumerable: true,
  get: function () {
    return RASTER_MUTATION_CAPABILITY;
  }
});
Object.defineProperty(exports, 'RENDER_REQUEST_CAPABILITY', {
  enumerable: true,
  get: function () {
    return RENDER_REQUEST_CAPABILITY;
  }
});
Object.defineProperty(exports, 'SNAPSHOT_REGISTRATION_CAPABILITY', {
  enumerable: true,
  get: function () {
    return SNAPSHOT_REGISTRATION_CAPABILITY;
  }
});
Object.defineProperty(exports, 'acquirePluginDefinitionLease', {
  enumerable: true,
  get: function () {
    return acquirePluginDefinitionLease;
  }
});
Object.defineProperty(exports, 'assertCapabilityRequirement', {
  enumerable: true,
  get: function () {
    return assertCapabilityRequirement;
  }
});
...
Object.defineProperty(exports, 'coreOperationIds', {
  enumerable: true,
  get: function () {
    return coreOperationIds;
  }
});
Object.defineProperty(exports, 'createCapabilityToken', {
  enumerable: true,
  get: function () {
    return createCapabilityToken;
  }
});
Object.defineProperty(exports, 'createDisposable', {
  enumerable: true,
  get: function () {
    return createDisposable;
  }
});
Object.defineProperty(exports, 'createNoopDisposable', {
  enumerable: true,
  get: function () {
    return createNoopDisposable;
  }
});
Object.defineProperty(exports, 'cropOperationIds', {
  enumerable: true,
  get: function () {
    return cropOperationIds;
  }
});
Object.defineProperty(exports, 'definePluginRef', {
  enumerable: true,
  get: function () {
    return definePluginRef;
  }
});
Object.defineProperty(exports, 'disposeInReverse', {
  enumerable: true,
  get: function () {
    return disposeInReverse;
  }
});
Object.defineProperty(exports, 'disposeInReverseSync', {
  enumerable: true,
  get: function () {
    return disposeInReverseSync;
  }
});
Object.defineProperty(exports, 'historyOperationIds', {
  enumerable: true,
  get: function () {
    return historyOperationIds;
  }
});
Object.defineProperty(exports, 'isCanonicalPluginDefinition', {
  enumerable: true,
  get: function () {
    return isCanonicalPluginDefinition;
  }
});
Object.defineProperty(exports, 'isCapabilityToken', {
  enumerable: true,
  get: function () {
    return isCapabilityToken;
  }
});
Object.defineProperty(exports, 'isPluginPermission', {
  enumerable: true,
  get: function () {
    return isPluginPermission;
  }
});
Object.defineProperty(exports, 'isPluginPlan', {
  enumerable: true,
  get: function () {
    return isPluginPlan;
  }
});
Object.defineProperty(exports, 'isPluginRef', {
  enumerable: true,
  get: function () {
    return isPluginRef;
  }
...
Object.defineProperty(exports, 'isValidSemVer', {
  enumerable: true,
  get: function () {
    return isValidSemVer;
  }
});
Object.defineProperty(exports, 'markCanonicalPluginDefinition', {
  enumerable: true,
  get: function () {
    return markCanonicalPluginDefinition;
  }
});
Object.defineProperty(exports, 'mosaicOperationIds', {
  enumerable: true,
  get: function () {
    return mosaicOperationIds;
  }
});
Object.defineProperty(exports, 'observePromise', {
  enumerable: true,
  get: function () {
    return observePromise;
  }
});
Object.defineProperty(exports, 'releasePluginDefinitionLease', {
  enumerable: true,
  get: function () {
    return releasePluginDefinitionLease;
  }
});
Object.defineProperty(exports, 'reportErrorSafely', {
  enumerable: true,
  get: function () {
    return reportErrorSafely;
  }
});
Object.defineProperty(exports, 'reportWarningSafely', {
  enumerable: true,
  get: function () {
    return reportWarningSafely;
  }
});
Object.defineProperty(exports, 'resolvePluginPlanApis', {
  enumerable: true,
  get: function () {
    return resolvePluginPlanApis;
  }
});
Object.defineProperty(exports, 'satisfiesSemVer', {
  enumerable: true,
  get: function () {
    return satisfiesSemVer;
  }
});
Object.defineProperty(exports, 'validatePluginManifest', {
  enumerable: true,
  get: function () {
    return validatePluginManifest;
  }
});
//# sourceMappingURL=core-capabilities-BJafsEkA.cjs.map