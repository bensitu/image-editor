...
function validateText(value, label = "Text") {
	if (typeof value !== "string" || value.length > MAX_TEXT_LENGTH) throw new require_annotation.AnnotationValidationError(`${label} must be a string of at most ${MAX_TEXT_LENGTH} characters.`);
	return value;
}
function validateFontField(value, label) {
	if (typeof value !== "string" || value.length === 0 || value.trim() !== value || value.length > MAX_FONT_FIELD_LENGTH || [...value].some((character) => character.charCodeAt(0) < 32)) throw new require_annotation.AnnotationValidationError(`${label} is invalid.`);
	return value;
}
function validateFontWeight(value) {
	...
	throw new require_annotation.AnnotationValidationError("Text font weight is invalid.");
}
...
function validateAlignment(value) {
	...
	throw new require_annotation.AnnotationValidationError("Text alignment is invalid.");
}
function validateFallbacks(value) {
	if (!Array.isArray(value) || value.length > 8) throw new require_annotation.AnnotationValidationError("Text font fallbacks are invalid.");
...
function resolveTextConfiguration(value = {}, base = defaultConfiguration) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError("Text configuration must be a plain object.");
	const allowed = new Set(Object.keys(defaultConfiguration));
	if (Object.keys(value).some((key) => !allowed.has(key))) throw new require_annotation.AnnotationValidationError("Text configuration contains unknown keys.");
	const merged = {
		...base,
		...value
	};
	if (merged.reflectionBehavior !== "preserve-readable" && merged.reflectionBehavior !== "mirror") throw new require_annotation.AnnotationValidationError("Text reflection behavior is invalid.");
...
function normalizeFeatureUpdate(value) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError("Text update must be a plain object.");
	...
	if (Object.keys(value).some((key) => !allowed.has(key))) throw new require_annotation.AnnotationValidationError("Text update contains unknown keys.");
	return Object.freeze({
...
function isSerializedText(value) {
	...
	try {
		if (!require_safe_fabric_serialization.isSafeSerializedFabricObject(value, { rootTypes: ["textbox"] })) return false;
...
var TextAnnotationController = class {
		...
		return Object.freeze({
			...
			codec: {
				...
				deserialize: async (value, context) => {
					...
					if (!(object instanceof context.fabric.Textbox)) throw new require_annotation.AnnotationValidationError("Serialized Text Annotation did not restore a Textbox.");
					return object;
				}
			},
			stateCodec: {
				...
				serialize: (object, context) => {
					const text = object;
					return Object.freeze({
						geometry: require_overlay.captureOverlayStateBounds(text, context),
						data: Object.freeze({
							version: 1,
							text: text.text,
				...
				validate: (value) => require_overlay.isOverlayStateBoundsGeometry(value.geometry) && isTextStateData(value.data),
				deserialize: (value, context) => {
					if (!require_overlay.isOverlayStateBoundsGeometry(value.geometry) || !isTextStateData(value.data)) throw new require_annotation.AnnotationValidationError("Serialized Text Annotation State data is malformed.");
					...
					require_overlay.restoreOverlayStateBounds(object, value.geometry, context, this.host.fabric);
					return object;
				}
			},
	...
	async create(options = {}) {
		...
		if (!isPlainRecord(options)) throw new require_annotation.AnnotationValidationError("Text creation options must be a plain object.");
	...
	async beginEditing(id) {
		...
		this.assertImageLoaded();
		if (this.session) throw new require_annotation.AnnotationValidationError("A Text editing session is already active.");
		const descriptor = this.annotations.get(id);
		const source = this.authoring.getObject(id, TEXT_ANNOTATION_KIND);
		if (!descriptor || !source) throw new require_annotation.AnnotationValidationError(`Text Annotation "${id}" was not found.`);
		if (descriptor.locked) throw new require_annotation.AnnotationValidationError("Locked Text cannot enter editing.");
	...
	update(id, patch) {
		...
		if (!isPlainRecord(patch)) return Promise.reject(new require_annotation.AnnotationValidationError("Text update must be an object."));
		if (((_a = this.session) === null || _a === void 0 ? void 0 : _a.annotationId) === id) {
			const shared = sharedUpdate(patch);
			if (Object.keys(shared).length > 0) return Promise.reject(new require_annotation.AnnotationValidationError("Commit or cancel Text editing before updating shared Annotation fields."));
	...
	subscribe(listener) {
		...
		if (typeof listener !== "function") throw new require_annotation.AnnotationValidationError("Text status listener must be a function.");
	...
	closeSession() {
		...
		require_core_capabilities.observePromise(Promise.resolve(session.visibility.dispose()), (error) => {
	...
	assertImageLoaded() {
		if (!this.host.isImageLoaded()) throw new require_annotation.AnnotationValidationError("Text Annotation requires a loaded image.");
	}
...
const textAnnotationPluginRef = require_core_capabilities.definePluginRef("annotation:text", "1.0.0");
function textAnnotationPlugin(options = {}) {
	...
	return require_sdk.definePlugin({
		...
		manifest: {
			...
			requires: [
				{
					token: require_annotation.ANNOTATION_CAPABILITY,
					range: "^1.0.0"
				},
				{
		...
		setupMode: "sync",
		setup(context) {
			const annotations = context.capabilities.require(require_annotation.ANNOTATION_CAPABILITY);
			...
			context.disposables.add(context.tools.register({
				...
				canRunOperation: (operationId) => operationId.startsWith("annotation-text:") || operationId.startsWith("annotation:") || operationId.endsWith(":enter") || operationId === require_core_capabilities.coreOperationIds.loadImage || operationId === require_core_capabilities.coreOperationIds.commitLoadImage || operationId === require_core_capabilities.coreOperationIds.loadState || operationId === require_core_capabilities.coreOperationIds.export
			}));
			context.disposables.add(annotations.subscribe((status) => {
				...
				if (context.tools.getActiveToolId() === TEXT_TOOL_ID) require_core_capabilities.observePromise(context.tools.exit("operation"), (error) => {
...
//#endregion
Object.defineProperty(exports, 'textAnnotationPlugin', {
  enumerable: true,
  get: function () {
    return textAnnotationPlugin;
  }
});
Object.defineProperty(exports, 'textAnnotationPluginRef', {
  enumerable: true,
  get: function () {
    return textAnnotationPluginRef;
  }
});
//# sourceMappingURL=annotation-text-D1zrOggU.cjs.map