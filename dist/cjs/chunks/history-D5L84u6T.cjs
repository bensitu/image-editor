...
var HistoryPluginController = class {
	...
	commit(record) {
		...
		if (record.operationId === require_core_capabilities.coreOperationIds.loadImage || record.operationId === require_core_capabilities.coreOperationIds.commitLoadImage || record.operationId === require_core_capabilities.coreOperationIds.loadState) {
	...
	undo() {
		...
		return this.operations.run(require_core_capabilities.historyOperationIds.undo, async () => {
	...
	redo() {
		...
		return this.operations.run(require_core_capabilities.historyOperationIds.redo, async () => {
	...
	return require_sdk.definePlugin({
		...
		setup(context) {
			...
			context.operations.register({
				id: require_core_capabilities.historyOperationIds.undo,
				mode: "mutation",
				conflictDomains: require_internal_operation_conflict_domains.DOCUMENT_WIDE_MUTATION_CONFLICT_DOMAINS,
				reentrancy: "queue"
			});
			context.operations.register({
				id: require_core_capabilities.historyOperationIds.redo,
				mode: "mutation",
				conflictDomains: require_internal_operation_conflict_domains.DOCUMENT_WIDE_MUTATION_CONFLICT_DOMAINS,
				reentrancy: "queue"
			});
...
//# sourceMappingURL=history-D5L84u6T.cjs.map