...
function shapeKind(value) {
	...
	throw new require_annotation.AnnotationValidationError("Shape kind is invalid.");
}
function point(value, label) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError(`${label} is invalid.`);
...
function normalizeShapeGeometry(value) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError("Shape geometry must be a plain object.");
	...
	if (Math.hypot(end.x - start.x, end.y - start.y) < MIN_GEOMETRY_SIZE) throw new require_annotation.AnnotationValidationError("Shape line and arrow endpoints must be distinct.");
	return Object.freeze({
		kind,
		start,
		end
...
function resolveShapeConfiguration(value = {}, base = defaultConfiguration) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError("Shape configuration must be a plain object.");
	const allowed = new Set(Object.keys(defaultConfiguration));
	if (Object.keys(value).some((key) => !allowed.has(key))) throw new require_annotation.AnnotationValidationError("Shape configuration contains unknown keys.");
	const merged = {
		...base,
		...value
	};
...
function normalizeFeatureUpdate(value) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError("Shape update must be a plain object.");
	...
	if (Object.keys(value).some((key) => !allowed.has(key))) throw new require_annotation.AnnotationValidationError("Shape update contains unknown keys.");
	return Object.freeze({
...
function isShapeStateGeometry(value) {
	...
	if (value.kind === "rect") return require_overlay.isOverlayStateBoundsGeometry(value.bounds);
...
function isSerializedShape(value) {
	...
	try {
		...
		if (value.version !== 1 || !isPlainRecord(serializedObject) || !require_safe_fabric_serialization.isSafeSerializedFabricObject(serializedObject, { rootTypes: [
			"rect",
			"line",
			"polyline",
			"path"
...
var ShapeAnnotationController = class {
			...
			stateCodec: {
				...
				serialize: (object, context) => {
					...
					const stateGeometry = geometry.kind === "rect" ? Object.freeze({
						...
						kind: geometry.kind,
						start: Object.freeze(context.toImageNormalized(require_overlay.objectPointToCanvas(object, geometry.start))),
						end: Object.freeze(context.toImageNormalized(require_overlay.objectPointToCanvas(object, geometry.end)))
					});
				...
				deserialize: (value, context) => {
					if (!isShapeStateGeometry(value.geometry) || !isShapeStateData(value.data)) throw new require_annotation.AnnotationValidationError("Serialized Shape Annotation State data is malformed.");
					...
					if (value.geometry.kind === "rect") {
						...
						require_overlay.restoreOverlayStateBounds(object, value.geometry.bounds, context, this.host.fabric);
						return object;
					}
					const geometry = {
	...
	enter(options) {
		...
		if (this.session) throw new require_annotation.AnnotationValidationError("A Shape session is already active.");
		if (!isPlainRecord(options)) throw new require_annotation.AnnotationValidationError("Shape session options must be a plain object.");
	...
	updatePreview(geometryInput) {
		...
		if (geometry.kind !== session.options.kind) throw new require_annotation.AnnotationValidationError("Shape preview kind does not match the session.");
	...
	async commit() {
		...
		if (!session.geometry) throw new require_annotation.AnnotationValidationError("Shape commit requires preview geometry.");
		const definition = {
			...session.options,
	...
	createDefinition(definition, operationId) {
		...
		this.assertImageLoaded();
		if (!isPlainRecord(definition)) return Promise.reject(new require_annotation.AnnotationValidationError("Shape definition must be a plain object."));
	...
	update(id, patch) {
		...
		if (!isPlainRecord(patch)) return Promise.reject(new require_annotation.AnnotationValidationError("Shape update must be an object."));
	...
	requireSession(operation) {
		...
		if (!this.session) throw new require_annotation.AnnotationValidationError(`Cannot ${operation} without a Shape session.`);
		return this.session;
	}
	assertImageLoaded() {
		if (!this.host.isImageLoaded()) throw new require_annotation.AnnotationValidationError("Shape Annotation requires a loaded image.");
	}
...
const shapeAnnotationPluginRef = require_core_capabilities.definePluginRef("annotation:shape", "1.0.0");
	...
	return require_sdk.definePlugin({
		...
		setup(context) {
			...
			context.disposables.add(context.tools.register({
				...
				canRunOperation: (operationId) => operationId.startsWith("annotation-shape:") || operationId.startsWith("annotation:") || operationId.endsWith(":enter") || operationId === require_core_capabilities.coreOperationIds.loadImage || operationId === require_core_capabilities.coreOperationIds.commitLoadImage || operationId === require_core_capabilities.coreOperationIds.loadState || operationId === require_core_capabilities.coreOperationIds.export
			}));
...
//#endregion
Object.defineProperty(exports, 'shapeAnnotationPlugin', {
  enumerable: true,
  get: function () {
    return shapeAnnotationPlugin;
  }
});
Object.defineProperty(exports, 'shapeAnnotationPluginRef', {
  enumerable: true,
  get: function () {
    return shapeAnnotationPluginRef;
  }
});
//# sourceMappingURL=annotation-shape-D90O9lkf.cjs.map