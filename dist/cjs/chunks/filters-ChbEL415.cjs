const require_core_capabilities = require('./core-capabilities-3eNZsGJV.cjs');
...
function validateKeys(value, allowed, path) {
	for (const key of Reflect.ownKeys(value)) {
		...
		if (require_safe_object_key.isUnsafeObjectKey(key)) throw new FilterDefinitionError(`Filter definition contains dangerous key "${key}".`, path);
...
function normalizeFilterBakeOptions(options, sourceMimeType) {
	...
	for (const key of Object.keys(record)) {
		if (require_safe_object_key.isUnsafeObjectKey(key)) throw new FilterBakeValidationError(`Filter bake options contain dangerous key "${key}".`);
...
function validateStateKeys(value) {
	for (const key of Object.keys(value)) {
		if (require_safe_object_key.isUnsafeObjectKey(key)) return `Filters state contains dangerous key "${key}".`;
...
var FiltersController = class {
	...
	normalizeConfigurationPatch(patch) {
		...
		for (const key of Object.keys(patch)) {
			if (require_safe_object_key.isUnsafeObjectKey(key)) throw new TypeError(`[ImageEditor] Filters configuration contains dangerous key "${key}".`);
...
//# sourceMappingURL=filters-ChbEL415.cjs.map