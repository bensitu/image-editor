...
var PluginManager = class {
	...
	createContexts(plugin, scope, required, optional, stack) {
		...
		const ensurePluginNow = async (dependency) => {
			...
			for (const installedPluginId of newlyInstalled) scope.addRollback(require_core_capabilities.createDisposable(() => this.rollbackInstalledPlugin(installedPluginId)));
			return outcome.api;
		};
...
Object.defineProperty(exports, 'aliasPluginDefinitionIdentity', {
  enumerable: true,
  get: function () {
    return aliasPluginDefinitionIdentity;
  }
});
//# sourceMappingURL=plugin-manager-BdEFq773.cjs.map