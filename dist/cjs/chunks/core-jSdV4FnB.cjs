...
const require_plugin_manager = require('./plugin-manager-DZffsIRA.cjs');
...
var ImageEditorCore = class {
	...
	async performImageLoad(source, options = {}) {
		...
		try {
			await this.plugins.runOperationForHost(require_core_capabilities.coreOperationIds.loadImage, source, async (loadSource, operationContext) => {
				...
				try {
					...
					await this.documentMutations.run({
						...
						kind: "raster",
						operationId: require_core_capabilities.coreOperationIds.commitLoadImage,
	...
	async loadFromState(input, options = {}) {
		...
		try {
			...
			await this.documentMutations.run({
				...
				kind: "compound",
				operationId: require_core_capabilities.coreOperationIds.loadState,
				conflictDomains: require_internal_operation_conflict_domains.DOCUMENT_WIDE_MUTATION_CONFLICT_DOMAINS,
				...options.signal ? { signal: options.signal } : {},
	...
	async relayout(options = {}) {
		...
		await this.geometry.run({
			...
			kind: "transform",
			operationId: require_core_capabilities.coreOperationIds.relayout,
	...
	createPluginManager() {
		...
		manager.registerHostOperation({
			id: require_core_capabilities.coreOperationIds.loadImage,
			mode: "busy",
			conflictDomains: ["image-decode"],
			reentrancy: "replace"
		});
		manager.registerHostOperation({
			id: require_core_capabilities.coreOperationIds.commitLoadImage,
			mode: "mutation",
			conflictDomains: require_internal_operation_conflict_domains.DOCUMENT_WIDE_MUTATION_CONFLICT_DOMAINS,
			reentrancy: "queue"
		});
		manager.registerHostOperation({
			id: require_core_capabilities.coreOperationIds.loadState,
			mode: "mutation",
			conflictDomains: require_internal_operation_conflict_domains.DOCUMENT_WIDE_MUTATION_CONFLICT_DOMAINS,
			reentrancy: "reject"
		});
		manager.registerHostOperation({
			id: require_core_capabilities.coreOperationIds.export,
			mode: "read",
			conflictDomains: [
				"document",
				"base-image",
		...
		manager.registerHostOperation({
			id: require_core_capabilities.coreOperationIds.relayout,
			mode: "mutation",
			conflictDomains: [
				"document",
				"base-image",
	...
	async runExport(options) {
		...
		const resolved = resolveExportOptions(options, this.options.exportDefaults);
		const operation = this.plugins.beginOperationForHost(require_core_capabilities.coreOperationIds.export);
		try {
	...
	async emitDocumentCommitted(descriptor) {
		...
		if (descriptor.operationId === require_core_capabilities.coreOperationIds.commitLoadImage && isCoreImageInfo(descriptor.result)) {
			await ((_b = this.plugins) === null || _b === void 0 ? void 0 : _b.emitCommitted("image:loaded", descriptor.result));
			return;
		}
		if (descriptor.operationId === require_core_capabilities.coreOperationIds.loadState) {
...
//# sourceMappingURL=core-jSdV4FnB.cjs.map