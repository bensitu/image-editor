...
function recordPluginCleanupError(errors, errorSink, error) {
	errors.push(error);
	require_core_capabilities.reportErrorSafely(errorSink, error);
}
function releasePluginCleanupRecord(record, owner) {
	require_core_capabilities.releasePluginDefinitionLease(record.plugin, owner);
}
...
var PluginManager = class {
	...
	installBatchSync(plugins) {
		...
		try {
			...
			try {
				...
				const cleanupErrors = [...cause instanceof require_plugin_identifier.PluginSetupError ? cause.cleanupErrors : [], ...this.rollbackPendingBatchSync(pendingRecords)];
				throw new require_plugin_identifier.PluginBatchInstallError(cause, cleanupErrors);
			}
			return Object.freeze({
	...
	initializeSync() {
		...
		try {
			for (const pluginId of this.installationOrder) {
				...
				if (require_core_capabilities.isPromiseLike(result)) throw new require_plugin_identifier.PluginLifecycleError(pluginId, "init", /* @__PURE__ */ new Error("Synchronous plugin onInit returned a Promise."));
			}
			this.hostState = "initialized";
		} catch (error) {
			this.hostState = "disposing";
	...
	normalizePluginDefinition(plugin) {
		...
		return require_core_capabilities.markCanonicalPluginDefinition(normalized, plugin);
	}
...
//# sourceMappingURL=plugin-manager-mBARysVw.cjs.map