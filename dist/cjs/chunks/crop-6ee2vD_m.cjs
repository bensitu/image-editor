...
async function applyCropOverlayPolicy(overlay, canvas, parent, policy, candidates, mutationId) {
	...
	await overlay.mutate({
		...
		operationId: require_core_capabilities.cropOperationIds.apply,
		action: "delete",
		objectIds: removeIds,
		parent,
...
var CropController = class {
	...
	async apply(options) {
		...
		this.closeSession(true);
		const mutationId = `${require_core_capabilities.cropOperationIds.apply}:${++this.mutationSequence}`;
		const resources = {
			replacement: null,
			replacedSource: null
		};
	...
	return require_sdk.definePlugin({
		...
		setup(context) {
			...
			for (const operationId of [
				require_core_capabilities.cropOperationIds.enter,
				require_core_capabilities.cropOperationIds.updateRect,
				require_core_capabilities.cropOperationIds.setAspectRatio,
				require_core_capabilities.cropOperationIds.setRotation,
				require_core_capabilities.cropOperationIds.cancel
			...
			context.disposables.add(context.operations.register({
				id: require_core_capabilities.cropOperationIds.apply,
				mode: "mutation",
			...
			context.disposables.add(context.tools.register({
				...
				canRunOperation: (operationId) => operationId.startsWith("crop:") || operationId === require_core_capabilities.mosaicOperationIds.enter || operationId === require_core_capabilities.coreOperationIds.loadImage || operationId === require_core_capabilities.coreOperationIds.commitLoadImage || operationId === require_core_capabilities.coreOperationIds.loadState || operationId === require_core_capabilities.coreOperationIds.export
			}));
			...
			return Object.freeze({
				...
				enter: (enterOptions) => runPreviewOperation(require_core_capabilities.cropOperationIds.enter, enterOptions !== null && enterOptions !== void 0 ? enterOptions : {}, async (crop, value) => {
					if (crop.isActive) {
						crop.enter(value);
						return;
					}
				...
				updateRect: (rect) => runPreviewOperation(require_core_capabilities.cropOperationIds.updateRect, rect, (crop, value) => crop.updateRect(value)),
				setAspectRatio: (ratio) => runPreviewOperation(require_core_capabilities.cropOperationIds.setAspectRatio, ratio, (crop, value) => crop.setAspectRatio(value)),
				setRotation: (degrees) => runPreviewOperation(require_core_capabilities.cropOperationIds.setRotation, degrees, (crop, value) => crop.setRotation(value)),
				...
				cancel: () => runPreviewOperation(require_core_capabilities.cropOperationIds.cancel, void 0, async (crop) => {
					crop.cancel();
...
//# sourceMappingURL=crop-6ee2vD_m.cjs.map