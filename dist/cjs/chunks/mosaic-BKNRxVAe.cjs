...
async function renderMosaicImage(host, source, cache, options, signal) {
	...
	if (cache.widthPx > policy.maxExportDimension || cache.heightPx > policy.maxExportDimension || !require_image_budget.isPixelAreaWithinBudget(cache.widthPx, cache.heightPx, pixelBudget)) throw new MosaicValidationError("Mosaic dimensions exceed the Core resource policy.");
	let dataUrl;
	try {
		...
		if (require_error.hasErrorName(error, "SecurityError")) throw new MosaicValidationError("Mosaic pixels cannot be exported because canvas access is blocked.");
		throw error;
	}
...
var MosaicController = class {
	...
	async commit(options) {
		...
		const mutationId = `${require_core_capabilities.mosaicOperationIds.commit}:${++this.mutationSequence}`;
		const resources = {
			cache: null,
			replacement: null,
			replacedSource: null
		...
		try {
			await this.geometry.run({
				id: mutationId,
				kind: "raster-replace",
				operationId: require_core_capabilities.mosaicOperationIds.commit,
				targetSize: {
	...
	assertCachePolicy(cache) {
		...
		const pixelBudget = Math.min(policy.maxInputPixels, policy.maxExportPixels);
		if (cache.widthPx > policy.maxExportDimension || cache.heightPx > policy.maxExportDimension || !require_image_budget.isPixelAreaWithinBudget(cache.widthPx, cache.heightPx, pixelBudget)) {
...
const mosaicPluginRef = require_core_capabilities.definePluginRef("plugin:mosaic", "1.0.0");
	...
	return require_sdk.definePlugin({
		...
		setup(context) {
			...
			for (const operationId of [
				require_core_capabilities.mosaicOperationIds.enter,
				require_core_capabilities.mosaicOperationIds.beginStroke,
				require_core_capabilities.mosaicOperationIds.appendStroke,
				require_core_capabilities.mosaicOperationIds.endStroke,
				require_core_capabilities.mosaicOperationIds.cancel,
				require_core_capabilities.mosaicOperationIds.configure
			...
			context.disposables.add(context.operations.register({
				id: require_core_capabilities.mosaicOperationIds.commit,
				mode: "mutation",
			...
			context.disposables.add(context.tools.register({
				...
				canRunOperation: (operationId) => operationId.startsWith("mosaic:") || operationId === require_core_capabilities.cropOperationIds.enter || operationId === require_core_capabilities.coreOperationIds.loadImage || operationId === require_core_capabilities.coreOperationIds.commitLoadImage || operationId === require_core_capabilities.coreOperationIds.loadState || operationId === require_core_capabilities.coreOperationIds.export
			}));
			...
			return Object.freeze({
				...
				enter: (enterOptions) => runPreviewOperation(require_core_capabilities.mosaicOperationIds.enter, enterOptions !== null && enterOptions !== void 0 ? enterOptions : {}, async (mosaic, value) => {
					if (mosaic.isActive) {
						mosaic.enter(value);
						return;
					}
				...
				beginStroke: (point) => runPreviewOperation(require_core_capabilities.mosaicOperationIds.beginStroke, point, (mosaic, value) => mosaic.beginStroke(value)),
				appendStroke: (point) => runPreviewOperation(require_core_capabilities.mosaicOperationIds.appendStroke, point, (mosaic, value) => mosaic.appendStroke(value)),
				endStroke: () => runPreviewOperation(require_core_capabilities.mosaicOperationIds.endStroke, void 0, (mosaic) => mosaic.endStroke()),
				...
				cancel: () => runPreviewOperation(require_core_capabilities.mosaicOperationIds.cancel, void 0, async (mosaic) => {
					mosaic.cancel();
					if (context.tools.getActiveToolId() === MOSAIC_TOOL_ID) await context.tools.exit("requested");
				}),
				configure: (patch) => runPreviewOperation(require_core_capabilities.mosaicOperationIds.configure, patch, (mosaic, value) => mosaic.configure(value)),
...
//#endregion
Object.defineProperty(exports, 'MosaicError', {
  enumerable: true,
  get: function () {
    return MosaicError;
  }
});
Object.defineProperty(exports, 'MosaicIntegrationError', {
  enumerable: true,
  get: function () {
    return MosaicIntegrationError;
  }
});
Object.defineProperty(exports, 'MosaicSessionError', {
  enumerable: true,
  get: function () {
    return MosaicSessionError;
  }
});
Object.defineProperty(exports, 'MosaicValidationError', {
  enumerable: true,
  get: function () {
    return MosaicValidationError;
  }
});
Object.defineProperty(exports, 'mosaicPlugin', {
  enumerable: true,
  get: function () {
    return mosaicPlugin;
  }
});
Object.defineProperty(exports, 'mosaicPluginRef', {
  enumerable: true,
  get: function () {
    return mosaicPluginRef;
  }
});
//# sourceMappingURL=mosaic-BKNRxVAe.cjs.map