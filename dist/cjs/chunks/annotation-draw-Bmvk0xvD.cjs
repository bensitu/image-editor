...
const require_annotation = require('./annotation-Bk5PWzaX.cjs');
...
function finiteRange(value, label, minimum, maximum) {
	if (typeof value !== "number" || !Number.isFinite(value) || value < minimum || value > maximum) throw new require_annotation.AnnotationValidationError(`${label} must be from ${minimum} to ${maximum}.`);
	return value;
}
function integerRange(value, label, minimum, maximum) {
	if (!Number.isSafeInteger(value) || Number(value) < minimum || Number(value) > maximum) throw new require_annotation.AnnotationValidationError(`${label} must be an integer from ${minimum} to ${maximum}.`);
	return Number(value);
}
function booleanValue(value, label) {
	if (typeof value !== "boolean") throw new require_annotation.AnnotationValidationError(`${label} must be boolean.`);
	return value;
}
function styleString(value, label, allowEmpty = false) {
	if (typeof value !== "string" || !allowEmpty && value.length === 0 || value.length > 128 || [...value].some((character) => character.charCodeAt(0) < 32)) throw new require_annotation.AnnotationValidationError(`${label} is invalid.`);
	return value;
}
function lineCap(value) {
	if (value === "butt" || value === "round" || value === "square") return value;
	throw new require_annotation.AnnotationValidationError("Draw line cap is invalid.");
}
function lineJoin(value) {
	if (value === "bevel" || value === "round" || value === "miter") return value;
	throw new require_annotation.AnnotationValidationError("Draw line join is invalid.");
}
function subMode(value) {
	if (value === "brush" || value === "erase") return value;
	throw new require_annotation.AnnotationValidationError("Draw sub-mode is invalid.");
}
...
function resolveBrushConfiguration(value = {}, base = defaultBrush) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError("Draw brush configuration must be a plain object.");
	const allowed = new Set(Object.keys(defaultBrush));
	if (Object.keys(value).some((key) => !allowed.has(key))) throw new require_annotation.AnnotationValidationError("Draw brush configuration contains unknown keys.");
	const merged = {
		...base,
		...value
	};
...
function resolveEraserConfiguration(value = {}, base = defaultEraser) {
	if (!isPlainRecord(value)) throw new require_annotation.AnnotationValidationError("Eraser configuration must be a plain object.");
	const allowed = new Set(Object.keys(defaultEraser));
	if (Object.keys(value).some((key) => !allowed.has(key))) throw new require_annotation.AnnotationValidationError("Eraser configuration contains unknown keys.");
	const merged = {
		...base,
		...value
	};
...
function normalizePoints(value, maximumCount) {
	if (!Array.isArray(value) || value.length < 2 || value.length > maximumCount) throw new require_annotation.AnnotationValidationError("Draw path point data is invalid.");
	try {
		return Object.freeze(value.map(normalizeDrawPoint));
	} catch {
		throw new require_annotation.AnnotationValidationError("Draw path contains an invalid point.");
	}
}
function isSerializedDraw(value) {
	...
	try {
		...
		if (value.version !== 1 || !isPlainRecord(serializedObject) || !require_safe_fabric_serialization.isSafeSerializedFabricObject(serializedObject, { rootTypes: ["path"] })) return false;
...
var DrawAnnotationController = class {
	constructor(host, authoring, options) {
		...
		if (!isPlainRecord(options)) throw new require_annotation.AnnotationValidationError("Draw options must be a plain object.");
		if (Object.keys(options).some((key) => key !== "brush" && key !== "eraser")) throw new require_annotation.AnnotationValidationError("Draw options contain unknown keys.");
		...
		return Object.freeze({
			...
			codec: {
				...
				deserialize: async (value, context) => {
					if (!isSerializedDraw(value)) throw new require_annotation.AnnotationValidationError("Serialized Draw data is malformed.");
					const object = (await context.fabric.util.enlivenObjects([value.object]))[0];
					if (!(object instanceof context.fabric.Path)) throw new require_annotation.AnnotationValidationError("Fabric did not restore a Draw path.");
			...
			stateCodec: {
				...
				serialize: (object, context) => {
					var _a, _b;
					const points = normalizePoints(object.editorDrawPoints, 65536).map((point) => Object.freeze(context.toImageNormalized(require_overlay.objectPointToCanvas(object, point))));
					return Object.freeze({
				...
				deserialize: (value, context) => {
					if (!isDrawStateGeometry(value.geometry) || !isDrawStateData(value.data)) throw new require_annotation.AnnotationValidationError("Serialized Draw Annotation State data is malformed.");
	...
	enter(options = {}) {
		...
		if (this.session) throw new require_annotation.AnnotationValidationError("A Draw session is already active.");
		if (!isPlainRecord(options)) throw new require_annotation.AnnotationValidationError("Draw enter options must be a plain object.");
		if (Object.keys(options).some((key) => key !== "subMode")) throw new require_annotation.AnnotationValidationError("Draw enter options contain unknown keys.");
		this.session = {
	...
	beginStroke(value) {
		...
		if (session.points.length > 0) throw new require_annotation.AnnotationValidationError("A Draw stroke is already active.");
		let point;
		try {
			point = normalizeDrawPoint(value);
		} catch {
			throw new require_annotation.AnnotationValidationError("Draw point coordinates must be finite and bounded.");
		}
	...
	appendStroke(value) {
		...
		try {
			...
			throw new require_annotation.AnnotationValidationError(error instanceof Error ? error.message : "Draw point limit was exceeded.");
		}
		this.refreshPreview(session);
	}
	async endStroke() {
	...
	configureBrush(patch) {
		...
		if ((_a = this.session) === null || _a === void 0 ? void 0 : _a.points.length) throw new require_annotation.AnnotationValidationError("Cancel the active Draw stroke before configuring it.");
		this.brush = resolveBrushConfiguration(patch, this.brush);
	}
	configureEraser(patch) {
		var _a;
		this.assertActive("configure Eraser");
		if ((_a = this.session) === null || _a === void 0 ? void 0 : _a.points.length) throw new require_annotation.AnnotationValidationError("Cancel the active Eraser stroke before configuring it.");
	...
	requireSession(operation) {
		...
		if (!this.session) throw new require_annotation.AnnotationValidationError(`Cannot ${operation} without a Draw session.`);
		return this.session;
	}
	requireActiveStroke(operation) {
		const session = this.requireSession(operation);
		if (session.points.length === 0) throw new require_annotation.AnnotationValidationError(`Cannot ${operation} without an active stroke.`);
		return session;
	}
	assertImageLoaded() {
		if (!this.host.isImageLoaded()) throw new require_annotation.AnnotationValidationError("Draw Annotation requires a loaded image.");
	}
	assertActive(operation) {
		if (this.disposed) throw new require_annotation.AnnotationValidationError(`Cannot ${operation} after disposal.`);
	}
};
...
const drawAnnotationPluginRef = require_core_capabilities.definePluginRef("annotation:draw", "1.0.0");
function drawAnnotationPlugin(options = {}) {
	...
	return require_sdk.definePlugin({
		...
		manifest: {
			...
			engine: "^3.0.0",
			requiresPlugins: [require_annotation.annotationFoundationRef],
			requires: [
				{
					token: require_annotation.ANNOTATION_AUTHORING_CAPABILITY,
					range: "^1.0.0"
				},
				{
		...
		setupMode: "sync",
		setup(context) {
			const authoring = context.capabilities.require(require_annotation.ANNOTATION_AUTHORING_CAPABILITY);
			...
			const image = context.capabilities.require(require_core_capabilities.BASE_IMAGE_INFO_CAPABILITY);
			...
			context.disposables.add(context.tools.register({
				...
				canRunOperation: (operationId) => operationId.startsWith("annotation-draw:") || operationId.startsWith("annotation:") || operationId.endsWith(":enter") || operationId === require_core_capabilities.coreOperationIds.loadImage || operationId === require_core_capabilities.coreOperationIds.commitLoadImage || operationId === require_core_capabilities.coreOperationIds.loadState || operationId === require_core_capabilities.coreOperationIds.export || operationId === require_core_capabilities.historyOperationIds.undo || operationId === require_core_capabilities.historyOperationIds.redo
			}));
...
//#endregion
Object.defineProperty(exports, 'drawAnnotationPlugin', {
  enumerable: true,
  get: function () {
    return drawAnnotationPlugin;
  }
});
Object.defineProperty(exports, 'drawAnnotationPluginRef', {
  enumerable: true,
  get: function () {
    return drawAnnotationPluginRef;
  }
});
//# sourceMappingURL=annotation-draw-Bmvk0xvD.cjs.map