...
const require_core_capabilities = require('./core-capabilities-DfizIg7d.cjs');
const require_image_budget = require('./image-budget-BCsM4W1R.cjs');
const require_sdk = require('./sdk-DULljMMa.cjs');
...
function validateKeys(value, allowed, path) {
	for (const key of Reflect.ownKeys(value)) {
		...
		if (require_plugin_identifier.isDangerousStateKey(key)) throw new FilterDefinitionError(`Filter definition contains dangerous key "${key}".`, path);
...
function normalizeDefinition(value, index) {
	const path = `$[${index}]`;
	if (!isRecord$1(value)) throw new FilterDefinitionError("Each Filter definition must be a plain object.", path);
...
function normalizeFilterBakeOptions(options, sourceMimeType) {
	...
	for (const key of Object.keys(record)) {
		if (require_plugin_identifier.isDangerousStateKey(key)) throw new FilterBakeValidationError(`Filter bake options contain dangerous key "${key}".`);
...
async function renderBakedImage(fabric, baseImage, definitions, options, imageInfo, policy, signal) {
	...
	if (width > policy.maxExportDimension || height > policy.maxExportDimension || !require_image_budget.isPixelAreaWithinBudget(width, height, pixelBudget)) throw new FilterBakeValidationError("Filtered Raster dimensions exceed the Core policy.");
	...
	try {
		...
		try {
			...
			if (require_error.hasErrorName(error, "SecurityError")) throw new FilterBakeValidationError("Filtered Raster pixels cannot be exported because canvas access is blocked.", error);
			throw error;
		}
		if (encodedBytes(dataUrl) > policy.maxInputBytes) throw new FilterBakeValidationError("Filtered Raster exceeds the Core input budget.");
...
function validateStateKeys(value) {
	for (const key of Object.keys(value)) {
		if (require_plugin_identifier.isDangerousStateKey(key)) return `Filters state contains dangerous key "${key}".`;
...
var FiltersController = class {
	...
	normalizeConfigurationPatch(patch) {
		var _a, _b, _c;
		if (!isRecord(patch)) throw new TypeError("[ImageEditor] Filters configuration patch must be a plain object.");
		for (const key of Object.keys(patch)) {
			if (require_plugin_identifier.isDangerousStateKey(key)) throw new TypeError(`[ImageEditor] Filters configuration contains dangerous key "${key}".`);
...
const filtersPluginRef = require_core_capabilities.definePluginRef("plugin:filters", "1.0.0");
function filtersPlugin(options = {}) {
	...
	return require_sdk.definePlugin({
		...
		setup(context) {
			...
			const exports = context.capabilities.require(require_core_capabilities.EXPORT_CONTRIBUTION_CAPABILITY);
			...
			context.capabilities.provide(require_sdk.VISIBLE_RASTER_BAKE_CAPABILITY, visibleRasterBake, { version: require_sdk.VISIBLE_RASTER_BAKE_CAPABILITY.version });
...
//#endregion
Object.defineProperty(exports, 'FilterBakeValidationError', {
  enumerable: true,
  get: function () {
    return FilterBakeValidationError;
  }
});
Object.defineProperty(exports, 'FilterDefinitionError', {
  enumerable: true,
  get: function () {
    return FilterDefinitionError;
  }
});
Object.defineProperty(exports, 'FilterImplementationError', {
  enumerable: true,
  get: function () {
    return FilterImplementationError;
  }
});
Object.defineProperty(exports, 'FiltersPluginDisposedError', {
  enumerable: true,
  get: function () {
    return FiltersPluginDisposedError;
  }
});
Object.defineProperty(exports, 'FiltersPreviewMissingError', {
  enumerable: true,
  get: function () {
    return FiltersPreviewMissingError;
  }
});
Object.defineProperty(exports, 'MAX_SUPPORTED_FILTER_COUNT', {
  enumerable: true,
  get: function () {
    return MAX_SUPPORTED_FILTER_COUNT;
  }
});
Object.defineProperty(exports, 'SUPPORTED_FILTER_TYPES', {
  enumerable: true,
  get: function () {
    return SUPPORTED_FILTER_TYPES;
  }
});
Object.defineProperty(exports, 'areFilterDefinitionsEqual', {
  enumerable: true,
  get: function () {
    return areFilterDefinitionsEqual;
  }
});
Object.defineProperty(exports, 'filtersPlugin', {
  enumerable: true,
  get: function () {
    return filtersPlugin;
  }
});
Object.defineProperty(exports, 'filtersPluginRef', {
  enumerable: true,
  get: function () {
    return filtersPluginRef;
  }
});
Object.defineProperty(exports, 'normalizeFilterDefinitions', {
  enumerable: true,
  get: function () {
    return normalizeFilterDefinitions;
  }
});
//# sourceMappingURL=filters-Crf0HOeu.cjs.map