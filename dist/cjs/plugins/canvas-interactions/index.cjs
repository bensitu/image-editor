...
var InteractionRuntime = class {
	...
	down(sample) {
		...
		try {
			...
			this.onStatusChange();
			if (isPromiseLike(claim.started)) require_core_capabilities.observePromise(claim.started, (error) => {
				var _a;
	...
	invoke(owner, operation, task, complete = false) {
		try {
			const result = task();
			if (isPromiseLike(result)) require_core_capabilities.observePromise(Promise.resolve(result).then(() => {
	...
	handleError(error, bindingId, operation) {
		...
		if (owner) try {
			...
			if (isPromiseLike(cleanup)) require_core_capabilities.observePromise(cleanup, (cleanupError) => {
...
var PointerCoordinateMapper = class {
	...
	toImagePoint(scenePoint) {
		...
		if (!require_core.isFiniteAffineMatrix(matrixValue)) return null;
		const local = require_core.applyAffineToPoint(require_core.invertAffine(matrixValue), scenePoint);
...
const canvasInteractionsPluginRef = require_core_capabilities.definePluginRef("plugin:canvas-interactions", "1.0.0");
	...
	return require_sdk.definePlugin({
		...
		setup(context) {
			controller = new CanvasInteractionsController(Object.freeze({
				...context.capabilities.require(require_core_capabilities.CANVAS_READ_CAPABILITY),
				...context.capabilities.require(require_core_capabilities.BASE_IMAGE_READ_CAPABILITY),
				...context.capabilities.require(require_core_capabilities.CORE_DIAGNOSTICS_CAPABILITY)
