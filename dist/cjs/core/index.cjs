...
class GeometryMutationCoordinator {
    ...
    run(request) {
        ...
        if (!this.usedMutationIds.start(request.id)) {
            return Promise.reject(new errors.GeometryMutationError(request.id, 'mutation id has already been used.'));
        }
        const controller = new AbortController();
...
class DocumentMutationCoordinator {
    ...
    run(request) {
        ...
        if (!this.usedTransactionIds.start(normalized.id)) {
            return Promise.reject(new errors.DocumentMutationRegistrationError(`Transaction id "${normalized.id}" has already been used.`, normalized.id));
        }
        const controller = new AbortController();
