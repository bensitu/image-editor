...
| `maxInputBytes`                   | `50000000`            | Maximum encoded file bytes or decoded Base64 payload bytes accepted before image decode, including image resources in public state restoration. Invalid values fall back to this default.                                                                                                     |
| `maxInputPixels`                  | `50000000`            | Maximum source image pixel count accepted from PNG/JPEG/WebP headers before image decode, including image resources in public state restoration. JPEG file metadata segments are traversed until the dimensions marker is found. Invalid values fall back to this default.                    |
...
| `initialImageBase64`              | `null`                | Base64-encoded PNG, JPEG, or WebP data URL whose asynchronous load starts during `init()`. `init()` returns before the image necessarily finishes loading; use `onImageLoaded` for readiness.                                                                                                 |
