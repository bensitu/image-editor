...
function parseArguments(args: readonly string[]): CliOptions | null {
    ...
    for (let index = 1; index < args.length; index += 1) {
        ...
        if (argument === '--write' || argument === '--dry-run' || argument === '--diff') {
            if (selectedMode) throw new Error('Select exactly one codemod mode.');
            mode = argument.slice(2) as CodemodMode;
            selectedMode = true;
            continue;
        }
        if (argument === '--report') {
    ...
    if (targets.length === 0) throw new Error('At least one source path is required.');
...
async function main(): Promise<void> {
    ...
    if (options.mode === 'diff') {
        for (const file of report.files) {
            if (file.diff) process.stdout.write(file.diff);
        }
    }
    if (options.reportPath) await writeCodemodReport(options.reportPath, report);
    ...
    for (const unresolved of report.files.flatMap((file) => file.unresolved)) {
        process.stderr.write(
            `${unresolved.file}:${unresolved.line}:${unresolved.column} ${unresolved.code} ${unresolved.message}\n`,
        );
    }
    if (report.unresolvedCount > 0) process.exitCode = 2;
    else if (options.mode !== 'write' && report.filesChanged > 0) process.exitCode = 1;
}
